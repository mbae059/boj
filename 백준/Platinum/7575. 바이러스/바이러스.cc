...
int KMP(const vi& v, const vi& pattern, const vi& pi) {
    int j = 0;
    int ret = 0;
    for(int i=0;i<v.size();i++) {
        while(j>0 && v[i]!=pattern[j]) j = pi[j-1];
        if(v[i]==pattern[j]) {
            j++;
            if(j==pi.size()) {
                ret = pi.size();
...
void Solve() {
    ...
    sort(all(v), [](const vi& l, const vi& r) {
        return l.size() < r.size();
    });

    vector<tuple<vi,vi, vi, vi>> pi;
    for(int i=0;i+K-1<v.front().size();i++) {
        vi tv(v.front().begin()+i, v.front().begin()+K+i);
        vi rtv = tv;
        reverse(all(rtv));
        pi.pbk({tv, getPi(tv), rtv, getPi(rtv)});
    }

    for(auto [tv, tpi, rtv, rtpi] : pi) {
        [&] () {
            for(int i=1;i<N;i++) {
                int mx = max(KMP(v[i], tv, tpi), KMP(v[i], rtv, rtpi));
                if(mx<K) return;
            }
            print("YES");
            exit(0);
