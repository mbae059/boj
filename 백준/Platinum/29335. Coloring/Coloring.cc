...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        if(mp.find(a)!=mp.end()) {
            auto piter = iter;
            advance(piter,-1);
            int x = (*iter).first - (*piter).first;
            int y = (*iter).second;
            area -= x*y;
            temp = 1e10;
        }
        mp[a] = b;
        iter = mp.lower_bound(a);
        vector<mii::iterator> v;

        auto niter = iter;
        advance(niter,1);
        
        if(niter==mp.end() || temp==1e10) temp = 0;
        else temp = (*niter).second;
        while(iter!=mp.begin() && (*iter).second<=b) {
            auto piter = iter;
            advance(piter, -1);
