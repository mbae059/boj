...
struct Trie {
    ...
    void insert(const string& str, int k=0) {
        ...
        int curKey = getKey(str[k]); //or 'a' or '0'
...
void Solve() {
    cin >> N >> M;
    string parent, pattern; cin >> parent >> pattern;

    set<string> s;
    for(int i=0;i<M;i++) {
        for(int j=i;j<M;j++) {
            string mutate = pattern;
            reverse(mutate.begin()+i, mutate.begin()+j+1);
            s.insert(mutate);
        }
    }
    ...
    while(!q.empty()) {
        ...
        rep(i,0,3) {
            ...
            if(cur==root) {
                ...
                Trie *dest = cur->fail;
                while(dest!=root && dest->next[i]==nullptr) {
                    dest = dest->fail;
                }
    ...
    for(auto c : parent) {
        int idx = getKey(c);
        while(cur!=root && cur->next[idx]==nullptr) cur = cur->fail;
