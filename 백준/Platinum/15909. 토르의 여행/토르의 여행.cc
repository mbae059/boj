...
void dfs(int cur) {
    ...
    if(cur*2<=N) {
        for(auto i : v[cur*2]) {
            v[cur].pbk(i+energy[cur]);
        }
    }
    if(cur*2+1<=N) {
        for(auto i : v[cur*2+1]) {
            v[cur].pbk(i+energy[cur]);
        }
    }
    v[cur].pbk(energy[cur]);
    sort(all(v[cur]));
}

ll getAnswer(ll cur, ll D, ll pre) {
    if(cur==0) return 0;
    ll ret = 0;

    if(cur*2==pre) {
        ret += upper_bound(all(v[cur*2+1]), D-energy[cur]) - lower_bound(all(v[cur*2+1]), D-energy[cur]);
    }
    else {
        ret += upper_bound(all(v[cur*2]), D-energy[cur]) - lower_bound(all(v[cur*2]), D-energy[cur]);
    }
    if(energy[cur]==D) ret++;
    return ret + getAnswer(cur/2, D-energy[cur], cur);
}
void Solve() {
    cin >> H;
    N = (1<<H)-1;
    rep(i,1,N) cin >> energy[i];
    ...
    while(Q--) {
        ll A, D; cin >> A >> D;
        ll answer = upper_bound(all(v[A]), D) - lower_bound(all(v[A]), D);
        answer += getAnswer(A/2, D-energy[A], A);
        cout << answer << endl;
    }
}
int main() {
