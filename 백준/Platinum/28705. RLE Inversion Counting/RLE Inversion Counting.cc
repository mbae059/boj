...
void Solve() {
    ...
    sort(all(gcompress));
    gcompress.erase(unique(all(gcompress)), gcompress.end());
    mii gidx;
    ...
    for(int i=1;i<=M;i++) {
        ...
        for(auto val : v[i]) {
            val = idx[val];
            sum1 += s.query(1,val+1,sz,1,sz);
            s.update(1,1,sz,val, 1);
        }
        //take care of sum2
        for(int j=1;j<=sz;j++) {
            int total = v[i].size();
            int val = s.query(1,j,j,1,sz);
            sum2 += val * (total - val);
        }
        sum2/=2;
        //take care of sum3
        for(auto val : v[i]) {
            val = gidx[val];
            sum3 += segtree.query(1,val+1,mx,1,mx);
        }
        //update global segtree
        for(auto val : v[i]) {
            val = gidx[val];
            segtree.update(1,1,mx,val, k[i]);
        }
        answer += sum1 * k[i];
        answer += sum2 * k[i] * (k[i]-1) / 2;
        answer += sum3 * k[i];
    }
    cout << answer.val();
}
