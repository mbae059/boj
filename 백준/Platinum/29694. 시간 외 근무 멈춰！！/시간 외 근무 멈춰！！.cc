...
vpii getPair(int a, int b, int m) {
    vpii ret;
    for(int i=0;i<=m/a;i++) {
        if((M-i*a)%b) continue;
        int j = (M-i*a)/b;
...
void getAnswer() {
    int answer=0;
    if(A==0 && B==0 && M==0) { //반드시 평일 시간 외 근무함.
        int cnt1 = 0;
        int cnt2 = 0;
        rep(i,1,l) {
            if(isWeekend(i)) cnt2++;
    ...
    else if(A==0 && B==0 && M!=0) no(); //cannot reach M
    else if(A==0 && B!=0 && M==0) { //반드시 평일 시간 외 근무함. 주말은 반드시 X
        int cnt = 0;
        rep(i,1,l) {
            if(!isWeekend(i)) cnt+=2;
            cnt-=work[i];
            if(cnt<0) no();
        }
    }
    else if(A==0 && B!=0 && M!=0) { //반드시 M%B만큼 시간외 근무해야함
        if(M%B) no();
        int cnt1=0;
        int cnt2=M/B;
        rep(i,1,l) {
