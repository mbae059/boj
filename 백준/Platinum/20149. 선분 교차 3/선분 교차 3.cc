...
bool isSameGradient(Line l1, Line l2) {
    ll a = l1.p2.first - l1.p1.first;
    ll b = l1.p2.second - l1.p1.second;

    ll x = l2.p2.first - l2.p1.first;
    ll y = l2.p2.second - l2.p1.second;

    if(a==0 && x==0) return true;
    if(b==0 && y==0) return true;
    if(a==0 || x==0 || b==0 || y==0) return false;
...
void Solve() {
    ...
    l1.setOrder();
    l2.setOrder();
    int val = LineInterSection(l1, l2);
    if(val==0) {
        cout << 0;
        return;
    }
    if(val && isSameGradient(l1, l2)) {
        cout << 1 << endl;
        if(l2.p2==l1.p1) {
            cout << l2.p2.first << " " << l2.p2.second;
        }
        else if(l1.p2==l2.p1) {
            cout << l1.p2.first << " " << l1.p2.second;
        }
        return;
    }
    cout << 1 << endl;
    ...
    cout << fixed << setprecision(12) << x/p << " " << y/p;
