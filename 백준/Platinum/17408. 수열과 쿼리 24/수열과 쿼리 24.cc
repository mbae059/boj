...
struct Segtree {
    ...
    pii segment(int node, int nodeLeft, int nodeRight) {
        if (nodeLeft == nodeRight) {
            return s[node] = {a[nodeLeft], nodeLeft};
        }
        int mid = (nodeLeft + nodeRight) / 2;
        pii pl = segment(node*2, nodeLeft, mid);
        pii pr = segment(node*2+1, mid+1, nodeRight);

        if(pl.first>pr.first) s[node] = pl;
        else s[node] = pr;
        return s[node];
    }
    void update(int node, int nodeLeft, int nodeRight, int idx, T num) {
        ...
        if(s[node*2] > s[node*2+1]) s[node] = s[node*2];
        else s[node] = s[node*2+1];
    }
    pii query(int node, int l, int r, int nodeLeft, int nodeRight) {
        if (nodeRight < l || r < nodeLeft) return {0,-1}; //could be 0, 1e9, -1e9
        if (l <= nodeLeft && nodeRight <= r) return s[node];
        int mid = nodeLeft + nodeRight >> 1;
        
        pii pl = query(node*2, l, r, nodeLeft, mid);
        pii pr = query(node*2+1, l, r, mid+1, nodeRight);
...
void Solve() {
    cin >> N;
    Segtree<int, op> segtree(N);
    rep(i,1,N) cin >> segtree.a[i];
    segtree.segment(1,1,N);
    cin >> Q;
    while(Q--) {
        int op, a, b; cin >> op >> a >> b;
        if(op==1) segtree.update(1,1,N,a,b);
        else {
            pii mx1 = segtree.query(1,a,b,1,N);
            segtree.update(1,1,N,mx1.second, 0);
            pii mx2 = segtree.query(1,a,b,1,N);
            segtree.update(1,1,N,mx1.second, mx1.first);
            print(mx1.first + mx2.first);
        }
    }
}
int32_t main() {
