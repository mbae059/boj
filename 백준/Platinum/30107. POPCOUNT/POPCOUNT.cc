...
void Solve() {
    ...
    int answer = A * prefix[N].first + B * prefix[N].second;
    print(answer);
}
int32_t main() {
    ...
    rep(i,1,500000) {
        auto iter = upper_bound(all(v),i);
        iter--;
        auto val = *iter;
        dp[i] = {pc[val], pc[i-val]};
    }
    rep(i,1,500000) {
        prefix[i].first = prefix[i-1].first + dp[i].first;
        prefix[i].second = prefix[i-1].second + dp[i].second;
    }
    cin >> T;
    while(T--) Solve();
}