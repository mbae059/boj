...
void Solve() {
    ...
    p_q<tiii, vtiii, decltype(cmp)> pq;
    spii visited;
    pq.push({src, -1, 0});
    while(!pq.empty()) {
        auto [cur, before, cost] = pq.top();
        pq.pop();
        if(cur==dest) {
            cout << cost;
            return;
        }
        for(auto [nextNode, dist] : edge[cur]) {
            int nextDistance = cost + dist;
            // cout << visited.contains({cur,nextNode}) << " " << s.contains({before, cur, nextNode}) << endl;
            if(visited.contains({cur, nextNode})) continue;
            if(s.contains({before, cur, nextNode})) continue;
            
            visited.insert({cur, nextNode});
            pq.push({nextNode, cur, nextDistance});
        }
    }
    cout << -1;
}
