...
void Solve() {
    ...
    int x, y; cin >> x >> y;
    bool chk[4] {1,1,1,1}; //up, down, left, right
    for(auto [a,b] : v) {
        int difx = a-x;
        int dify = b-y;
        ...
        if(difx>=0 && dify>=0) {
            if(absdifx>=absdify) chk[3] = 0;
            if(absdifx<=absdify) chk[0] = 0;
        }
        if(difx>=0 && dify<=0) {
            if(absdifx>=absdify) chk[3] = 0;
            if(absdifx<=absdify) chk[1] = 0;
        }
        if(difx<=0 && dify>=0) {
            if(absdifx>=absdify) chk[2] = 0;
            if(absdifx<=absdify) chk[0] = 0;
        }
        if(difx<=0 && dify<=0) {
            if(absdifx>=absdify) chk[2] = 0;
    ...
    if(*max_element(chk,chk+4)) cout << "YES";
    else cout << "NO";
}
int32_t main() {
