...
ll solve(int l, int r) {
	...
	ll maxValue = max(solve(l, mid), solve(mid + 1, r));

	int left = mid;
	int right = mid + 1;

	ll minValue = min(v[left], v[right]);
	...
	while (l < left || right < r) {
		if (right < r && ((l == left) || v[left - 1] < v[right + 1])) {
			minValue = min(minValue, ll( v[++right]));
			sum += v[right];
		}
		else {
			minValue = min(minValue, ll(v[--left]));
			sum += v[left];
		}
