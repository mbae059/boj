...
void Solve() {
	...
	answer[1] = 1;
	cmultiple.pbk(1);
	for(int i=1, cc=C;i<=M;i++, cc = cc * C % mod) {
		answer[i] += cc;
		cmultiple.pbk(cc);
	}
	vi nonC;
	...
	// print(nonC);
	// cout << endl;
	// for(int i=0;i<cmultiple.size();i++) {
	// 	for(auto j : nonC) {
	// 		if(i+j+1>M) break;
	// 		answer[i+j+1] += cmultiple[i] * j % mod;
	// 	}
	// }
	...
	rep(i,1,M) {
		tanswer[i] += tanswer[i-1];
		tanswer[i] %= mod;
	}
	rep(i,1,M) {
		tanswer[i] += tanswer[i-1] * C;
		tanswer[i] %= mod;
	}
