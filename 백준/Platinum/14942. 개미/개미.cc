...
//vpii edge[MAX] {};
//int d[MAX][18];
pll parent[MAX][18]; //18 is log2(MAX)
ll level[MAX];
ll maxLevel;
ll energy[MAX] {};
bool visited[MAX] {};
...
void dfs(int cur) { //set tree
    visited[cur] = 1;
    for(auto [next, dist] : edge[cur]) {
        if(visited[next]) continue;
        parent[next][0] = {cur, dist};
        dfs(next);
    }
}
...
int sparse(int cur) {
    ...
    while(cur!=1) {
        for(int i=maxLevel;i>=0;i--) {
            if(energy[src]-parent[cur][i].second>=0 && parent[cur][i].first!=0) {
                energy[src]-=parent[cur][i].second;
                cur = parent[cur][i].first;
                break;
            }
            if(i==0) return cur;
        }
...
void Solve() {
    ...
    rep(j,1,maxLevel) {
        rep(i,1,N) {
            auto p = parent[i][j-1];
            parent[i][j] = {parent[p.first][j-1].first, parent[p.first][j-1].second + p.second};
        }
    }
