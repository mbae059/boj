...
struct Answer {
    ...
    void process(const string& temp) {
        if(str.size() < temp.size()) {
            str = temp;
        }
        else if(str.size()==temp.size()) {
            str = (str > temp ? temp : str);
        }
        point += Points[temp.size()];
        cnt++;
    }

    void print() {
        cout << point << " " << str << " " << cnt << endl;
    }
...
struct Trie {
    ...
    void find(int y, int x, string str) {
        ...
        rep(i,0,7) {
            ...
            if(next[nextc-'A']) {
                string nextStr = str + nextc;
                next[nextc-'A']->find(nexty, nextx, nextStr);
            }
        }
        visited[y][x] = 0;
    }
