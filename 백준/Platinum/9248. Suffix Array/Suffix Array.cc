...
*
*/
#define MAX 500001 //string length
int g[MAX] {}; 
int tg[MAX] {}; 
int SA[MAX] {};
int RSA[MAX] {};
int lcp[MAX] {};
int t;
bool cmp(int a, int b) { //if true, that means the group is different. if false, the group which contains a and b is same
    if(g[a]==g[b]) {
        return g[a+t] < g[b+t];
    }
    return g[a] < g[b];
...
void getLCP(const string& str) {
    ...
    for(int i=0;i<N;i++) {
        ...
        if(k==0) continue; //k is the shortest string
        int j = SA[k-1];
