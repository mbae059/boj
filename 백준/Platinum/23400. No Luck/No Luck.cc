...
void segment(int node, int nodeLeft, int nodeRight) {
    ...
    s[node].resize(s[node*2].size() + s[node*2+1].size());
    merge(all(s[node*2]), all(s[node*2+1]), s[node].begin());
}

int query(int node, int nodeLeft, int nodeRight, int l, int r, int c) {
    if(nodeRight<l || r<nodeLeft) return 0;
    if(l<=nodeLeft && nodeRight<=r) {
        return nodeRight - nodeLeft + 1 - (lower_bound(all(s[node]), c) - s[node].begin());
    }
    int mid = midpoint(nodeLeft, nodeRight);
    return query(node*2, nodeLeft, mid, l,r,c) + query(node*2+1,mid+1,nodeRight,l,r,c);
}
void Solve() {
    cin >> N >> Q;
    rep(i,1,N) cin >> arr[i];
    ...
    while(Q--) {
        int a, b, c; cin >> a >> b >> c;
        if(arr[a]>=b) cout << 0 << endl;
        else cout << query(1,1,N,a+1,a+c,b) << endl;
    }
}
int32_t main() {
