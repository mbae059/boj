...
void Solve() {
    ...
    while(Q--) {
        ...
        dq.pbk({0, 1});
        rep(i,2,N) {
            if(!dq.empty() && i-dq.front().second>K) dq.pop_front();

            dp[i] = v[dq.front().second]<=v[i] ? dq.front().first+ 1 : dq.front().first;
            
            while(!dq.empty() ) {
                if(dq.back().first>dp[i]) dq.pop_back();
                else if(dq.back().first==dp[i] && v[dq.back().second]<=v[i]) dq.pop_back();
                else break;
            }
            dq.pbk({dp[i], i});
        }
