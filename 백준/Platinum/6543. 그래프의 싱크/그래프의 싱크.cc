...
#define MAX 5001 //when used in 2-sat, you have to double node because not only x1~x100 is required but also Nx1~Nx100
vvi SCC;
int d[MAX];
bool finished[MAX];
vi edge[MAX];
...
int outDegree[MAX]; //index refers to SN
...
void Solve() {
    while(true) {
        ...
        for(int i=0;i<SN;i++) {
            if(outDegree[i]==0) {
                for(auto j : SCC[i]) answer.insert(j);
            }
        }
