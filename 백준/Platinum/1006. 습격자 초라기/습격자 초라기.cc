...
void dp(int cur) {
    for(int i=cur;i<=N;i++) {
        ...
        a[i+1] = min(a[i+1], a[i] + (arr[0][i]+arr[1][i] <= W ? 1 : 2));
        if(i>1 && arr[0][i-1] + arr[0][i] <= W && arr[1][i-1] + arr[1][i] <= W) a[i+1] = min(a[i+1], a[i-1] + 2);

        b[i+1] = a[i+1]+1;
        c[i+1] = a[i+1]+1;

        if(arr[0][i]+arr[0][i+1] <= W) b[i+1] = min(b[i+1], c[i]+1);
        if(arr[1][i]+arr[1][i+1] <= W) c[i+1] = min(c[i+1], b[i]+1);
    }
}
void Solve() {
    cin >> N >> W;
    ...
    if(arr[0][1]+arr[0][N]<=W) {
        a[2] = 1;
        b[2] = 2;
        c[2] = (arr[1][1] + arr[1][2]<=W ? 1 : 2);
        dp(2);
        answer = min(answer, c[N]+1);
    }
    if(arr[1][1]+arr[1][N]<=W) {
        a[2] = 1;
        b[2] = (arr[0][1] + arr[0][2]<=W ? 1 : 2);
        c[2] = 2;
        dp(2);
        answer = min(answer, b[N]+1);
    }
