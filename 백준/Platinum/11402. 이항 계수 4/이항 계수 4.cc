...
ll comb(int n, int k) {
    ...
    if(k==0) return 1;
    if(k==1) return n;
    return ret = (comb(n-1, k-1) + comb(n-1, k)) % M;
}
void Solve() {
    cin >> N >> K >> M;

    vi lucaN = getLuca(N, M);
    vi lucaK = getLuca(K, M);
    memset(dp, -1, sizeof(dp));
    int l = min(lucaN.size(), lucaK.size());

    int answer=1;
    for(int i=0;i<l;i++) {
        answer = answer * comb(lucaN[i], lucaK[i]);
        answer %= M;
    }
    cout << answer;
}
