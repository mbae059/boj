...
#define MAX 1000003 //when used in 2-sat, you have to double node because not only x1~x100 is required but also Nx1~Nx100
vvi SCC;
int d[MAX];
bool finished[MAX];
int edge[MAX] {};
...
int dfs(int x) {
    ...
    else if (d[next] == 0) { //방문 안 한 이웃
        parent = min(parent, dfs(next));
    }
    else if (finished[next] == 0) { //처리 중인 이웃
...
int SCCedge[MAX]; //index refers to SN. Could be replaced with set if you don't want to overlap
int inDegree[MAX]; //index refers to SN
void SCCtopology_sort() {
    fill(SCCedge, SCCedge+MAX, -1);
    for(int i=1;i<=N;i++) { //id starts with 1
        int next = edge[i];
        if(next!= 0 && sn[next]!=sn[i] && SCCedge[sn[i]]==-1) {
            SCCedge[sn[i]] = sn[next]; //SN could be overlapped. Could be solved with set but it is often not needed
...
void bfs() {
    ...
    while(!q.empty()) {
        ...
        // cout << cur << " "<< dis << " " << SCCedge[cur] << endl;
        MM = max(MM, dis);

        int next = SCCedge[cur];
        if(next==-1) continue;
        cmpDegree[next] += 1;
        dp[next] = max(dp[next], dp[cur] + int(SCC[next].size()));

        if(cmpDegree[next]==inDegree[next]) {
           q.push(next);
        }
    }
    cout << MM;
