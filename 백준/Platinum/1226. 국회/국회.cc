...
void Solve() {
    ...
    while(cur!=0) {
        if(flag) {
            if(idx-v[cur].first<=sum/2 && idx-v[cur].first>=0 && dp[cur-1][idx-v[cur].first]) {
                idx-=v[cur].first;
                answer.pbk(v[cur].second);
                flag=0;
            }
        }
        else {
            if(idx-v[cur].first>=0 && dp[cur-1][idx-v[cur].first]) {
                idx-=v[cur].first;
                answer.pbk(v[cur].second);
            }
        }
