...
ll query(int node, int l, int r, int nodeLeft, int nodeRight) { //l and r is the range.
    ...
    if(s[node*2]==s[node]) return query(node*2, l, r, nodeLeft, mid);
    else return query(node*2+1, l,r,mid+1, nodeRight);
}
vi v;
void Solve() {
    cin >> N >> M;
    ...
    rep(i,1,2*M-1) {
        update(1, v[i], 1, 1000000, 1);
    }
    cout << query(1, 1, 1000000, 1, 1000000) << " ";
    for(int i=M+1;i<=N-M+1;i++) {
        update(1, v[i-M], 1, 1000000, -1);
        update(1, v[i+M-1], 1, 1000000, 1);
        cout << query(1, 1,1000000, 1, 1000000) << " ";
    }
}
int main() {
