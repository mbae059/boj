...
void Solve() {
    ...
    while(l<=r) {
        ...
        int sum = 0;
        for(auto [a,b,d] : tv) {
            if(dsu.isSameParent(a,b)) continue;
            dsu.merge(a,b);
            sum += d;
        }
        if(sum==K) {
            cout << "YES" << endl;
            for(int i=1;i<=mid;i++) cout << get<2>(v[i]) << endl;
            for(int i=mid+1;i<=M;i++) cout << get<3>(v[i]) << endl;
            return;
        }
        else if(sum<K) r = mid-1;
        else l = mid+1;
    }
    //now l is the index of critical edge
    int idx = l;
    l = get<2>(v[idx]);
    r = get<3>(v[idx]);
    ...
    while(l<=r) {
        ...
        tv.pbk({get<0>(v[idx]), get<1>(v[idx]), mid});
        sort(all(tv), cmp);
        DSU dsu(N);
        int sum = 0;
        for(auto [a,b,d] : tv) {
    ...
    tv.pbk({get<0>(v[idx]), get<1>(v[idx]), length});
    sort(all(tv), cmp);
    DSU dsu(N);
    int sum =0 ;
    for(auto [a,b,d] : tv) {
    ...
    if(sum==K) {
        cout << "YES" << endl;
        rep(i,1,idx-1) cout << get<2>(v[i]) << endl;
        cout << length << endl;
        rep(i,idx+1,M) cout << get<3>(v[i]) << endl;
    }
    else {
        cout << "NO";
    }
