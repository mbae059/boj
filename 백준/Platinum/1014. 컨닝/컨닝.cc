...
//////////////////////////////////////////////////////////
/*
//Bipartite matching, Hop-croft, O(E * sqrt(V))
//if for all N, K can match, then extend N points to N * K
#define MAX 10001
vi edge[MAX];
bool used[MAX];
...
int dp[11][1111] {}; //cur row and previous bit

bool chkSeat(int cur, int bit) {
    for(int i=1;i<=M;i++) {
        if((bit & (1<<(i-1)) && matrix[cur][i]=='x')) {
            return false;
        }
    }
    return true;
...
bool adjacentBit(int bit) {
    for(int i=0;i<M-1;i++) {
        if(3 & (bit >> i)==3) return true;
    }
    return false;
}
bool canCheat(int preBit, int curBit) {
    for(int i=0;i<M-1;i++) {
        int a = 3 & (preBit >> i);
        int b = 3 & (curBit >> i);
...
int dfs(int cur, uint32_t bit) {
    if(cur==N+1) return 0;
    if(!chkSeat(cur, bit)) return 0;
    if(dp[cur][bit]!=-1) return dp[cur][bit];
    dp[cur][bit] = 0;
    // cout << cur << " " << bit << endl;
    for(int i=0;i<(1<<M);i++) {
        if(adjacentBit(i)) continue;
        if(canCheat(bit, i)) continue;
        dp[cur][bit] = max(dp[cur][bit], dfs(cur+1, i) + popcount(bit));
    }
    return dp[cur][bit];
}
void Solve() {
