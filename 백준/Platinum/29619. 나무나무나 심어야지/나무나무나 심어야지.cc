...
bool done[MAX] {};
int b[MAX] {};
//for 1~N memset(done, 0, sizeof(done)), if(dfs(i)) cnt++;
bool dfs(int cur) {
    for(auto next : edge[cur]) {
...
void Solve() {
    ...
    rep(i,1,N) {
        if(idx[i]==0) continue;
        segtree.update(1,1,N,idx[i], fruit[i]);
    }
    reverse(all(query));
    vi answer;
    for(auto [op,i,j,w] : query) {
        if(op==1) {
            segtree.update(1,1,N, idx[j], 0);
        }
        else {
            auto [s,e] = euler[i];
            int val = segtree.query(1,s,e,1,N);
            answer.pbk(val==0 ? -1 : val);
        }
    }
    reverse(all(answer));
