...
void segment(int node, int l, int r) {
    if(l==r) {
        s[node].pbk(a[l]);
        return;
    }
    int mid = midpoint(l,r);
    segment(node*2, l, mid);
    segment(node*2+1, mid+1, r);
    
    int idx1 = 0;
    int idx2 = 0;

    while(idx1<s[node*2].size() && idx2<s[node*2+1].size()) {
        if(s[node*2][idx1]<s[node*2+1][idx2]) {
            s[node].pbk(s[node*2][idx1]);
            idx1++;
        }
        else {
            s[node].pbk(s[node*2+1][idx2]);
            idx2++;
        }
    }
    while(idx1<s[node*2].size()) {
        s[node].pbk(s[node*2][idx1]);
        idx1++;
    }
    while(idx2<s[node*2+1].size()) {
        s[node].pbk(s[node*2+1][idx2]);
        idx2++;
    }
}

int query(int node, int nodeLeft, int nodeRight, int l, int r, int value) {
    if(nodeRight<l || r<nodeLeft) return 0;
    if(l<=nodeLeft && nodeRight<=r) {
        int ret = nodeRight - nodeLeft + 1 - (upper_bound(all(s[node]), value) - s[node].begin());
        return ret;
    }
    int mid = midpoint(nodeLeft, nodeRight);
    return query(node*2, nodeLeft, mid, l, r, value) + query(node*2+1, mid+1, nodeRight, l, r, value);
}
void Solve() {
    cin >> N;
    rep(i,1,N) cin >> a[i];
