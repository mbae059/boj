...
//when using ETT, set global variable to 0 and do s[cur] = ++g; (do dfs) e[cur] = g;
struct Matrix {
...
void segment(int node, int nodeLeft, int nodeRight) {
    if(nodeLeft==nodeRight) {
        mst[node].pbk(color[rs[nodeLeft]]);
        return;
    }
    ...
    merge(all(mst[node*2]), all(mst[node*2+1]), mst[node].begin());
}

int query(int node, int nodeLeft, int nodeRight, int l, int r, int c) {
    if(nodeRight < l || r < nodeLeft) return 0;
    else if(l<=nodeLeft && nodeRight<=r) {
        return upper_bound(all(mst[node]), c) - mst[node].begin();
    }
    int mid = midpoint(nodeLeft, nodeRight);
