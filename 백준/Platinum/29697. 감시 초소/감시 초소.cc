...
void Solve() {
    ...
    int prefix[N+1] {};
    iota(p,p+N+1, 0);
    rep(i,1,N) prefix[i] = prefix[i-1] + p[i];
    int mx = upper_bound(prefix, prefix+N+1, P) - prefix - 1;
    int dp[N+1] {};
    fill(dp+1,dp+N+1,1e18);
    rep(i,1,N) {
        int l = max(1ll, i-mx+1);
        int r = i;
        deque<pii> dq;
        while(l<=i) {
            while(1 + getPeople(l,i-1) + getPeople(i+1,r+1) <= P && r+1<=N) r++;
            while(!dq.empty() && dq.back().first>=dp[l-1]+c[i]) {
                dq.pop_back();
            }
            dq.pbk({dp[l-1]+c[i], r});
            l++;
