...
void flip(int y, int x) {
    for(int i=0;i<5;i++) {
        ...
        tmatrix[ny][nx] = !tmatrix[ny][nx];
    }
}

bool isSetZero(int idx) {
    for(int j=0;j<M;j++) {
        if(tmatrix[idx][j]) return false;
    }
    return true;
}
...
void setAnswer() {
    vector<bool> tv;
    int cnt1 = count(all(answer), 1);
    rep(i,0,N-1) {
        rep(j,0,M-1) tv.pbk(temp[i][j]);
    }
    int cnt2 = count(all(tv),1);
    if(answer.empty()) answer = tv;
    else if(cnt1>cnt2) answer = tv;
    else if(cnt1==cnt2 && answer>tv) answer = tv;
}
...
void Solve() {
    ...
    for(int bits=0;bits<(1<<M);bits++) {
        memcpy(tmatrix, matrix, sizeof(matrix));
        for(int j=0;j<M;j++) {
            if(bits&(1<<j)) {
                flip(0, j);
                temp[0][j] = 1;
