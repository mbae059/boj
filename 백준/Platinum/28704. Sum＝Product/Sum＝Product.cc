...
void Solve() {
    ...
    auto total_sum = accumulate(a+1,a+N+1, 0);
    int answer=N;
    vi idx;
    rep(i,0,N+1) {
        if(a[i]!=1) idx.pbk(i);
    ...
    for(int l=1;l<idx.size()-1;l++) {
        int mul = 1;
        int sum = 0;
        for(int r=l;r<idx.size()-1;r++) {
            mul *= a[idx[r]];
            sum += a[idx[r]];
            if(mul>total_sum) break;
            if(l!=r) {
                int lones = idx[l] - idx[l-1] - 1;
                int rones = idx[r+1] - idx[r] - 1;
                int need = mul - (sum + idx[r]-idx[l]-1-(r-l+1-2));
                if(0<=need && need<=lones+rones) {
                    answer += min({need, lones+rones-need, lones, rones})+1;
                    // answer += lones+rones-need+1;
                }
            }
        }
    }
