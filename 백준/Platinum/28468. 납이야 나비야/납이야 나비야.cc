...
void Solve() {
    ...
    int answer=0;
    rep(i,1,N) {
        if(edge[i].size()<6) continue;
        pii v[N*N];
        int idx = 0;
        int arr[N+1] {};
        for(int j=0;j<edge[i].size();j++) { //N
            for(int k=j+1;k<edge[i].size();k++) { //N
                int a = edge[i][j];
                int b = edge[i][k];
                if(adjacent[a][b]) {
                    v[idx] = {a,b};
        ...
        for(int i=0;i<idx;i++) {
            auto [a,b] = v[i];
            int val = sz - arr[a] - arr[b] + 1;
            answer += val * (degree-4) * (degree-5) / 2;
        }
    }
    answer /= 2;
    cout << answer;
