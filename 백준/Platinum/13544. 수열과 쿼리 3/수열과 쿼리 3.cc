...
int query(int node, int nodeLeft, int nodeRight, int l, int r, int val) {
    if(nodeRight<l || r<nodeLeft) return 0;
    if(l<=nodeLeft && nodeRight<=r) {
        return nodeRight-nodeLeft+1 - (upper_bound(all(s[node]), val) - s[node].begin());
    }
    int mid = midpoint(nodeLeft, nodeRight);
    return query(node*2, nodeLeft, mid, l, r, val) + query(node*2+1, mid+1, nodeRight, l, r, val);
}
void Solve() {
    cin >> N;
    rep(i,1,N) cin >> a[i];
