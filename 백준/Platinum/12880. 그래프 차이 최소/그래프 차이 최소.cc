...
#define MAX 55 //when used in 2-sat, you have to double node because not only x1~x100 is required but also Nx1~Nx100
int matrix[MAX][MAX] {};
int d[MAX];
bool finished[MAX];
vpii edge[MAX];
...
bool inRange(int distance, int low, int high) {
...
int dfs(int x, int low, int high) {
    ...
    for (auto next : edge[x]) {
		...
		if(!inRange(nextDistance, low, high)) continue;
        if (d[nextNode] == 0) { //방문 안 한 이웃
            parent = min(parent, dfs(nextNode, low, high));
        }
...
void Solve() {
	...
	rep(i,1,N) {
		rep(j,1,N) {
			cin >> matrix[i][j];
			if(i!=j) edge[i].pbk({j, matrix[i][j]});
			distance.pbk(matrix[i][j]);
		}
	}
	sort(all(distance));
	...
	int dif = INF;
	int left =0;
	for(int right=0;right<distance.size();right++) {
		for(;left<=right;left++) {
			Initialize(); //Initialize for tarjan algorithm
			int low = distance[left];
			int high = distance[right];
			rep(i,1,N) {
				if(d[i]==0) dfs(i, low, high);
			}
			if(SN==1) {
				dif = min(dif, distance[right]-distance[left]);
			}
			else {
				break;
			}
