...
void Dijkstra() {
    p_q<pair<int, pii>, vector<pair<int, pii>>, cmp> pq; //cost, position, oil price
    pq.push({ 0, {1, gas[1]} });
    while (!pq.empty()) {
        auto [cost, p] = pq.top();
        auto [cur, oilPrice] = p;
        pq.pop();
        if(dp[cur][oilPrice]!=-1) continue;
        dp[cur][oilPrice] = cost;
        if(cur==N) {
            cout << cost;
            return;
        }
        for (auto next : edge[cur]) {
            int nextOilPrice = min(oilPrice, gas[next.first]);
            if(dp[next.first][nextOilPrice]==-1) {
                pq.push({cost + next.second * oilPrice, {next.first, nextOilPrice}});
            }
        }
    }
}
