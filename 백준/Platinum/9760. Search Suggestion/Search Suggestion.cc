...
auto comp = [] (int l, int r) {
    ...
    auto str2 = city[r];
    auto p2 = popular[r];
    if(p1==p2) return str1<str2;
    return p1>p2;
};
void segment(int node, int nodeLeft, int nodeRight) {
    if(nodeLeft==nodeRight) {
        s[node].pbk(nodeLeft);
        return;
    }
    int mid = midpoint(nodeLeft, nodeRight);
    segment(node*2, nodeLeft, mid);
    segment(node*2+1, mid+1, nodeRight);
    vi tv(s[node*2].size() + s[node*2+1].size());
    merge(all(s[node*2]), all(s[node*2+1]), tv.begin(), comp);
    int n = min(3, int32_t(tv.size()));
    s[node].resize(n);
    for(int i=0;i<n;i++) s[node][i] = tv[i];
}

vi query(int node, int nodeLeft, int nodeRight, int l, int r) {
    if(nodeRight<l || r<nodeLeft) return {};
    if(l<=nodeLeft && nodeRight<=r) {
        assert(s[node].size()<=3);
        return s[node];
    }
    int mid = midpoint(nodeLeft, nodeRight);
    auto lv = query(node*2, nodeLeft, mid, l, r);
    auto rv = query(node*2+1, mid+1, nodeRight, l, r);
    vi tv(lv.size() + rv.size());
    merge(all(lv), all(rv), tv.begin(), comp);
...
void Solve() {
    ...
    while(Q--) {
        ...
        string str2 = str;
        str2.back() += 1;
        int l = lower_bound(city+1, city+1+N, str1) - city;
        int r = lower_bound(city+1, city+1+N, str2) - city;
        if(l==r) cout << "N/A" << endl;
        else if(r-l>1000) cout << r-l << endl;
        else {
            r--;
