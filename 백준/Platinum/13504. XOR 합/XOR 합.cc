...
struct Trie {
    ...
    void insert(int num, int k) {
        ...
        next[curKey]->insert(num, k-1); // 다음 문자 삽입
    }
    ...
    int dfs(int num, int k) {
        ...
        if(next[1-curKey]) return (1<<k) | next[1-curKey]->dfs(num, k-1);
        else return next[curKey]->dfs(num,k-1);
    }
};
...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        root->insert(prefix[i], 30);
    }
    int answer = *max_element(prefix+1,prefix+1+N);
    rep(i,1,N) {
        chmax(answer, root->dfs(prefix[i], 30));
    }
    cout << answer << endl;
    delete root;
}
