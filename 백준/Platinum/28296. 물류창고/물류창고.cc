...
ll answer[MAX] {};
void Kruskal() { //MST
    cin >> N >> K >> M; //N is node, M is edge, Node starts from 1 to N
    rep(i,1,N) cin >> company[i];
    ...
    for (int i = 0; i < edge.size() ; i++) {
        ...
        if (!isSameParent(nodeA, nodeB)) {
            if(mp[getParent(nodeA)].size() < mp[getParent(nodeB)].size()) {
                for(auto p : mp[getParent(nodeA)]) {
                    answer[p.first] += dis * p.second * mp[getParent(nodeB)][p.first];
                }
            }
            else {
                for(auto p : mp[getParent(nodeB)]) {
                    answer[p.first] += dis * p.second * mp[getParent(nodeA)][p.first];
                }
            }
            unionParent(nodeA, nodeB);
        }
    }
    rep(i,1,K) cout << answer[i] << endl;
}
