...
bool dq1HasA() {
    if(dq1.size() < a.size()) return false;

    for(int i=0, j=0;i<a.size();i++, j++) {
        if(a[i]!=dq1[dq1.size()-a.size()+j]) return false;
    }
    return true;
}
void dq1Erase() {
...
bool dq2HasA() {
    if(dq2.size() < a.size()) return false;
    for(int i=0;i<a.size();i++) {
        if(a[i]!=dq2[i]) return false;
    }
    return true;
}
