...
int LineInterSection(Line l1, Line l2) { 
	...
	if(l1_l2==0 && l2_l1==0) { //l1 and l2 is on the same line. If p1 <= p4 && p3 <= p2, the line meets.
        if(l1.p1.x==l1.p2.x && l1.p1.x==l2.p1.x && l1.p1.x==l2.p2.x) {
            if(l1.p1.y > l1.p2.y) swap(l1.p1, l1.p2);
		    if(l2.p1.y > l2.p2.y) swap(l2.p1, l2.p2);
            if(l1.p1.y < l2.p2.y && l2.p1.y < l1.p2.y) return 4;
            else if(l1.p1.y==l2.p2.y || l1.p2.y==l2.p1.y) return 3;
            else return 0;
        }
        if(l1.p1.x > l1.p2.x) swap(l1.p1, l1.p2);
		if(l2.p1.x > l2.p2.x) swap(l2.p1, l2.p2);
        if(l1.p1.y==l1.p2.y && l1.p1.y==l2.p1.y && l1.p1.y==l2.p2.y) {
            if(l1.p1.x < l2.p2.x && l2.p1.x < l1.p2.x) return 4;
            else if(l1.p1.x==l2.p2.x || l1.p2.x==l2.p1.x) return 3;
            else return 0;
        }

		if(!(l1.p1.x <= l2.p2.x && l2.p1.x <= l1.p2.x)) return 0;
        if(ccw1 || ccw2 || ccw3 || ccw4) return 3;
        else return 4;

	}
    if(l2_l1==0 && l1_l2<0) return 3;
    return (l1_l2 <= 0) && (l2_l1 <= 0);
}

void Solve() {
    int xmin, ymin, xmax, ymax; cin >> xmin >> ymin >> xmax >> ymax;
    Line s1 = Line(Point(xmin,ymin), Point(xmin,ymax));
    Line s2 = Line(Point(xmin,ymax), Point(xmax,ymax));
    Line s3 = Line(Point(xmax,ymin), Point(xmax,ymax));
    Line s4 = Line(Point(xmin,ymin), Point(xmax,ymin));

    cin >> A >> B >> C >> D;
    Line line = Line(Point(A,B), Point(C,D));

    vi val;
    val.pbk(LineInterSection(s1, line));
    val.pbk(LineInterSection(s2, line));
    val.pbk(LineInterSection(s3, line));
    val.pbk(LineInterSection(s4, line));
    int inf = *max_element(all(val));
    if(inf==4) {
        cout << 4 << endl;
        return;
    }
