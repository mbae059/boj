...
bool bfs(int l, int r) {
    ...
    if(!(l<=height[src.first][src.second] && height[src.first][src.second]<=r)) return false;
    ...
    while(!q.empty()) {
        ...
        rep(i,0,7) {
            ...
            if(l<=height[nexty][nextx] && height[nexty][nextx]<=r) {
...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,N) {
            cin >> matrix[i][j];
            if(matrix[i][j]=='P') src = {i,j};
            if(matrix[i][j]=='K') dest.pbk({i,j});
        }
    }
    rep(i,1,N) {
        rep(j,1,N) {
