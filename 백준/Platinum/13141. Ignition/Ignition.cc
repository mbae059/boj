...
void Solve() {
    ...
    rep(i,1,M) {
        int a, b, c; cin >> a >> b >> c;
        maxTable[a][b] = max(maxTable[a][b], c);
        maxTable[b][a] = max(maxTable[b][a], c);
        dp[a][b] = min(dp[a][b], c);
        dp[b][a] = min(dp[b][a], c);
    }
    floyd_warshall();
    ...
    rep(i,1,N) {
        ...
        rep(j,1,N) {
            rep(k,1,N) {
                int dif = abs(dp[i][j] - dp[i][k]);
                lanswer = max(lanswer, dif + (maxTable[j][k] - dif)/(double)2 + min(dp[i][j], dp[i][k]));
            }
        }
        answer = min(answer, lanswer);
    }
