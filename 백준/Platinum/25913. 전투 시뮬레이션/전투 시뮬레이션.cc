...
int query(int node, int nodeLeft, int nodeRight, int l, int r, int a, int b) {
    if(nodeRight<l || r<nodeLeft) return 1e18;
    if(l<=nodeLeft && nodeRight<=r) {
        int val = (a+b)/2;
        auto idx = lower_bound(all(tree[node]), val) - tree[node].begin();
        if(idx==0) return abs(2*tree[node][idx]-a-b);
        else if(idx==tree[node].size()) return abs(2*tree[node][idx-1]-a-b);
        else return min(abs(2*tree[node][idx]-a-b), abs(2*tree[node][idx-1]-a-b));
    }
    int mid = midpoint(nodeLeft, nodeRight);
    return min(query(node*2, nodeLeft, mid, l, r, a, b), query(node*2+1, mid+1, nodeRight, l, r, a, b));
}
void Solve() {
    cin >> N;
    rep(i,1,N) {
    ...
    while(Q--) {
        ...
        int l = a + three - 1;
        int r = a + 2*three - 1;
        cout << query(1,1,N,l,r,prefix[a-1], prefix[b]) << endl;
    }
}
int32_t main() {
