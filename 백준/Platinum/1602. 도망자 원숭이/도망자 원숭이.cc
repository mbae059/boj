...
void floyd_warshall() {
    for(int k=1;k<=N;k++) {
        int x = delay[k].second;
        rep(i, 1, N) {
            rep(j, 1, N) {
                int w = max(delay[k].first, wait[i][j]);
                dp[i][j] = min(dp[i][j], dp[i][x]-wait[i][x] + dp[x][j] - wait[x][j] + w);
            }
        }
    }
}
...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,N) {
            ...
            wait[i][j] = max(delay[i].first, delay[j].first);
        }
    }
    rep(i,1,M) {
        int a, b, c; cin >> a >> b >> c;
        dp[a][b] = dp[b][a] = c + max(delay[a].first, delay[b].first);
    }
    sort(delay+1, delay+1+N);
