...
void doLeft(vvi& temp) {
    rep(i,1,N) {
        ...
        rep(j,1,N) {
            ...
            if(value!=temp[i][j] && value!=-1) {
                temp[i][idx] = value;
                idx++;
                value = temp[i][j];
            }
            else if(value!=temp[i][j] && value==-1) {
                value = temp[i][j];
            }
            else if(value==temp[i][j]) {
                temp[i][idx] = value*2;
                idx++;
                value = -1;
            }
        ...
        for(int j=idx;j<=N;j++) temp[i][j] = 0;
    }
}
void doRight(vvi& temp) {
    rep(i,1,N) {
        ...
        for(int j=1;j<=idx;j++) temp[i][j] = 0;
    }
}
void doUp(vvi& temp) {
    rep(j,1,N) {
        ...
        for(int i=idx;i<=N;i++) temp[i][j] = 0;
    }
}
void doDown(vvi& temp) {
    rep(j,1,N) {
        ...
        for(int i=1;i<=idx;i++) temp[i][j] = 0;
    }
}
int answer=0;
...
void dfs(int cur, vvi& src) {
    ...
    rep(i,1,N) {
        rep(j,1,N) {
            answer = max(answer, src[i][j]);
            mx = max(mx, src[i][j]);
        }
    }
    if(cur==LIMIT) return;
    if(mx * power2(LIMIT-cur) <= answer) return;
    vvi temp(N+1, vi(N+1));
