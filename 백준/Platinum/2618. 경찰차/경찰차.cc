...
int dfs(int p1, int p2) {
	...
	if(next==M+1) return 0;
	int val1, val2;
	if(p1==0) val1 = dfs(next, p2) + getDist({1,1}, query[next]);
	else val1 = dfs(next, p2) + getDist(query[p1], query[next]);

	if(p2==0) val2 = dfs(p1, next) + getDist({N,N}, query[next]);
	else val2 = dfs(p1, next) + getDist(query[p2], query[next]);
