...
#define MAX 22222 //when used in 2-sat, you have to double node because not only x1~x100 is required but also Nx1~Nx100
vvi SCC;
int d[MAX];
bool finished[MAX];
vi edge[MAX];
...
int meet(int i, int j) {
	if(input[i].first == input[j].first) { //x is same
		if(abs(input[i].second-input[j].second)<=2*R) {
			return 1;
		}
	}
	else if(input[i].second == input[j].second) {
		if(abs(input[i].first-input[j].first)<=2*R) {
			return 2;
		}
	}
	return 0;
...
void sat2() { // (x1 or x2) and (Nx1 or x3) //Nx1->x2, Nx2->x1. x1->x3, Nx3->Nx1
    cin >> N >> R >> M; // N is # of node, M is # of conditions
    rep(i,1,M) {
