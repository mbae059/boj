...
int B[MAX] {}; //saves the sz of palindrome each side
...
void manacher(string str) {
    ...
    for (int i = 0; i < sz; i++) {
        if (i <= rA) A[i] = min(A[2 * pA - i], rA - i);
        else A[i] = 0;

        if (i <= rB) B[i] = min(B[2 * pB - i], rB - i);
        else B[i] = 0;

        while (i-A[i]-1>=0 && i+A[i]+1<sz && mirror[str[i-A[i]-1]]==str[i+A[i]+1] && mirror[str[i]]==str[i]) {
            A[i]++;
        }
        
        while (i-B[i]-1>=0 && i+B[i]+1<sz && point[str[i-B[i]-1]]==str[i+B[i]+1] && point[str[i]]==str[i]) {
            B[i]++;
        }
        if (rA < i + A[i]) {
            rA = i + A[i];
    ...
    int answer = max(*max_element(A, A+sz), *max_element(B,B+sz));
    if(answer==0) cout << "NOANSWER";
    else cout << answer;
}
void Solve() {
    string str; cin >> str;
