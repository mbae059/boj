...
void Solve() {
    cin >> N;
    vector<vector<pair<int, char>>> edge(N+1);
    rep(i,1,N-1) {
        int a,b; cin >> a >> b;
        char c; cin >> c;
        edge[a].pbk({b,c});
    ...
    vi kmp(N-1);
    int answer=0;
    auto dfs = [&](auto&& dfs, int cur, string& str, int j) -> void {
        if(cur!=1) {
            while(j>0 && str.back()!=pattern[j]) j = pi[j-1];
            if(str.back()==pattern[j]) {
                j++;
                if(j==pattern.size()) {
                    answer++;
                    j = pi[j-1];
