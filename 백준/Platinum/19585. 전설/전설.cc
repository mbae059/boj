...
struct Trie {
    ...
    void insert(const string& str, int k=0) {
        ...
        if(next.find(curKey)==next.end()) next[curKey] = new Trie();

        next[curKey]->insert(str, k+1); // 다음 문자 삽입
    }

    void findColor(const string& str, int k=0) {
        if (finish) v1.pbk(k); // 문자열이 끝나는 위치를 반환
		if(str.size()==k) return;
        if(next.find(str[k])==next.end()) return;
        next[str[k]]->findColor(str, k+1); // 다음 문자를 탐색
    }
    void findName(const string& str, int k=0) {
        if (finish) v2.pbk(k); // 문자열이 끝나는 위치를 반환
		if(str.size()==k) return;
        if(next.find(str[k])==next.end()) return;
        return next[str[k]]->findName(str, k+1); // 다음 문자를 탐색
    }
};
void Solve() {
    cin >> N >> M;
    Trie *rootColor = new Trie;
    Trie *rootParticipant = new Trie;

    rep(i,1,N) {
        string str; cin >> str;
        rootColor->insert(str);
    }
    rep(i,1,M) {
        string str; cin >> str;
		reverse(all(str));
        rootParticipant->insert(str);
    }
    ...
    while(Q--) {
		...
		v1.clear();
		v2.clear();
        rootColor->findColor(str);
        reverse(all(str));
        rootParticipant->findName(str);
        ...
        [&] () {
			for(int i=0;i<v1.size();i++) {
				auto iter = lower_bound(all(v2), str.size()-v1[i]);
                if(iter==v2.end()) continue;
                
				if(*iter==str.size()-v1[i]) {
					cout << "Yes" << endl;
					return;
				}
			}
