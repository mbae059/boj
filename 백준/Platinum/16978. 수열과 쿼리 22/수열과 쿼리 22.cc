...
void Solve() {
    ...
    segment(1,1,N);
    cin >> Q;
    vpll change; //change A_i value to b
    change.pbk({0,0});
    vector<tllll> q;
    rep(i,1,Q) {
        ll op; cin >> op;
    ...
    sort(all(q), [&](tllll x, tllll y) {
        return get<0>(x) < get<0>(y);
    });
    vpll answer;
    for(ll i=0, l=1;i<q.size();i++) {
        auto [a,b,c,d] = q.at(i);
        while(a>=l) {
            update(1, change[l].first, 1, N, change[l].second);
            l++;
        }
        answer.pbk({d, query(1, b,c,1,N)});
    }
    sort(all(answer));
    for(auto [a,b] : answer) cout << b << endl;
}
int main() {
