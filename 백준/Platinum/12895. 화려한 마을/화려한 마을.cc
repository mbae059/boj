...
void update(int node, int l, int r, int nodeLeft, int nodeRight, ll dif) { //This is for lazy propagation
...
void Solve() {
    ...
    while(Q--) {
        ...
        if(c=='C') {
            ...
            cin >> x >> y;
            if(x>y) swap(x,y);
            uint64_t answer = query(1,x,y,1,N);
            cout << popcount(answer) << endl;
        }
    }
}
int main() {
