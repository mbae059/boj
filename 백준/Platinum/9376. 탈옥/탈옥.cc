    ...
    while(!dq.empty()) {
        ...
        rep(i,0,3) {
            ...
            if(matrix[nexty][nextx]=='#') {
                dq.pbk({nexty, nextx, d+1});
            }
            else {
                dq.push_front({nexty, nextx, d});
            }
        }
    }
}
...
void Solve() {
    ...
    rep(i,0,N+1) {
        rep(j,0,M+1) matrix[i][j] = '.';
    }
    rep(i,1,N) {
        rep(j,1,M) {
            cin >> matrix[i][j];
            if(matrix[i][j]=='$') target.pbk({i,j});
        }
    }
    bfs(make_pair(0,0));
    bfs(target[0]);
