...
struct Trie {
    ...
    void insert(int num, int digit=DIGIT) {
        if (digit==-1) {
            finish = true;
            return;
        }
        bool curKey = num & (1<<digit);
        if (next[curKey]==nullptr) next[curKey] = new Trie();
        next[curKey]->subTrie += 1;
        next[curKey]->insert(num, digit-1); // 다음 문자 삽입
    }

    int find(int num, int digit=DIGIT) {
        if (digit==-1) return 0; // 문자열이 끝나는 위치를 반환
        int curKey = num & (1<<digit);
        ...
        int ret = 1<<digit;
        if (next[curKey]) return ret + next[curKey]->find(num, digit-1); // 찾는 값이 존재하지 않음
        else {
            curKey = (curKey+1) % 2;
            return next[curKey]->find(num, digit-1);
        }
    }

    void erase(int num, int digit=DIGIT) {
        bool curKey = num & (1<<digit);
        next[curKey]->subTrie-=1;
        if(digit==0 || next[curKey]->subTrie==0) {
            delete next[curKey];
            next[curKey] = nullptr;
            return;
        }
        
        next[curKey]->erase(num, digit-1);
    }

    void traverse(int digit=DIGIT) {
        if(digit<0) return;

        for(int i=0;i<TRIENODE;i++) {
            if(next[i]) {
                cout << i;
                next[i]->traverse(digit-1);
            }
        }
    }
};
