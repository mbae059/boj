...
struct Planet {
    int x, y, z;
    int idx;
    Planet(int x, int y, int z, int idx) : x(x), y(y), z(z), idx(idx) {}
...
p_q<tuple<ll, int,int>, vector<tuple<ll, int, int>>, cmp> pq;
void print() {
...
void Solve() {
    ...
    rep(i,0,N-2) {
        ...
        pq.push({abs(p1.x-p2.x), p1.idx, p2.idx});
    }
    ...
    rep(i,0,N-2) {
        ...
        pq.push({abs(p1.y-p2.y), p1.idx, p2.idx});
    }
    ...
    rep(i,0,N-2) {
        ...
        pq.push({abs(p1.z-p2.z), p1.idx, p2.idx});
    }
