...
void Solve() {
    ...
    d[K]= 0;
    int step = 0;
    for(int i=K;i+step<=500000;i+=step) {
        step++;
        d[i+step] = d[i] + 1;
    }
    int answer = 500000;
    ...
    while(!q.empty()) {
        ...
        if(answer<=step) break;
        q.pop();
        if(d[cur]!=-1 && d[cur]%2==step%2 && d[cur]>=step) {
            answer = min(answer, d[cur]);
        }

        int stepOdd = (step+1)%2;
        if(cur-1>=0 && visited[cur-1][stepOdd]>step+1) {
            q.push({cur-1, step+1});
            visited[cur-1][stepOdd]=step+1;
        }
        if(cur+1<=500000 && visited[cur+1][stepOdd]>step+1) {
            q.push({cur+1, step+1});
            visited[cur+1][stepOdd]=step+1;
        }
        if(cur*2<=500000 && visited[cur*2][stepOdd]>step+1) {
            q.push({cur*2, step+1});
            visited[cur*2][stepOdd]=step+1;
        }
    }
    cout << (answer==500000 ? -1 : answer);
}
int main() {
