...
int query(int node, int nodeLeft, int nodeRight, int l, int r, int c) {
    if(nodeRight<l || r<nodeLeft) return 0;
    if(l<=nodeLeft && nodeRight<=r) {
        return upper_bound(all(tree[node]), c) - tree[node].begin();
    }
...
void Solve() {
    ...
    mint::set_mod(1e9+7);
    rep(i,1,N) {
        mint p = query(1,1,N,in[i],out[i],a[i]+w[i]);
        mint q = out[i] - in[i] + 1;
