...
//KMP
#define MAX 360000
vi makeTable(const vector<bool>& pattern) {
...
void KMP(const vector<bool>& parent, const vector<bool>& pattern) {
...
void Solve() {
    cin >> N;
    vector<bool> parent(2*MAX);
    vector<bool> pattern(MAX);
    rep(i,1,N) {
        int x; cin >> x;
        parent[x] = parent[x+MAX] = 1;
    }
    rep(i,1,N) {
        int x; cin >> x;
        pattern[x] = 1;
