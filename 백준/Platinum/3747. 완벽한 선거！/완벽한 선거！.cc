...
#define MAX 2001 //when used in 2-sat, you have to double node because not only x1~x100 is required but also Nx1~Nx100
vvi SCC;
int d[MAX];
bool finished[MAX];
vi edge[MAX];
...
void sat2() { // (x1 or x2) and (Nx1 or x3) //Nx1->x2, Nx2->x1. x1->x3, Nx3->Nx1
    ...
    rep(i,1,2*N) edge[i].clear();
    id = SN = 0;
    for (int i = 0; i < M; i++) {
        char ca, cb; int A, B; //If num is negative #, then it is (not) positive num
