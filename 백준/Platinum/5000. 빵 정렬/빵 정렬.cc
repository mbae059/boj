...
void Solve() {
    ...
    int dest[N+1] {};
    rep(i,1,N) cin >> src[i];
    rep(i,1,N) cin >> dest[i];

    Segtree<int> segtree1(N);
    Segtree<int> segtree2(N);
    int val1 = 0;
    int val2 = 0;
    rep(i,1,N) {
        val1 += segtree1.query(1,src[i],N,1,N);
        segtree1.update(1,1,N,src[i],1);
    }
    rep(i,1,N) {
        val2 += segtree2.query(1,dest[i],N,1,N);
        segtree2.update(1,1,N,dest[i],1);
    }

    if(abs(val1-val2)%2) cout << "Impossible";
    else cout << "Possible";
}
int32_t main() {
