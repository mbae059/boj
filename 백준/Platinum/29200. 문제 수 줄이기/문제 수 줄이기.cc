...
void Solve() {
    ...
    dp[2][2] = prefix[2];
    dp[3][1] = prefix[2] + a[3];
    dp[3][2] = a[1] + (prefix[1] ^ prefix[3]);
    dp[3][3] = prefix[3];
    dp[4][1] = max(dp[3][3] + a[4], dp[3][2] + a[4]);
    dp[4][3] = a[1] + (prefix[4] ^ prefix[1]);
    dp[4][4] = prefix[4];
    rep(i,5,N) {
        if(dp[i-1][2]!=-1) dp[i][1] = max(dp[i][1], dp[i-1][2] + a[i]);
        if(dp[i-1][3]!=-1) dp[i][1] = max(dp[i][1], dp[i-1][3] + a[i]);
        if(dp[i-1][4]!=-1) dp[i][1] = max(dp[i][1], dp[i-1][4] + a[i]);

        if(dp[i-2][1]!=-1) dp[i][2] = max(dp[i][2], dp[i-2][1] + (prefix[i] ^ prefix[i-2]));
        if(dp[i-2][3]!=-1) dp[i][2] = max(dp[i][2], dp[i-2][3] + (prefix[i] ^ prefix[i-2]));
        if(dp[i-2][4]!=-1) dp[i][2] = max(dp[i][2], dp[i-2][4] + (prefix[i] ^ prefix[i-2]));
        
        if(dp[i-3][1]!=-1) dp[i][3] = max(dp[i][3], dp[i-3][1] + (prefix[i] ^ prefix[i-3]));
        if(dp[i-3][2]!=-1) dp[i][3] = max(dp[i][3], dp[i-3][2] + (prefix[i] ^ prefix[i-3]));
        if(dp[i-3][4]!=-1) dp[i][3] = max(dp[i][3], dp[i-3][4] + (prefix[i] ^ prefix[i-3]));

        if(dp[i-4][1]!=-1) dp[i][4] = max(dp[i][4], dp[i-4][1] + (prefix[i] ^ prefix[i-4]));
        if(dp[i-4][2]!=-1) dp[i][4] = max(dp[i][4], dp[i-4][2] + (prefix[i] ^ prefix[i-4]));
        if(dp[i-4][3]!=-1) dp[i][4] = max(dp[i][4], dp[i-4][3] + (prefix[i] ^ prefix[i-4]));
    }
