...
pii query(int node, int nodeLeft, int nodeRight, int l, int r, int c) {
    if(nodeRight<l || r<nodeLeft) return {0,0};
    if(l<=nodeLeft && nodeRight<=r) {
        int p1 = lower_bound(all(s[node]), c) - s[node].begin();
        int p2 = upper_bound(all(s[node]), c) - s[node].begin();
        return {p1,p2};
    }

    int mid = midpoint(nodeLeft, nodeRight);
    auto lp = query(node*2, nodeLeft, mid, l, r, c);
    auto rp = query(node*2+1, mid+1, nodeRight, l, r, c);
