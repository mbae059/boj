...
void set_tree(int node, int pnode) {
    ...
    for(int i=1;i<=maxLevel;i++) {
        ...
        mn[node][i] = min(mn[prev][i-1], mn[node][i-1]);
        mx[node][i] = max(mx[prev][i-1], mx[node][i-1]);
    }

    for(auto child : edge[node]) {
        if(child.first==pnode) continue;
        mn[child.first][0] = child.second;
        mx[child.first][0] = child.second;
        set_tree(child.first, node);
    }
}

pii setAnswer(pii a, pii b) {
    ...
    if(level[target]!=level[compare]) { 
        for(int i=maxLevel;i>=0;i--) {
            if(level[parent[target][i]] >= level[compare]) {
                answer = setAnswer(answer, {mn[target][i], mx[target][i]});
                target = parent[target][i];
            }
        }
    }
    if(target==compare) return answer;

    
    for(int i=maxLevel;i>=0;i--) {
        if(parent[target][i]!=parent[compare][i]) {
            answer = setAnswer(answer, {mn[target][i], mx[target][i]});
            answer = setAnswer(answer, {mn[compare][i], mx[compare][i]});

            target = parent[target][i];
            compare = parent[compare][i];
        }
    }
    answer = setAnswer(answer, {mn[target][0], mx[target][0]});
    answer = setAnswer(answer, {mn[compare][0], mx[compare][0]});
