...
void Solve() {
    ...
    for(int i=1;i<N;i++) {
        for(int j=1;j<N;j++) {
            for(int x=1;x<=i;x++) {
                for(int y=1;y<=j;y++) {
                    int area = dp[i][j] - dp[i][y-1] - dp[x-1][j] + dp[x-1][y-1];
                    mp[i][j][area]++;
                }
            }
        }
    ...
    for(int i=1;i<N;i++) {
        for(int j=1;j<N;j++) {
            for(int x=i+1;x<=N;x++) {
                for(int y=j+1;y<=N;y++) {
                    int area = dp[x][y] - dp[i][y] - dp[x][j] + dp[i][j];
                    answer += mp[i][j][area];
                }
            }
            mp[i][j].clear();
    ...
    for(int i=1;i<=N-1;i++) {
        for(int j=N;j>=2;j--) {
            for(int x=1;x<=i;x++) {
                for(int y=j;y<=N;y++) {
                    int area = dp[i][y] - dp[i][j-1] - dp[x-1][y] + dp[x-1][j-1];
                    mp[i][j][area]++;
                }                
            }
        }
    ...
    for(int i=1;i<=N-1;i++) {
        for(int j=N;j>=2;j--) {
            for(int x=i+1;x<=N;x++) {
                for(int y=1;y<j;y++) {
                    int area = dp[x][j-1] - dp[x][y-1] - dp[i][j-1] + dp[i][y-1];
                    answer += mp[i][j][area];
                }
            }
        }
