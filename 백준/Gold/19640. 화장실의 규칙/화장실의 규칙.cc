...
void Solve() {
    ...
    auto cmp = [](tiiii l, tiiii r) {
        auto [d1,h1,i1, idx1] = l;
        auto [d2,h2,i2, idx2] = r;

        if(d1!=d2) return d1<d2;
        if(h1!=h2) return h1<h2;
        return i1>i2;
    };
    p_q<tiiii, vtiiii, decltype(cmp)> pq;
    queue<tiii> q[M+1];
    rep(i,1,N) {
        int idx = (i-1)%M + 1;
        cin >> D >> H;
    ...
    int answer=0;
    while(!pq.empty()) {
        auto [d,h,i, idx] = pq.top();
        pq.pop();
        if(idx==K+1) cout << answer << endl;
        answer++;

        if(!q[i].empty()) {
            auto [td,th,tidx] = q[i].front();
            q[i].pop();
            pq.push({td,th,i, tidx});
        }
    }
}
int32_t main() {
