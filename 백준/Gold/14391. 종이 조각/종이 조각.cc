...
*/
int graph[4][4];
bool chk[4][4]; // 1 is row, 0 is column
int answer=0;
void Examine() {
    int totalSum=0;
    for(int i =0;i<N;i++) { //computes row
        int sum = 0;
        for(int j=0;j<M;j++) {
            if(chk[i][j]) sum = sum*10 + graph[i][j];
            else {
                totalSum += sum;
                sum =0;
            }
    ...
    for(int i =0;i<M;i++) { //computes column
        int sum = 0;
        for(int j=0;j<N;j++) {
            if(!chk[j][i]) sum = sum*10 + graph[j][i];
            else {
                totalSum += sum;
                sum=0;
            }
