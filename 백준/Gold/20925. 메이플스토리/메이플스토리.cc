...
void Solve() {
    ...
    rep(i,1,T) {
        rep(j,1,N) {
            rep(k,1,N) {
                //j입장에서 k->j로 가는 법
                //i초에서 dist[k][j]초 걸려 k에서 j까지 감
                if(i-dist[k][j]>=0 && dp[k][i-dist[k][j]]>=v[j].first) {
                    chmax(dp[j][i], dp[k][i-dist[k][j]]);
                }
            }
            if(dp[j][i-1]>=v[j].first) chmax(dp[j][i], dp[j][i-1] + v[j].second);
        }
    }
    int answer=0;
