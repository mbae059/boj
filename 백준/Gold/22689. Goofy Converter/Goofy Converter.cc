...
void Solve() {
    while(true) {
        ...
        rep(i,N,N+M-1) {
            ...
            else if(answer[i]==-1) answer[i] = 0;
        }
        if(l) flag = 1;
        for(int i=N-1;i>=1;i--) {
            if(answer[i]==-1) answer[i] = answer[i+M];
        }
        ...
        if(flag) cout << "Goofy" << endl;
        else {
            for(int i=1;i<=N+M-1;i++) cout << answer[i];
            cout << endl;
        }
    }
}
