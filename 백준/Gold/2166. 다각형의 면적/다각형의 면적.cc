...
double CCW(Point A, Point B, Point C) { //A, B, C is in order
   return (B.x - A.x) * (C.y - A.y) - (C.x - A.x) * (B.y - A.y); //Cross product
...
void Solve() {
    ...
    double sum =0;
    rep(i, 1, N-2) {
        sum += CCW(P, v.at(i), v.at(i+1));
    }
    printf("%0.1lf",abs(sum)/2);
