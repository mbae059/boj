...
vector<Fireball> matrix[MAX][MAX];

int add(int i, int direction, int speed) {
    i += (direction * speed);
    if(i>0) i = (i%N==0 ? N : i%N);
    else i = i%N + N;
...
void Solve() {
    ...
    while(K--) {
        while(!q.empty()) {
            ...
            // cout << r << " " << c << " " << m << " " << s << " " << d << endl;
            q.pop();
            int nextR = add(r, dir[d][0], s);
            int nextC = add(c, dir[d][1], s);

            matrix[nextR][nextC].pbk(Fireball(nextR, nextC, m, s, d));
        }

        rep(i,1,N) {
            rep(j,1,N) {
                if(matrix[i][j].size()==1) {
                    Fireball fireball = matrix[i][j].front();
                ...
                //2 or more fireballs
                    ...
                    int totalM = 0;
                    int totalS = 0;
                    bool dirOdd=1, dirEven=1;

                    for(auto fireball : matrix[i][j]) {
                        totalM += fireball.m;
                        totalS += fireball.s;
                    ...
                    int dirStart = 0;
                    if(dirOdd || dirEven) dirStart = 0;
                    else dirStart = 1;

                    if(totalM/5!=0) {
                        for(int k=0;k<8;k+=2) {
                            q.push(Fireball(i, j, totalM/5, totalS/matrix[i][j].size(), dirStart + k));
                        }
                    }
                }
                matrix[i][j].clear();
