...
int getSize(int y, int x) {
    ...
    while(true) {
        rep(i,0,3) {
            int nexty = y+dir[i][0]*k;
            int nextx = x+dir[i][1]*k;
            if(!inRange(nexty, nextx)) return k-1;
            if(matrix[nexty][nextx]=='.') return k-1;
        }
        k++;
    }
}
void flip(int y, int x, int k, char c) {
    rep(i,0,k) {
        rep(j,0,3) {
            int nexty = y+dir[j][0]*i;
            int nextx = x+dir[j][1]*i;

            matrix[nexty][nextx] = c;
        }
    }
}
void Solve() {
            ...
            rep(k,0,step1) {
                ...
                rep(a,1,N) {
                    rep(b,1,M) {
                        ...
                        int step2 = getSize(a,b);
                        int area1 = 4*k+1;
                        int area2 = 4*step2 + 1;
                        mx = max(mx, area1 * area2);
                    }
                }
            }
            flip(i,j,step1,'#');
