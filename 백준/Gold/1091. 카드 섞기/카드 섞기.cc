...
bool isSame(const vi& cards, const vi& cmp) {
	for (int i = 0; i < cmp.size(); i++) {
		if (cards[i] % 3 != cmp[i]) return false;
	}
	return true;
}

bool repeat(const vi& cards, const vi& cmp) {
	for (int i = 0; i < N; i++) {
		if (cards[i] != cmp[i]) return false;
	}
	return true;
}
...
int main() {
	...
	FOR(i, N) cards.pbk(i);
	vi ori_cards;
	for (auto i : cards) ori_cards.pbk(i);
	ll cnt = 0;

	if (isSame(cards, dest)) {
		cout << 0;
		return 0;
	}
	do {
		...
		//cout << endl;
		cnt++;
	} while (!repeat(cards, ori_cards) && !isSame(cards, dest));
