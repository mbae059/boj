...
struct Snowman {
	...
	Snowman(int a, int b, int sum) : a(a), b(b), sum(sum) {}

	bool operator < (const Snowman& S) {
		return sum < S.sum;
	}
};

bool isDifferent(Snowman a, Snowman b) {
...
void Solve() {
	...
	rep(i,0,N-1) {
		rep(j,i+1, N-1) {
			Snowmans.pbk(Snowman(i,j,v[i]+v[j]));
		}
	}
	sort(all(Snowmans));
	...
	for(int i=0;i<Snowmans.size();i++) {
		auto elsa = Snowmans[i];
		for(int j=i+1;j<Snowmans.size();j++) {
			auto anna = Snowmans[j];
			if(isDifferent(elsa, anna)) {
				answer = min(answer, abs(elsa.sum-anna.sum));
				break;
			}
		}
	}
