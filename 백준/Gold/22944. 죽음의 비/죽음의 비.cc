...
int T, N, K, M, H, W, Q; // S is for MCMF, network flow
...
struct Node {
    int idx, health, dis, umbrella;

    Node(int idx, int health, int umbrella, int dis) : idx(idx), health(health), umbrella(umbrella), dis(dis) {}
};
struct cmp { //pii
...
void Dijkstra() {
    ...
    pq.push(Node(S, H, 0, 0));
    
    rep(i,0,12) {
        rep(j,0,10000) d[i][j] = INF;
    }
    d[S][H] = 0;
    int answer=INF;
    ...
    while (!pq.empty()) {
        int cur = pq.top().idx;
        int health = pq.top().health;
        int umbrella = pq.top().umbrella;
        int dis = pq.top().dis;
        pq.pop();
        ...
        if(d[cur][health] < dis) continue; //could be used if d[cur] has been updated more than once.
        for (auto next : edge[cur]) {
            int nextIdx = next.first;
            int nextDis = next.second;

            if(umbrella + health <= nextDis-1) continue;
            int nextHealth = health;
            if(umbrella) {
                nextDis -= (umbrella+1);
                if(nextDis>0) nextHealth -= nextDis-1;
            }
            else {
                nextHealth -= nextDis-1;
            }
            if(nextHealth<=0) continue;

            rep(i,0,nextHealth) d[nextIdx][i] = min(d[nextIdx][i], dis+next.second);
            pq.push({nextIdx, nextHealth, D-1, dis+next.second});
        }
    }
...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,N) {
            cin >> matrix[i][j];
            if(matrix[i][j]=='S') S = v.size();
            if(matrix[i][j]=='E') E = v.size();
            if(matrix[i][j]!='.') {
                v.pbk({i,j});
            }
        }
    ...
    for(int i=0;i<v.size();i++) {
        for(int j=i+1;j<v.size();j++) {
            auto p1 = v.at(i);
            auto p2 = v.at(j);
            auto dis = abs(p1.first-p2.first) + abs(p1.second-p2.second);
