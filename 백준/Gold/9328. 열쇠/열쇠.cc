...
bool hasKey(char c, int keys) { //determines if this is the door and whether he can pass it through or not
    if('A'<=c && c<='Z')
        return (1<<(c-'A')) & keys;
    return false;
}

bool hadKey(char c, int keys) {
    return (1<<(c-'a')) & keys;
}
void getKey(char c, int& keys, bool& flag) {
    if(!hadKey(c, keys)) {
        flag = 1;
        keys |= (1<<(c-'a'));
    }
...
void Solve() {
    ...
    while(flag) {
        ...
        rep(j,1,M) {
            char c1 = matrix[1][j];
            if(c1=='.' || hasKey(c1, keys) || isKey(c1) || c1=='$') {
                if(isKey(c1)) getKey(c1, keys, flag);
                if(c1=='$') {
                    answer++;
                    matrix[1][j] = '.';
                }
            ...
            char c2 = matrix[N][j];
            if(c2=='.' || hasKey(c2, keys) || isKey(c2) || c2=='$') {
                if(isKey(c2)) getKey(c2, keys, flag);
                if(c2=='$') {
                    answer++;
                    matrix[N][j] = '.';
                }
        ...
        while(!q.empty()) {
            ...
            rep(i,0,3) {
                ...
                char nextc = matrix[nexty][nextx];
                if(nextc=='*') continue;
                if('A'<=nextc && nextc <='Z') {
                    if(hasKey(nextc, keys)) {
                        q.push({nexty, nextx});
