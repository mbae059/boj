...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        if(!s.empty()) {
            dp[i][0]+=dp[s.top().first][0];
            dp[i][0]+=getDist(s.top().first-i, s.top().second-a[i]);
        }
        s.push({i, a[i]});
    }
    while(!s.empty()) s.pop();
    ...
    for(int i=N;i>=1;i--) {
        ...
        if(!s.empty()) {
            dp[i][1]+=dp[s.top().first][1];
            dp[i][1]+=getDist(s.top().first-i, s.top().second-a[i]);
        }
        s.push({i,a[i]});
    }
    cin >> Q;
