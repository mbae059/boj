...
void Solve() {
    ...
    while(!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            ...
            int nx= x+dx[i];
            if(!inRange(ny,nx)) continue;
            if(matrix[ny][nx]=='O') continue;
            int cnt = 0;
            for(int j=0;j<4;j++) {
                int nny = ny + dy[j];
                int nnx = nx + dx[j];
                if(!inRange(nny,nnx)) continue;
                if(matrix[nny][nnx]=='O') cnt++;
            }
            if(cnt>=2) q.push({ny,nx});
        }
    }
    vector<vpii> dp(N+1, vpii(M+1));
    int answer=0;
    rep(i,1,N) {
        rep(j,1,M) {
            if(matrix[i][j]!='O') continue;
            dp[i][j] = {1,1};
            if(matrix[i-1][j-1]=='O') dp[i][j] = max(dp[i][j], {dp[i-1][j-1].first+1, dp[i-1][j-1].second+1});
            if(matrix[i-1][j]=='O') dp[i][j] = max(dp[i][j], {dp[i-1][j].first+1, dp[i-1][j].second});
            if(matrix[i][j-1]=='O') dp[i][j] = max(dp[i][j], {dp[i][j-1].first, dp[i][j-1].second+1});
            if(matrix[i-1][j-1]=='O' and matrix[i+1][j+1]!='O' and matrix[i+1][j]!='O' and matrix[i][j+1]!='O') {
                if(!(dp[i][j].first<=K || dp[i][j].second<=K)) answer += dp[i][j].first * dp[i][j].second;
            }
        }
    }
    print(answer);
