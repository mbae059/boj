...
#define MAX 1000002
vll v(MAX);
vll Fenwick_Tree(MAX);

void Update(int idx, ll Value) { //For Making Fenwick Tree, for(int i=1~N) Update(i, Arr[i]);
	while (idx < Fenwick_Tree.size()) {
		Fenwick_Tree[idx] = Fenwick_Tree[idx] + Value;
		idx = idx + (idx & -idx);
	}
}
ll sum(int idx) { //IF 3~5 sum is required it should be sum(5)-sum(2);
	ll result = 0; //BE CAREFUL ON RANGE (Long Long could be used)
	while (idx > 0) {
...
vi makeTable(string pattern) {
	vi table(pattern.size());
...
void KMP(string str, string cmp) { // str is bigger than cmp
	vi table = makeTable(cmp);

	int j=0;

	for(int i=1;i<str.size();i++) {
		while(j>0 && str[i] != cmp[j]) j = table[j-1];
		if(str[i]==cmp[j]) {
			if(j==cmp.size()-1) {
				cout << i-cmp.size()+1 << endl;
				j = table[j];
			}
			else j++;
		}
