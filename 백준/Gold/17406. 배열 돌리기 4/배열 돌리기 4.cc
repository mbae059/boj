...
struct RotateOp {
    int r, c, s;
    RotateOp(int r, int c, int s) : r(r), c(c), s(s) {}
};
vector<RotateOp> v;
...
void Rturn(int r, int c, int s) {
    int k = 1;
    
    while(k<=s) {
        int tl = matrix[r-k][c-k];
        for(int j=c-k;j<c+k;j++) {
            matrix[r-k][j] = matrix[r-k][j+1];
        }
        for(int i=r-k;i<r+k;i++) {
...
void dfs(int idx, int cnt) { //implement with dfs(1, 0). N and K must be global variable
	...
	for(int i=0;i<K;i++) { //idx, N을 잘볼것
		if(visited[i]==1) continue;
		
		visited[i]=1;
		combination.push_back(i);
        turn(v[i].r, v[i].c, v[i].s);
		dfs(i+1, cnt+1); //be careful with i+1, cnt+1
		visited[i]=0;
        Rturn(v[i].r, v[i].c, v[i].s);
