...
void Solve() {
    while(cin>>N) {
        ...
        while(!q.empty()) {
            auto [cur, j] = q.front();
            q.pop();
            if(answer.second < pow(FEE, j) * value[cur]) {
                answer.first = cur;
                answer.second = pow(FEE, j) * value[cur];
            }
            else if(answer.second == pow(FEE, j) * value[cur]) {
                answer.first = min(answer.first, cur);
            }
            for(auto next : edge[cur]) {
        ...
        assert(answer.first!=INF);
        assert(answer.second!=-1);

        cout << "Import from " << char(answer.first+'A') << endl;
    }
}
int main() {
