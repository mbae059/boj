...
void bfs(int y, int x) {
    ...
    for(auto p : v) {
        ...
        int tempGoDown = 0;
        while(cury+1<=N && matrix[cury+1][curx]=='.') {
            tempGoDown++;
            cury++;
        ...
        if(tempGoDown==0) continue; 
        
        goDown = min(goDown, tempGoDown);
    }
    for(auto p : v) {
        int cury = p.first;
        int curx = p.second;
...
void Left(int H) {
    rep(i,1,M) {
        if(matrix[H][i]=='x') {
            matrix[H][i]='.';
            if(i+1 <= M && matrix[H][i+1]=='x') { //examine right
                bfs(H, i+1);
            }
            if(H-1>=1 && matrix[H-1][i]=='x') { //examine above
                bfs(H-1, i);
            }
            if(H+1<=N && matrix[H+1][i]=='x') {
                bfs(H+1, i);
            }
            return;
        }
...
void Right(int H) {
    for(int i=M;i>0;i--) {
        if(matrix[H][i]=='x') {     
            matrix[H][i]='.';
            if(i-1 >=1 && matrix[H][i-1]=='x') { //examine left
                bfs(H, i-1);
            }
            if(H-1>=1 && matrix[H-1][i]=='x') { //examine above
                bfs(H-1, i);
