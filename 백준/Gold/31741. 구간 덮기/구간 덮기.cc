...
void Solve() {
    ...
    for(int i=0;i<N;i++) {
        auto [s1,e1] = v[i];
        //v[i] must be the lower half of the segment
        if(s1>S) continue;
        if(r.empty()) break;
        auto iter = upper_bound(all(r), e1);
        if(iter!=r.begin()) iter--;
        if(*iter<=e1) chmin(answer, e1-*iter);
    }
    ...
    for(int i=1;i<=N;i++) {
        ...
        if(s1<=S || E<=e1) continue;
        auto iter1 = lower_bound(all(l), s1);
        auto iter2 = upper_bound(all(r), e1);
        if(iter1!=l.end() && iter2!=r.begin()) {
            iter2--;
            int e = *iter1;
            int s = *iter2;
            if(e<s) chmin(answer, e-s1+e1-s);
        }
    }
