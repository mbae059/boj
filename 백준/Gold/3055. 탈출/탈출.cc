...
void Solve() {
    ...
    while(!q.empty()) {
        ...
        rep(i,0,3) {
            ...
            if(matrix[nexty][nextx]=='D' || matrix[nexty][nextx]=='X') continue;
    ...
    queue<tiii> hq;
    hq.push({src.first, src.second, 0});
    visited[src.first][src.second] = 1;
    while(!hq.empty()) {
        auto [y,x,step] = hq.front();
        hq.pop();
        if(matrix[y][x]=='D') {
            cout << step;
            return;
        ...
        rep(i,0,3) {
            ...
            if(!inRange(nexty,nextx)) continue;
            if(visited[nexty][nextx]) continue;
            if(matrix[nexty][nextx]=='X') continue;
            if(d[nexty][nextx]<=step+1) continue;
            visited[nexty][nextx] = 1;
            hq.push({nexty,nextx,step+1});
        }
    }
    cout << "KAKTUS";
}
