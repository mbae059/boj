...
void Solve() {
    ...
    dp[1][1][0] =1;
    while(!q.empty()) {
        auto [y,x,use] = q.front();
        q.pop();
        for(int i=0;i<4;i++) {
            int ny = y + dy[i];
            int nx = x + dx[i];
            if(!inRange(ny,nx)) continue;
            if(visited[ny][nx][use]) continue;
            
            if(matrix[ny][nx]=='1' && use==0) {
                visited[ny][nx][1] = 1;
                q.push({ny,nx,1});
                dp[ny][nx][1] = dp[y][x][use]+1;
            }
            if(matrix[ny][nx]=='0') {
                visited[ny][nx][use] = 1;
                q.push({ny,nx,use});
                dp[ny][nx][use] = dp[y][x][use]+1;
            }
        }
    }
    if(visited[N][M][0] || visited[N][M][1]) cout << min(dp[N][M][0], dp[N][M][1]);
    else cout << -1;
