...
void bfs(pii start) {
    queue<pii> q;
    q.push(start);
    island[start.first][start.second] = ++id;
    ...
    while(!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            ...
            if(graph[nexty][nextx] && island[nexty][nextx]==0) {
                island[nexty][nextx] = id;
                q.push({nexty, nextx});
            }
        }
    }
...
void Solve() {
    ...
    for(int i=0;i<N;i++) {
        ...
        for(int j=0;j<M;j++) {
            if(graph[i][j]==1) {
                if(start && start != island[i][j]) {
                    if(sum > 1) {
                        edge.pbk(Edge(start, island[i][j], sum));
                        // cout << start << " " << island[i][j] << " " << sum << endl;
                    }
                    start = island[i][j];
                    sum=0;
                }
                else if(start && start == island[i][j]) sum=0;
                else start = island[i][j];
            }
            else if(start) sum++;
        }
    }
