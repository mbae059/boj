...
int Dijkstra() {
    ...
    while (!pq.empty()) {
        ...
        if(d[cur][mx] < dis) continue; //could be used if d[cur] has been updated more than once.
        ...
        for (auto next : edge[cur]) {
            ...
            int nextMx = max(mx, next.second);

            if(visited[nextNode][nextMx]) continue;
            if(nextDis > C) continue;
            if(d[nextNode][nextMx] > nextDis) {
                d[nextNode][nextMx] = nextDis;
                pq.push(Node(nextNode, nextDis, nextMx));
            }
        }
    }
