...
void print() {
    rep(i,0,N-1) {
        rep(j,0,N-1) {
            if(listPiece[i][j].size()) {
                for(auto k : listPiece[i][j]) {
                    cout << k+1 << " : ";
                }
            }
            else cout << -1 << " ";
...
void Solve() { 
    ...
    rep(i, 0, 999) {
        rep(k, 0, K-1) {
            ...
            if(!inRange(nextx, nexty) || board[nextx][nexty]==2) {
                changeDir(d);
                nextx = x + dir[d][0];
                nexty = y + dir[d][1];

                if(!inRange(nextx, nexty) || board[nextx][nexty]==2) { //stays
                    continue;
                }
            }

            auto iter = find(listPiece[x][y].begin(), listPiece[x][y].end(), k);
            if(board[nextx][nexty]==0) {
                listPiece[nextx][nexty].splice(listPiece[nextx][nexty].end(), listPiece[x][y], iter, listPiece[x][y].end());
                for(auto pieceIdx : listPiece[nextx][nexty]) {
            ...
            if(board[nextx][nexty]==1) {
                reverse(iter, listPiece[x][y].end());
                listPiece[nextx][nexty].splice(listPiece[nextx][nexty].end(), listPiece[x][y], iter, listPiece[x][y].end());
                for(auto pieceIdx : listPiece[nextx][nexty]) {
                    auto& piece = pieces[pieceIdx];
                    piece.x = nextx;
            ...
            if(listPiece[nextx][nexty].size()>=4) {
                cout << i+1;
                return;
            }
            // print();
