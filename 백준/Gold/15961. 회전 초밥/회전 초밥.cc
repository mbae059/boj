...
int sushi[MAX] {};
int kind[3333] {};
int getSushi(int num) {
    return num%N == 0 ? N : num%N;
}
...
void Solve() {
    ...
    rep(i,1,K) {
        kind[sushi[i]] += 1;
        if(kind[sushi[i]]==1) s.insert(sushi[i]);
    }
    answer = s.size();
    kind[C] += 1;
    s.insert(C);
    rep(i,2,N) {
        int r = getSushi(i+K-1);
        kind[sushi[r]] += 1;
        if(kind[sushi[r]]==1) s.insert(sushi[r]);

        int l = getSushi(i-1);
        kind[sushi[l]] -= 1;
        if(kind[sushi[l]]==0) s.erase(sushi[l]);
