...
#define PI 3.1415926
double getArea(int p1, int p2, pii p) {
    return 1000*p.second * sin(PI * (p.first-p1) / 1800.0) + 1000*p.second * sin(PI * (p2-p.first) / 1800.0) - 1000*1000*sin(PI * (p2-p1) / 1800.0);
}
...
void Solve() {
    ...
    int line=0;
    for(auto p : v) {
        double area1 = getArea(p.first, p.second, p1);
        double area2 = getArea(p.first, p.second, p2);
        if(area1*area2<0) line++;
    }
