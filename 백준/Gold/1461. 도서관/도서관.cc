...
int main() {
	...
	if (p.size() == 0 || n.size() == 0) larger = p.empty() ? n : p;
	else {
		shorter = p.back() > n.back() ? n : p;
		larger = p.back() < n.back() ? n : p;
	}
	int step = 0;

	for (int i = shorter.size() - 1; i >= 0; i -= M) {
		step += shorter[i]*2;
		//cout << shorter[i] << " ";
	}
	for (int i = larger.size() - 1; i >= 0; i -= M) {
		step += larger[i]*2;
		//cout << larger[i] << " ";
	}
	step -= larger[larger.size() - 1];
	cout << step;
}