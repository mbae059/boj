...
void Solve() {
    ...
    ll answer = LLONG_MAX;
    for(int i=0;i<A.first;i++) { //B를 A.first개만큼~
        M = N - B.first * i;
        ll tempMoney = i * B.second;

        if(M>0) {
            ll acnt = M / A.first;
            if(M % A.first==0) {
                answer = min(answer, tempMoney + acnt * A.second);
            }
            else {
                answer = min(answer, tempMoney + (acnt + 1) * A.second);
            }
        }
        else {
            answer = min(answer, tempMoney);
        }
    }
    cout << answer;
}
