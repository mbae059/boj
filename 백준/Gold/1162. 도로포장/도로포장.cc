...
// This is Dijkstra
#define MAX 10001
vpll edge[MAX]; // first is idx, second is weight of edge
ll dp[MAX][22] {};

struct cmp { //pii
    bool operator() (const tlll& i, const tlll& j) { //cur node, k used, dist
        return get<2>(i) > get<2>(j);
    }
};
void Dijkstra(int num) {
    p_q<tlll, vector<tlll>, cmp> pq;
    pq.push({ num, 0, 0 });
    rep(i,1,N) {
        rep(j,0,K) dp[i][j] = LLONG_MAX;
    }
    dp[num][0] = 0;
    while (!pq.empty()) {
        auto [cur, k, dist] = pq.top();
        pq.pop();
        if(dp[cur][k]<dist) continue;
        if(cur==N) {
            cout << dist;
            return;
        }
        for (auto next : edge[cur]) {
            if(dp[next.first][k] > dp[cur][k] + next.second) {
                dp[next.first][k] = dp[cur][k] + next.second;
                pq.push({next.first, k, dp[next.first][k]});
            }
            if(k<K && dp[next.first][k+1] > dp[cur][k]) {
                dp[next.first][k+1] = dp[cur][k];
                pq.push({next.first, k+1, dp[next.first][k+1]});
            }
        }
    }
}
