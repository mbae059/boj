...
bool inRange(pii p1, pii p2, pii p) {
    auto [l1,r1] = p1;
    auto [l2,r2] = p2;
...
void Solve() {
    ...
    vtiiii cv;
    for(auto [a,b] : v) { //100개
        for(int i=1;i<=K/2-1;i++) { //가로 //K개
            int width = i;
            int height= K/2-width;

            for(int j=0;j<=height;j++) {
                cv.pbk({a-j,b,a+height-j,b+width});
            }
            for(int j=0;j<=width;j++) {
                cv.pbk({a,b-j,a+height,b-j+width});
            }
        }
    }
