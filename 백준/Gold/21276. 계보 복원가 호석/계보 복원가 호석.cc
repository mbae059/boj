...
#define MAX 1111
map<string, int> mp;
map<int, string> mpReverse;

int cnt = 0;
int inDegree[MAX] {};
vi edge[MAX];
bool visited[MAX] {};
vector<string> baseAncestor;
...
void Solve() {
    ...
    rep(i,1,N) {
        if(inDegree[i] == 0) {
            baseAncestor.pbk(mpReverse[i]);
            v.pbk(i);
        }
    }
    for(auto i : v) dfs(i);
    sort(all(baseAncestor));
    
    cout << baseAncestor.size() << endl;
    print(baseAncestor);
