...
void Solve() {
    cin >> N;
    vector<tuple<string,int,int,int>> v(N+1);
    ...
    rep(i,1,N) {
        rep(j,i+1,N) {
            auto [str1, P1, A1, S1] = v[i];
            auto [str2, P2, A2, S2] = v[j];

            int win1 = win(P1, P2) + win(A1, A2) + win(S1, S2);
            int win2 = win(P2, P1) + win(A2, A1) + win(S2, S1);
    ...
    auto bfs = [&](int start) -> void {
        ...
        while(!q.empty()) {
            ...
            for(auto next : edge[cur]) {
                tinDegree[next]++;
                if(tinDegree[next]!=inDegree[next]) continue;
                visited[next] = 1;
                q.push(next);
            }
        }
    ...
    vector<string> answer;
    rep(i,1,N) {
        if(inDegree[i]==0) answer.pbk(get<0>(v[i]));
    }

    sort(all(answer));

    for(auto str : answer) cout << str << endl;
}
int32_t main() {
