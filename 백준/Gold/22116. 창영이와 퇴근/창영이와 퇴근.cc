...
// This is Union Find
#define MAX 1001
int parent[MAX*MAX]; //parent[MAX] should be 1, 2, 3...
...
//This is Kruskal with union find (MST)
...
int main() {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N - 1; j++) {
			edge.push_back(Edge(idx, idx + 1, abs(toHome[i][j] - toHome[i][j + 1])));
			idx++;
		}
		idx++;
	}
	...
	for (int i = 0; i < N - 1; i++) {
		for (int j = 0; j < N; j++) {
			edge.pb(Edge(idx, idx + N, abs(toHome[i][j] - toHome[i + 1][j])));
			idx++;
		}
	}
	Kruskal();
