...
void Solve() {
    ...
    */
    ...
    vector<int> v(N+1, 1e9); //현재 노드가 i일때 src에서 i까지 거리가 v[i]
    p_q<pii, vpii, greater<>> pq; //distance, node
    pq.push({0,src});
    v[src] = 0;
    while(!pq.empty()) {
        auto [d, cur] = pq.top();
        pq.pop();
        // if(v[cur]!=d) continue;
        for(auto [next , dd] : edge[cur]) { //delta distance
            auto nd = d + dd;
            if(nd >= v[next]) continue;
            v[next] = nd;
            pq.push({nd, next});
        }
    }
    for(int i=1;i<=N;i++) {
        if(v[i]==1e9) cout << "INF" << endl;
        else cout << v[i] << endl;
    }
}
int32_t main() {
