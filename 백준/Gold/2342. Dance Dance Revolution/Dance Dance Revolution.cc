    ...
    for(int k=0;k<N;k++) {
        for(int i=0;i<5;i++) {
            for(int j=0;j<5;j++) {
                ...
                if(i==v[k+1]) chmin(dp[k+1][i][j], dp[k][i][j]+1);
                if(j==v[k+1]) chmin(dp[k+1][i][j], dp[k][i][j]+1);

                if(i==0) chmin(dp[k+1][v[k+1]][j], dp[k][i][j]+2);
                if(j==0) chmin(dp[k+1][i][v[k+1]], dp[k][i][j]+2);

                if(i>0 && abs(i-v[k+1])!=2) chmin(dp[k+1][v[k+1]][j], dp[k][i][j]+3);
                if(j>0 && abs(j-v[k+1])!=2) chmin(dp[k+1][i][v[k+1]], dp[k][i][j]+3);

                if(i>0 && abs(i-v[k+1])==2) chmin(dp[k+1][v[k+1]][j], dp[k][i][j]+4);
                if(j>0 && abs(j-v[k+1])==2) chmin(dp[k+1][i][v[k+1]], dp[k][i][j]+4);
            }
        }
        // for(int i=0;i<5;i++) {
        //     for(int j=0;j<5;j++) {
