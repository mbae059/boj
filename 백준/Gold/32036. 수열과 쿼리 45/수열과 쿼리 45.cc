...
p_q<int> lower;
p_q<int, vi, greater<>> upper;

void setPQ(int num, int& l, int& r) {
...
pii getAnswer(int num, int &l, int& r, int y) {
    setPQ(num, l, r);
    int ret = 0;
    // cout << l << " : " << r << endl;
    if(!lower.empty()) ret += lower.size() * lower.top() - l;
    if(!upper.empty()) ret += -lower.top() * upper.size() + r;

    return {lower.top(), ret+y};
}
void Solve() {
    cin >> Q;
    pii answer;
