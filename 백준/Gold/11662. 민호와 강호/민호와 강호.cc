...
pdd minho(double p) {
    return {l1.first + (r1.first-l1.first) * (p/100), l1.second + (r1.second - l1.second) * (p/100)};
}
pdd ganho(double p) {
    return {l2.first + (r2.first-l2.first) * (p/100), l2.second + (r2.second - l2.second) * (p/100)};
}
...
void Solve() {
    ...
    while(hi-lo>= 1e-6) {
        ...
        auto distP = dist(m_p, g_p);
        auto distQ = dist(m_q, g_q);

        answer = min({answer, distP, distQ});
        if(distP < distQ) {
            hi = q;
        }
        else lo = p;
