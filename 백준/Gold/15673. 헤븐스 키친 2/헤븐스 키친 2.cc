...
void Solve() {
    ...
    vvi prefixL(N+2, vi(2));
    vvi prefixR(N+2, vi(2));

    rep(i,1,N) {
        int valL[] = {prefixL[i-1][0] + a[i], prefixL[i-1][1] + a[i], a[i]};
        prefixL[i][0] = *min_element(valL, valL+3);
        prefixL[i][1] = *max_element(valL, valL+3);
    }
    for(int i=N;i>=1;i--) {
        int valR[] = {prefixR[i+1][0] + a[i], prefixR[i+1][1] + a[i], a[i]};
        prefixR[i][0] = *min_element(valR, valR+3);
        prefixR[i][1] = *max_element(valR, valR+3);
    }

    rep(i,2,N) {
        chmin(prefixL[i][0], prefixL[i-1][0]);
        chmax(prefixL[i][1], prefixL[i-1][1]);
    }

    for(int i=N-1;i>=1;i--) {
        chmin(prefixR[i][0], prefixR[i+1][0]);
        chmax(prefixR[i][1], prefixR[i+1][1]);
    }

    int answer=-1e9;
    rep(i,1,N-1) {
        chmax(answer, prefixL[i][0] * prefixR[i+1][0]);
        chmax(answer, prefixL[i][1] * prefixR[i+1][1]);
    }
    print(answer);
}
int32_t main() {
