...
void Solve() {
    ...
    while(!q.empty()) {
        ...
        while(cur) {
            int val = cur%10;
            cur/=10;
            if(((cur && val>=1) || (cur==0 && val>1)) && ccur-idx>0 && dp[ccur-idx]==1e18) {
                dp[ccur-idx] = dp[ccur]+1;
                q.push(ccur-idx);
            }
            if(val<9 && ccur+idx<=5'000'000 && dp[ccur+idx]==1e18) {
                dp[ccur+idx] = dp[ccur]+1;
                q.push(ccur+idx);
            }
            idx*=10;
        ...
        if(ccur+1<=5'000'000 && dp[ccur+1]==1e18) {
            dp[ccur+1] = dp[ccur]+1;
            q.push(ccur+1);
        }
        if(ccur-1>0 && dp[ccur-1]==1e18) {
            dp[ccur-1] = dp[ccur]+1;
            q.push(ccur-1);
        }
    }
