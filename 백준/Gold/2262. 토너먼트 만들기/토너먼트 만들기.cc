...
struct Segtree {
    ...
    T query(int node, int l, int r, int nodeLeft, int nodeRight) {
        if(nodeRight<l || r<nodeLeft) return 1e9; //could be 0, 1e9, -1e9
...
Segtree<int, bop> segtree(MAX);
map<pii, int> mp;
int go(int l, int r) {
    if(l==r) return 0;
    else if(l+1==r) return abs(segtree.a[r]-segtree.a[l]);
    if(mp.find({l,r})!=mp.end()) return mp[{l,r}];
    int ret = 1e9;
    for(int i=l;i<=r-1;i++) {
        int lhs = go(l,i);
        int rhs = go(i+1,r);
        int lmn = segtree.query(1,l,i,1,N);
        int rmn = segtree.query(1,i+1,r,1,N);
        // if(l==4 && r==6) cout << lhs << " " << rhs << " " << lmn << " " << rmn << endl;
        ret = min(ret, lhs+rhs+abs(lmn-rmn));
    }
    mp[{l,r}] = ret;
    // cout << l << " " << r << " " << ret << endl;
    return ret;
