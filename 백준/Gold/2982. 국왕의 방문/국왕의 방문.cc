    ...
    while (!pq.empty()) {
        ...
        for (auto next : edge[cur]) {
            if(d[next.first] > d[cur] + next.second) {
                if(mp[cur][next.first]!=make_pair(0,0) && (dis>=mp[cur][next.first].first) && dis < mp[cur][next.first].second) 
                    d[next.first] = mp[cur][next.first].second + next.second;
                else {
...
void Solve() {
    ...
    rep(i,0,G-2) {
        int a = v.at(i);
        int b = v.at(i+1);
        mp[a][b] = make_pair(sum, sum+dp[a][b]);
        mp[b][a] = make_pair(sum, sum+dp[a][b]);
        sum += dp[a][b];
    }
    Dijkstra(A);
    cout << d[B]-K;
