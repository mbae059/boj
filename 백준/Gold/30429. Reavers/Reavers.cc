...
void dfs(int cur) {
    visited[cur]=1;
    for(auto [next, info] : rarr[cur]) {
        if(visited[next]) continue;
        if((isAlien[cur] ^ info)==0) {
            v1.pbk(next);
            isAlien[next]=0;
        }
        else {
...
int tinDegree[100001] {};
bool visited1[100001] {};
void dfs1(int cur) {
    visited1[cur] = 1;
    auto [next, info] = arr[cur];
    tinDegree[next]++;
    if(tinDegree[next]==inDegree[next]) {
        dfs1(next);
    }
}
void getBegin() {
...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        rarr[arr[i].first].pbk({i, arr[i].second});
        inDegree[arr[i].first]++;
    }
    memset(isAlien,-1,sizeof(isAlien));

    getBegin();
    int answer=0;
    
    rep(i,1,N) { //cycle로만 이루어진 graph 처리
        if(visited1[i]==0) {
            isAlien[i]=0;
            v1.pbk(i);
            dfs(i);
            answer += min(v1.size(), v2.size());
            v1.clear();
            v2.clear();
        }
    }
