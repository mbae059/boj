...
int getDif(int target, int current) {
...
void Solve() {
    ...
    if(abs(a[1]-a[2])<X) chmin(answer, X-abs(a[1]-a[2]));
    else chmin(answer,0);
    if(abs(a[N-1]-a[N])<X) chmin(answer, X-abs(a[N-1]-a[N]));
    else chmin(answer,0);

    rep(i,2,N-1) {
        chmin(answer, getDif(a[i]+X, a[i-1]) + getDif(a[i]+X, a[i+1]));
        chmin(answer, getDif(a[i+1]+X, a[i]) + getDif(max(a[i], a[i+1]+X)+X, a[i-1]));
        chmin(answer, getDif(a[i-1]+X, a[i]) + getDif(max(a[i], a[i-1]+X)+X, a[i+1]));
        chmin(answer, getDif(max(a[i-1],a[i+1])+X, a[i]));
    }
    cout << answer;
