...
void Solve() {
    ...
    auto getPoint = [&](int l, int r) {
        return (r-l+1) * (prefix[r] - prefix[l-1]);
    };
    for(int _ = 0;_<N-1;_++) {
        int i = order[_];
        if(visited[i-1] && visited[i+1]) {
            auto [l1, r1] = range[dsu.getParent(i-1)];
            auto [l2, r2] = range[dsu.getParent(i+1)];
            local -= getPoint(l1,r1) + getPoint(l2,r2);
            local += getPoint(l1, r2);
            dsu.merge(i-1,i);
            dsu.merge(i,i+1);
            
            range[dsu.getParent(i)] = {l1, r2};
        }
        else if(visited[i-1]) {
            auto [l,r] = range[dsu.getParent(i-1)];
            local -= getPoint(l,r);
            local += getPoint(l, r+1);
            dsu.merge(i-1,i);

            range[dsu.getParent(i)] = {l, r+1};
        }
        else if(visited[i+1]) {
            auto [l,r] = range[dsu.getParent(i+1)];
            local -= getPoint(l,r);
            local += getPoint(l-1, r);
            dsu.merge(i,i+1);

            range[dsu.getParent(i)] = {l-1, r};
        }
        else {
            local += a[i];
            range[i] = {i,i};
