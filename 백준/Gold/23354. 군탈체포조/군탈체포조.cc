...
void Solve() {
	...
	vvi edge(idx+1, vi(idx+1));
	for(auto [p, idx] : mp) {
		auto [y,x] = p;
		
		vvi visited(N+1, vi(N+1));
		visited[y][x] = 1;
		...
		while(!pq.empty()) {
			...
			for(int i=0;i<4;i++) {
				...
				visited[ny][nx] = 1;
				if(matrix[ny][nx]==-1) pq.push({d, ny, nx});
				else pq.push({d+matrix[ny][nx], ny, nx});
			}
		}
	}
	...
	do {
		...
		chmin(answer, tanswer);
	} while(next_permutation(path.begin()+1, path.begin()+idx));
	print(answer);
}
int32_t main() {
