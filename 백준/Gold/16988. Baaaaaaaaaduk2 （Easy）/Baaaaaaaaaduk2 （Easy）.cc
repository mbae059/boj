...
void dfs(int y, int x) {
    ...
    rep(i,0,3) {
        ...
        if(matrix[nexty][nextx]==0) flag = false;
        else if(matrix[nexty][nextx]==2 && visited[nexty][nextx]==0) {
            dfs(nexty, nextx);
        }
    }
}
...
void Solve() {
    ...
    for(int i=0;i<v.size();i++) {
        for(int j=i+1;j<v.size();j++) {
            ...
            matrix[p1.first][p1.second] = 0;
            matrix[p2.first][p2.second] = 0;
        }
    }
    cout << ganswer;
}
