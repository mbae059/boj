...
void Solve() {
    ...
    vvi ground(N+1, vi(M+1, 1));

    rep(i,0,N) ground[i][0] = 0;
    rep(j,0,M) ground[0][j] = 0;
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            ground[i][j] = ground[i-1][j] + ground[i][j-1] - ground[i-1][j-1] + ground[i][j];
        }
    }
    ...
    while (Q--) {
        ...
        int buildings1 = ground[r2][c2];
        int buildings2 = ground[r2][c1-1];
        int buildings3 = ground[r1-1][c2];
        int buildings4 = ground[r1-1][c1-1];

        int dif = buildings1 - buildings2 - buildings3 + buildings4;
