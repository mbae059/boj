...
void Solve() {
    ...
    auto dfs = [&](auto&& self, int cur) -> pair<int,int> {
        ...
        for(auto next : edge[cur]) {
            ...
            auto p = self(self, next);
            if(a[cur]+p.second > ret.second) {
                ret = {p.first, a[cur] + p.second};
            }
            else if(a[cur]+p.second == ret.second) {
    ...
    auto node1 = dfs(dfs, 1).first;
    fill(all(visited),0);
    auto p = dfs(dfs, node1);
    cout << p.second << " " << min(node1, p.first);
}
int32_t main() {
