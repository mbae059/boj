...
void Solve() {
    ...
    vector<vector<vector<bool>>> dp(N, vector<vector<bool>>(1001, vector<bool>(2, 0)));

    auto inRange = [&](int idx, int a, int b) {
        return a <= idx && idx <= b;
    };
    
    bool answer = 0;

    auto dfs = [&](auto&& self, int cnt, int idx, bool shield) -> void {
        if(idx>1000) return;
        if(idx<=0) return;

        if(dp[cnt][idx][shield]) return;
        
        dp[cnt][idx][shield] = true;

        auto [mn, mx] = v[cnt];

        if(inRange(idx, mn, mx)) {
            if(cnt==N-1) {
                answer = 1;
                return;
            }
            self(self, cnt+1, idx+1, shield);
            self(self, cnt+1, idx-1, shield);
            self(self, cnt+1, idx, shield);
        }
        else if(!inRange(idx, mn-1, mx+1) && shield) {
            if(cnt==N-1) {
                answer = 1;
                return;
            }
            ...
            self(self, cnt+1, idx+1, 0);
            self(self, cnt+1, idx-1, 0);
            self(self, cnt+1, idx, 0);
        }
    };
