...
pll dfs(ll num, ll block, ll d) {
    ...
    for(auto p : edge[num]) {
        ...
        auto val = dfs(next, block, nextD);
        
        if(ret.second < val.second) ret = val;
    }
    return ret;
}
ll getDiameter(ll root, ll block) {
    memset(visited, 0, sizeof(visited));
    ll a = dfs(root, block, 0).first;
    memset(visited, 0, sizeof(visited));
    return dfs(a, block, 0).second;
}
void Solve() {
    cin >> N;
    rep(i,1,N-1) {
    ...
    for(auto t : v) {
        ll a, b, c;
        tie(a,b,c) = t;
        answer = max(answer, getDiameter(a, b) + getDiameter(b, a) + c);
    }
    cout << answer;
}
int main() {
