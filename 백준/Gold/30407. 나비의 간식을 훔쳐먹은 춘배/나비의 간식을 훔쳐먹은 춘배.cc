...
set<tuple<int,int,bool,bool,int>> s;
int r[222] {};

int go(int idx, int distance, bool ignore1, bool ignore2, int health) {
    if(health<=0) return -1;
    if(idx==N+1) return health;
    if(s.find({idx, distance, ignore1, ignore2, health})!=s.end()) return -1;
    s.insert({idx,distance, ignore1, ignore2, health});
    if(ignore2) {
        return go(idx+1, distance+K, 0, 0, health);
    }
    ...
    int nhealth;
    // /2
    ndamage = max(0ll, (damage-distance)/2);
    nhealth = health - ndamage;
    int ret1 = go(idx+1, distance, ignore1, ignore2, nhealth);
    // move away
    int ndistance = distance + K;
    ndamage = max(0ll, damage - ndistance);
    nhealth = health - ndamage;
    int ret2 = go(idx+1, ndistance, ignore1, ignore2, nhealth);
    int ret3 = -1;
    if(ignore1) {
        ndamage = max(0ll, damage - distance);
        ret3 = go(idx+1, distance, 0, 1, health - ndamage);
    }
    // cout << idx << " " << distance << " " << ignore1 << " " << ignore2 << " " << health << " " << max({ret1,ret2,ret3}) << endl;
