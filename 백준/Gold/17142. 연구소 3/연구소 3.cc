...
void bfs() {
	...
	for(auto i : combination) {
		q.push(v[i]);
		infection[v[i].first][v[i].second] = 0;
	}
	...
	while(!q.empty()) {
		...
		q.pop();
		if(matrix[y][x]!=2) totalInfection = max(totalInfection, infection[y][x]);
		rep(i,0,3) {
			int nexty = y+dir[i][0];
			int nextx = x+dir[i][1];
			if(!inRange(nexty, nextx)) continue;
			if(infection[nexty][nextx]!=INF) continue;
			if(matrix[nexty][nextx]==1) continue;
			infection[nexty][nextx] = infection[y][x] + 1;
			q.push({nexty,nextx});
		}
	}
	rep(i,1,N) {
		rep(j,1,N) {
			if(matrix[i][j]!=1 && infection[i][j]==INF) return;
		}
	}
	answer = min(answer, totalInfection);
}
