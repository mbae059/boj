...
int getBill(int w) {
    int ret = 0;
    if(w<=100) return ret + w * 2;
    ret += 200;
    w -= 100;
    if(w<=9900) return ret + w * 3;
    ret += 9900*3;
    w -= 9900;
    if(w<=990'000) return ret + w * 5;
    ret += 990'000*5;
    w -= 990'000;
...
void Solve() {
    while(true) {
        ...
        int totalElectric = getElectric(A);
        
        int l = (totalElectric+1)/2;
        int r = totalElectric;
        ...
        while(l<=r) {
            ...
            int high = mid;
            int low = totalElectric - high;

            int dif = getBill(high) - getBill(low);
            if(dif>=B) {
                answer = getBill(low);
                r = mid-1;
            }
