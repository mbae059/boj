...
void Solve() {
	...
	if(v==vector<int>{-1}) print(matrix[0][H]);
	if(v==vector<int>{0}) print(matrix[1][H]);
	if(v==vector<int>{1}) print(matrix[2][H]);
	if(v==vector<int>{-1,0}) print(matrix[3][H]);
	if(v==vector<int>{0,1}) print(matrix[4][H]);
	if(v==vector<int>{-1,1}) print(matrix[5][H]);
	if(v==vector<int>{-1,0,1}) print(matrix[6][H]);
...
int32_t main() {
	...
	rep(j,3,1000000) {
		...
		matrix[3][j] = (matrix[3][j-1] * matrix[3][j-1] + matrix[3][j-1] * matrix[3][j-2])%mod;
		matrix[4][j] = (matrix[4][j-1] * matrix[4][j-1] + matrix[4][j-1] * matrix[4][j-2])%mod;
		matrix[5][j] = (2 * matrix[5][j-1] * matrix[5][j-2])%mod;

		matrix[6][j] = (matrix[6][j-1] * matrix[6][j-2] * 2 + matrix[6][j-1] * matrix[6][j-1]) % mod;
	}
    cin >> T;
    while(T--) Solve();
}