...
void dfs(int y, int x, int d) {
    ...
    if(matrix[y][x]==1 && (d==2 || d==3)) return;
    if(matrix[y][x]==2 && (d==0 || d==1)) return;

    int nextd = d;
    if(matrix[nexty][nextx]==3) {
        if(d==0) nextd = 3;
        if(d==1) nextd = 2;
        if(d==2) nextd = 1;
        if(d==3) nextd = 0;
    }
    else if(matrix[nexty][nextx]==4) {
        if(d==0) nextd = 2;
        if(d==1) nextd = 3;
        if(d==2) nextd = 0;
        if(d==3) nextd = 1;
    }
    if(visited[nexty][nextx][nextd]) return;
...
void Solve() {
    ...
    for(auto p : src) {
        rep(i,0,3) dfs(p.first, p.second, i);
    }
