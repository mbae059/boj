...
void Solve() {
    ...
    int answer=N-1;
    if(!difv.empty()) chmin(answer, difv.front());

    for(auto dif : difv) {
        answerv.pbk(min(answer, dif/3));
    }
    ...
    if(difv.size()>=2) {
        ...
        answerv.pbk(min(answer,tdifv.front()));
    }
    if(f1<f2) swap(f1,f2);
    if((f1 || f2) && !difv.empty()) {
        auto tdifv = difv;
        ...
        tdifv.pbk(x1/2);
        sort(all(tdifv));
        answerv.pbk(min({answer, f1, tdifv.front()}));
    }

    if(f1 && f2) {
        answerv.pbk(min({answer, f1,f2}));
    }

    assert(!answerv.empty());
    cout << *max_element(all(answerv));
