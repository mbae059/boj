...
bool cal(char tmatrix[10][10], const vi& v) {
    int ret = 0;
    for(auto i : v) {
        auto [a,b] = coord[i];
        int val = tmatrix[a][b] - 'A' + 1;
        ret += val;
    }
    return ret==26;
}
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            cin >> matrix[i][j];
            char c = matrix[i][j];
            if('A'<=c && c<='L') alpha.erase(find(all(alpha), c));
            if(c=='x') v.pbk({i,j});
        }
    }
    coord.pbk({1,5});
    ...
    do {
        ...
        rep(i,1,N) {
            rep(j,1,M) {
                ...
                if(tmatrix[i][j]=='x') {
                    tmatrix[i][j] = alpha[idx];
                    idx++;
                }
            }
        }
        
        if(cal(tmatrix, {1,2,3,4}) && cal(tmatrix, {4,6,9,11}) && cal(tmatrix, {1,5,8,11}) && cal(tmatrix, {0,2,5,7}) && cal(tmatrix, {0,3,6,10}) && cal(tmatrix, {7,8,9,10})) {
            rep(i,1,N) {
                rep(j,1,M) cout << tmatrix[i][j];
                cout << endl;
            }
            exit(0);
        }
