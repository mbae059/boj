...
bool isVampire(ll a, ll b) {
    ...
    rep(i,0,9) if(cnt1[i]!=cnt2[i]) return false;
    return true;
}
void Solve() {
    for(ll i=1;i<=1000000;i++) {
        for(ll j=i+1;j<=1000000;j++) {
            if(i*j>=1000256) break;
            if(isVampire(i,j)) vampire.pbk(i*j);
        }
    }
    sort(all(vampire));
    while(cin >> N){
        if(N==0) return;
        cout << *lower_bound(all(vampire), N) << endl;
    }
}
int main() {
