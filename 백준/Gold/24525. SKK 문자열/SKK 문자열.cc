...
void Solve() {
    ...
    for(int i=1;i<=N;i++) {
        ...
        if(c[i]=='S') prefixS[i]+=2;

        prefixK[i] = prefixK[i-1];
        if(c[i]=='K') prefixK[i]++;

        prefixSK[i] = prefixS[i] - prefixK[i];
    }
    ...
    q.push({0,0});
    for(int i=1;i<=N;i++) {
        if(!q.empty() && q.front().first!=prefixSK[i]) {
            mp[q.front().first] = q.front().second;
            q.pop();
        }
        int dif = prefixSK[i];
        if(mp.contains(dif)) {
            chmax(answer, i-mp[dif]);
        }
        else {
            if(q.empty()) q.push({dif, i});
        }
    }
    print(answer);
}
