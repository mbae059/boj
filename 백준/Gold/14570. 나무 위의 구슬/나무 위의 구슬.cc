...
void Solve() {
    ...
    ll node = 1;
    while(1) {
        if(edge[node].left == -1 && edge[node].right == -1) break;

        else if(edge[node].left == -1) node = edge[node].right;
        else if(edge[node].right == -1) node = edge[node].left;
