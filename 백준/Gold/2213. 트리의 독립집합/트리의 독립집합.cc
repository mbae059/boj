...
int w[MAX] {};
vi edge[MAX];
int dp[MAX][2] {}; //0 is don't choose, 1 is choosen
bool visited[MAX] {};
vi trace[MAX][0];
vpii answerEdge[MAX][2];
vi answer;
void dfs(int cur) {
	visited[cur] = 1;
	...
	for(auto next : edge[cur]) {
		...
		if(dp[next][0] > dp[next][1]) {
			dp[cur][0] += dp[next][0];
			answerEdge[cur][0].pbk({next, 0});
		}
		else {
			dp[cur][0] += dp[next][1];
			answerEdge[cur][0].pbk({next, 1});
		}
		dp[cur][1] += dp[next][0];
		answerEdge[cur][1].pbk({next, 0});
	}
}

void answerDfs(int cur, int chosen) {
	if(chosen==1) answer.pbk(cur);

	for(auto next : answerEdge[cur][chosen]) {
		answerDfs(next.first, next.second);
	}
}
void Solve() {
	cin >> N;
