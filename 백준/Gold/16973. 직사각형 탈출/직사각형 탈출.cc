...
void Solve() {
    ...
    while(!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            ...
            if(nextx < 0 || nexty < 0 || nextx+H > N || nexty+W > M) continue;
            [&] () {
                for(int i=0;i<H;i++) {
                    if(matrix[i+nextx][nexty] || matrix[i+nextx][nexty+W-1]) return;
                }
                for(int i=0;i<W;i++) {
                    if(matrix[nextx][i+nexty] || matrix[nextx+H-1][i+nexty]) return;
                }
