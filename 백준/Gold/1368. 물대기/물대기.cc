...
void Prim() { //MST
    ...
    primPQ.push(Node(pq.top().second,pq.top().first));
    pq.pop();
    
    ll sum = 0;
    while(!primPQ.empty()) {
        int cur = primPQ.top().node;
        int dis = primPQ.top().dis;
        primPQ.pop();
        ...
        for(auto next : edge[cur]) {
            ...
            if(v[nextNode] >= nextDis) {
                primPQ.push(Node(nextNode, nextDis));
            }
            else {
                primPQ.push(Node(nextNode, v[nextNode]));
            }
        }
    }
    cout << sum;
