...
void Solve() {
    ...
    press.pbk({0,0});
    rep(i,1,N) {
        if(v[i]==press.back().first) press.back().second++;
        else press.pbk({v[i], 1});
    }
    N = press.size()-1;

    rep(i,1,N) prefix[i] = prefix[i-1] + press[i].first*press[i].second;

    ll answer = 0;
    for(int i=2;i<N;i++) {
        answer = (answer + (prefix[i-1] * (prefix[i] - prefix[i-1])) % MOD * (prefix[N]-prefix[i]) % MOD) % MOD;
    }
    cout << answer;
}
int main() {
