...
void Solve() {
    ...
    vector<vector<vector<int>>> dp(N+1, vvi(M+1, vi(T+1,1e18)));
    rep(i,1,N) {
    ...
    for(int t=0;t<=T;t++) dp[1][1][t] = 0;
    p_q<tiiii, vtiiii, greater<>> pq;
    pq.push({0, 0, 1, 1});
    while(!pq.empty()) {
        auto [d, t, y, x] = pq.top();
        pq.pop();

        if(dp[y][x][t]!=d) continue;
        // cout << d << " " << t << " " << y << " " << x << endl;
        for(int i=0;i<4;i++) {
            int ny = y+dy[i];
            int nx = x+dx[i];
            if(!inRange(ny,nx)) continue;
            if(dp[ny][nx][t]>d+matrix[ny][nx]) {
                dp[ny][nx][t] = d + matrix[ny][nx];
                pq.push({dp[ny][nx][t], t, ny, nx});
            }
        }

        if(!teleport.contains({y,x})) continue;
        if(t>=T) continue;
        
        auto [ty, tx] = teleport[{y,x}];
        if(dp[ty][tx][t+1]>d) {
            dp[ty][tx][t+1] = d;
            pq.push({dp[ty][tx][t+1], t+1, ty, tx});
        }
    }

    vvi answer(N+1, vi(M+1, 1e18));
    for(int i=1;i<=N;i++) {
        for(int j=1;j<=M;j++) {
            for(int t=0;t<=T;t++) {
                chmin(answer[i][j], dp[i][j][t]);
            }
        }
    }
    while(Q--) {
        int p, a, b; cin >> p >> a >> b;
        if(answer[a][b]<=p) print(1);
        else print(0);
    }
}
int32_t main() {
