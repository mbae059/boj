...
bool isGood(const string& str, const string& good) {
    int idx = 0;
    for(auto c : str) {
        if(c==good[idx]) idx++;
        if(idx==good.size()) return true;
    }
    return false;
}
void Solve() {
    ...
    rep(i,1,N) {
        cin >> str[i];
        for(auto& c : str[i]) c = tolower(c);
    }
    rep(i,1,M) {
        cin >> good[i];
        for(auto& c : good[i]) c = tolower(c);
    }
    for(int i=1;i<=N;i++) {
        int cnt = 0;
        for(int j=1;j<=M;j++) {
            if(isGood(str[i], good[j])) cnt++;
        }
        cout << cnt << endl;
    }
}
