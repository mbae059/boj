...
void Solve() {
    ...
    for(int i=0;i<N;i++) {
        for(int j=i;j<N;j++) {
            for(int k=j;k<N;k++) {
                auto [x1,y1,z1] = comb[i];
                auto [x2,y2,z2] = comb[j];
                auto [x3,y3,z3] = comb[k];
                cnt++;
                int x = t[x1][y1][z1];
                int y = t[x2][y2][z2];
                int z = t[x3][y3][z3];

                int val = x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3];
                if(i==j && j==k) {
                    if(x>=3) answer = max(answer, x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3]);
                }
                else if(i==j) {
                    if(x>=2 && z>=1) answer = max(answer, x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3]);
                }
                else if(i==k) {
                    if(x>=2 && y>=1) answer = max(answer, x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3]);
                }
                else if(j==k) {
                    if(x>=1 && y>=2) answer = max(answer, x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3]);
                }
                else {
                    if(x>=1 && y>=1 && z>=1) answer = max(answer, x1+x2+x3+a[y1][y2]+a[y1][y3]+a[y2][y3]+b[z1][z2]+b[z1][z3]+b[z2][z3]);
                }
            }
        }
    }
