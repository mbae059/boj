...
bool isDifferent(char c1, char c2) {
    return (c1=='B' && c2=='W') || (c1=='W' && c2=='B');
}
void Solve() {
    cin >> N >> M;
    char matrix[N+1][M+1] {};
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            dp[i][j] = 1;
            if(isSame(matrix[i][j], matrix[i-1][j-1]) && isDifferent(matrix[i-1][j], matrix[i][j]) && isDifferent(matrix[i][j-1], matrix[i][j])) {
