...
public:
    ...
    int dis;
    char z;
    Edge(int a, int b, int z, int dis) : z(z), dis(dis){
        node[0] = a;
        node[1] = b;
    }
    bool operator < (const Edge& edge) {
        if(z!=edge.z) return z<edge.z;
...
struct Answer {
    ...
    void chmin(const string& _str, int _mn) {
        if(_str.size()!=N-1) return;
        if(str.empty() || str>_str) {
            str = _str;
            mn = _mn;
        }
    }
