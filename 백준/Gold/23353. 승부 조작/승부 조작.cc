...
void Solve() {
	...
	vvi prefix(N+2, vi(N+2));
	vvi rprefix(N+2, vi(N+2));

	auto prefixRow = [&](vvi& prefix, vvi& rprefix) {
		rep(j,1,N) {
			rep(i,1,N) {
		...
		rep(i,1,N) {
			rep(j,1,N) {
				...
				chmax(answer, prefix[i-1][j] + rprefix[i+1][j] + 1);
			}
		}
		return answer;
	};

	auto prefixColumn = [&](vvi& prefix, vvi& rprefix) {
		rep(i,1,N) {
			rep(j,1,N) {
				if(matrix[i][j]!=1) continue;
		...
		rep(i,1,N) {
			rep(j,1,N) {
				if(matrix[i][j]==0) continue;
				chmax(answer, prefix[i][j-1] + rprefix[i][j+1] + 1);
			}
		}
		return answer;
	};

	auto prefixDiagonal1 = [&](vvi& prefix, vvi& rprefix) {
		rep(i,1,N) {
			rep(j,1,N) {
				if(matrix[i][j]!=1) continue;
		...
		rep(i,1,N) {
			rep(j,1,N) {
				if(matrix[i][j]==0) continue;
				chmax(answer, prefix[i-1][j-1] + rprefix[i+1][j+1] + 1);
			}
		}
		return answer;
	};

	auto prefixDiagonal2 = [&](vvi& prefix, vvi& rprefix) {
		rep(i,1,N) {
			rep(j,1,N) {
				if(matrix[i][j]!=1) continue;
				prefix[i][j] = prefix[i-1][j+1] + 1;
	...
	int answer=0;
	chmax(answer, prefixRow(prefix, rprefix));
	chmax(answer, prefixColumn(prefix, rprefix));
	chmax(answer, prefixDiagonal1(prefix, rprefix));
	chmax(answer, prefixDiagonal2(prefix, rprefix));
