...
int cnt = 0;
int k1, k2;
void dq(int r1, int r2, int c1, int c2, bool black) {
    if(r2 <= R1 || R2<r1 || c2 <=C1 || C2<c1) return;
    if(r1==r2-1 && c1==c2-1) {
        answer[r1-R1][c1-C1] = black;
        return;
    }
    int step = (r2-r1)/N;
    rep(i,0,N-1) {
        rep(j,0,N-1) {
            if(k1 <= i && i <= k2 && k1 <= j && j <= k2) dq(r1+step*i, r1+step*(i+1), c1+step*j, c1+step*(j+1), 1);
            else dq(r1+step*i, r1+step*(i+1), c1+step*j, c1+step*(j+1), black);
        }
    }
...
void Solve() {
    ...
    dq(0, (int)pow(N, s), 0, (int)pow(N, s), 0);
