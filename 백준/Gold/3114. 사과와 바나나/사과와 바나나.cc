...
void Solve() {
	...
	rep(i,1,N) {
		rep(j,1,M) {
			string str; cin >> str;
			char c = str.front();
			int cnt = stoi(str.substr(1));
			if(c=='B') banana[i][j] = cnt;
			else apple[i][j] = cnt;
		}
	}
	...
	rep(j,1,M) {
		rep(i,1,N) {
			bprefix[i][j] = bprefix[i-1][j] + banana[i][j];
		}
	}
	rep(i,1,N) {
		rep(j,1,M) {
			aprefix[i][j] = aprefix[i][j-1] + apple[i][j];
		}
	}

	rep(i,2,N) {
		rep(j,2,M) {
			dp[i][j] = max({dp[i-1][j-1]+bprefix[i-1][j]+aprefix[i][j-1], dp[i-1][j]+aprefix[i][j-1], dp[i][j-1]+bprefix[i-1][j]});
		}
	}
	cout << dp[N][M];
}
