...
void Solve() {
	...
	auto func = [](int val, int dif, vi& arr, int& cnt) {
        cnt -= abs(arr[val]);
		arr[val] += dif;
        cnt += abs(arr[val]);
	};

	auto dfs = [&](auto&& dfs, int l, int r, vi& arr, int cnt) -> void {
		if(l==r) return;
		if((r-l+1)%2==0 && cnt==0) chmax(answer, r-l+1);
		if(dp[l][r]) return;
		...
		if((r-l+1)%2==0) {
			func(v[l], -1, arr, cnt);
			func(v[mid+1], 2, arr, cnt);
			dfs(dfs, l+1, r, arr, cnt);
			func(v[l], 1, arr, cnt);
            func(v[mid+1], -2, arr, cnt);
            
            func(v[r], 1, arr, cnt);
			dfs(dfs, l, r-1, arr, cnt);
            func(v[r], -1, arr, cnt);
		}
		else {
            func(v[l], -1, arr, cnt);
