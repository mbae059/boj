...
vector<int> v;
si s;
void dfs(int idx, const string& str) {
    if(idx==N) {
        s.insert(accumulate(all(v), 0LL));
        return;
    }
    for(int i=1;i+idx<=N;i++) {
        v.pbk(stoi(str.substr(idx,i)));
        dfs(i+idx, str);
        v.pop_back();
    }
}

int getSum(const string& str, int m) {
    int ret = 0;
    for(auto c : str) {
        int digit = c-'0';
        ret += pow(digit, m);
...
void Solve() {
    ...
    if(other==0) cout << "Hello, BOJ 2023!" << endl;
    else {
        int answer=0;
        for(int m=1;;m++) {
            int val = getSum(str, m);
            if(s.find(val)!=s.end()) answer++;
            if(val>1e9) break;
        }
        print(answer);
    }
