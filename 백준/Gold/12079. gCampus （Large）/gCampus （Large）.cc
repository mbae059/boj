...
#define MAX 101
pair<int, vi> dp[MAX][MAX]; //input should be done in dp table
...
void Solve() {
    ...
    rep(i,0,N-1) {
        rep(j,0,N-1) {
            for(auto k : dp[i][j].second) visited[k] = 1;
        }
    }
    rep(i,0,M-1) if(visited[i]==0) cout << i << " ";
    cout << endl;
}
int32_t main() {
