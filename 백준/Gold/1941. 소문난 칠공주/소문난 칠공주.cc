...
bool chkConnected() {
    ...
    memset(visitedConnected, 0, sizeof(visitedConnected));
    ...
    queue<pii> q;
    int firsty = v[combination[0]].first;
    int firstx = v[combination[0]].second;

    q.push({firsty, firstx});
    visitedConnected[firsty][firstx] = 1;
    int cnt = 0;
    while(!q.empty()) {
        int y = q.front().first;
        ...
        rep(i,0,3) {
            ...
            if(visitedConnected[nexty][nextx]) continue;
            if(seat[nexty][nextx]==0) continue;
            visitedConnected[nexty][nextx] = 1;
            q.push({nexty, nextx});
        }
    }
