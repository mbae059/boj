...
void wiggle(pii p, spii& s, int k) {
    s.insert(p);
    if(k==0) return;
...
bool cmp(spii& s, pii p) {
    return s.contains(p);
}

bool cmp(spii& s1, spii& s2) {
    for(auto p : s1) {
...
bool cmp2(spii& s1, spii& s2) {
    spii s3;
    for(auto [p1, p2] : s1) {
        for(auto [p3, p4] : s2) {
            s3.insert({p1-p3, p2-p4});
        }
    }
    for(auto p : s2) {
        if(s3.contains(p)) return true;
    }
    return false;
}

bool cmp3(spii& s, pii p) {
    for(auto [a,b] : s) {
        pii tp = {p.first - a, p.second - b};
        if(s.contains(tp)) return true;
    }
    return false;
}
void Solve() {
    ...
    pii dest; cin >> dest.first >> dest.second;
    vector<spii> vdest = make_vdest(dest);
    int answer=-1;
    if(K>=5) {
        if(cmp(vdest[5], {0,0}) || cmp(vdest[3], items) || cmp2(vdest[1], items)) answer = 5;
    }
    if(K>=4) {
        if(cmp(vdest[4], {0,0}) || cmp(vdest[2], items) || cmp3(items, dest)) answer = 4;
    }
    if(K>=3) {
        if(cmp(vdest[3], {0,0}) || cmp(vdest[1], items)) answer = 3;
    }
    if(K>=2) {
        if(cmp(vdest[2], {0,0}) || cmp(items, dest)) answer = 2;
    }
    if(K>=1) {
        if(cmp(vdest[1], {0,0})) answer = 1;
    }
    print(answer);
}
int32_t main() {
