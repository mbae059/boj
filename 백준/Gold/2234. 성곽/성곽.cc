...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            int nexty = i+1;
            int nextx = j;
            if(inRange(nexty, nextx) && room[i][j]!=room[nexty][nextx]) {
                answer = max(answer, mp[room[i][j]] + mp[room[nexty][nextx]]);
            }
            nexty = i;
            nextx = j+1;
            if(inRange(nexty, nextx) && room[i][j]!=room[nexty][nextx]) {
    ...
    int bigRoom = 0;
    for(auto p : mp) {
        bigRoom = max(bigRoom, p.second);
    }
    cout << idx-1 << " " << bigRoom << " " << answer;
}
int main() {
