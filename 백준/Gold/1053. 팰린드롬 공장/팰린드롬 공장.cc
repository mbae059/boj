...
int go(int l, int r, string& str) {
    if(l>=r) return 0;
    int& ret = dp[l][r];
    if(ret!=-1) return ret;
    return ret = min({go(l+1,r, str)+1, go(l,r-1,str)+1, go(l+1, r-1,str) + (str[l]!=str[r])});
}
void Solve() {
    string str; cin >> str;
    memset(dp,-1,sizeof(dp));
    N = str.size();
    int answer = go(0,N-1,str);
    ...
    for(int i=0;i<N;i++) {
        for(int j=i+1;j<N;j++){
            ...
            auto next = str;
            swap(next[i],next[j]);
            answer = min(answer, go(0,N-1,next)+1);
        }
    }
    cout << answer;
}
