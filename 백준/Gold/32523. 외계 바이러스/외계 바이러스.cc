...
void Solve() {
	...
	rep(i,1,N) {
		rep(j,1,M) cin >> matrix[i][j];
	}

	vector<vector<int>> prefix1(N+2, vector<int>(M+2));
	vector<vector<int>> prefix2(N+2, vector<int>(M+2));
	vector<vector<int>> prefix3(N+2, vector<int>(M+2));
	vector<vector<int>> prefix4(N+2, vector<int>(M+2));

	rep(j,1,M) {
		rep(i,1,N) {
			if(matrix[i][j]=='0') continue;
			prefix1[i][j] = prefix1[i-1][j] + matrix[i][j]-'0';
		}
	}
	...
	rep(i,1,N) {
		rep(j,1,M) {
			if(matrix[i][j]=='0') continue;
			prefix2[i][j] = prefix2[i][j-1] + matrix[i][j] - '0';
		}
	}
	...
	rep(i,1,N) {
		rep(j,1,M) {
			if(matrix[i][j]=='0') continue;
			prefix3[i][j] = prefix3[i-1][j+1] + matrix[i][j] - '0';
			prefix4[i][j] = prefix4[i-1][j-1] + matrix[i][j] - '0';
		}
	}
	...
	rep(i,1,N) {
		rep(j,1,M) {
			for(int k=max(2ll, answer);i+k-1<=N && j+k-1<=M;k++) {
				int a = prefix2[i][j+k-1] - prefix2[i][j];
				int b = prefix1[i+k-1][j] - prefix1[i][j];
				int c = prefix2[i+k-1][j+k-1] - prefix2[i+k-1][j];
				int d = prefix1[i+k-1][j+k-1] - prefix1[i][j+k-1];
				int e = prefix4[i+k-1][j+k-1] - prefix4[i][j];
				int f = prefix3[i+k-1][j] - prefix3[i][j+k-1];

				if(matrix[i][j]=='1' && a==k-1 && b==k-1 && f==k-1) chmax(answer, k);
				if(matrix[i][j]=='1' && a==k-1 && d==k-1 && e==k-1) chmax(answer, k);
				if(matrix[i][j]=='1' && b==k-1 && e==k-1 && c==k-1) chmax(answer, k);
				if(matrix[i][j+k-1]=='1' && f==k-1 && d==k-1 && c==k-1) chmax(answer, k);
			}
		}
	}
	print(answer);
