...
bool inRange(int x, int y) {
    bool inRangeGreen = (x<=9 && y<=3);
    bool inRangeBlue = (x<=3 && y<=9);

    return (inRangeGreen || inRangeBlue);
}
void goGreen(vpii& v) {
    int i = 0;
    [&] () {
...
void lineClearGreen() {
    for(int i=6;i<=9;i++) {
        [&] () {
            ...
            //the row is all 1
            //clear line and Increment Point
            Point++;
            EraseGreenLine(i);
        } ();
    }
...
void EraseBlueLine(int y) {
    for(int j=0;y-j>=4;j++) {
        for(int i=0;i<4;i++) {
            int curx = i;
            int cury = y-j;

            board[curx][cury] = board[curx][cury-1];
        }
    }
}
void lineClearBlue() {
