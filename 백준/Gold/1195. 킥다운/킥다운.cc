...
int main() {
	...
	for (int i = 0; i + str1.size() < str2.size(); i++) {
		for (int j = 0; j < str1.size(); j++) {
			auto idx2 = i + j;
			auto idx1 = j;
			//cout << str2[idx2] << "  " << str1[idx1] << endl;
			if (str2[idx2] == '2' && str1[idx1] == '2') break;
			else if (j == str1.size() - 1) {
				if(i < str1.size()) {
					mm = min(mm, int(str2size + str1.size() - i));
				}
				else if (str2size + str1.size() - 1 < i + str1.size()-1) {
					mm = min(mm, int(i + str1.size() - str1.size()));
				}
				else {
					mm = str2size;
					break;
