...
int length = 1e9;
void cmp(const string& str1, const string& str2) {
    rep(s,-M+1,-1) {
        [&] () {
            for(int i=0;i<s+M;i++) {
                if(str1[i]==str2[i-s] && str1[i]=='2') return;    
            }

            length = min(length, -s+N);
        } ();
    }
    rep(s, 0, N-1) {
        [&] () {
            for(int i=s;i<min(N,s+M);i++) {
                if(str1[i]==str2[i-s] && str1[i]=='2') return;
            }
            if(s+M<=N) length = N;
            else length = min(length, M+s);
        } ();
    }
}

void tcmp(const string& str1, const string& str2) {
    rep(s,-M+1,-1) {
        [&] () {
            for(int i=0;i<s+M;i++) {
    ...
    rep(s, 0, N-1) {
        [&] () {
            ...
            if(s+M<=N) cout << "LEN : " << N << endl;
            else cout << "LEN : " << M+s << endl;
            cout << endl;
        } ();
    }
}
