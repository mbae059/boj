...
pii a[6] {}; //second, damage

int answer = 1e9;

void dfs(int second, int HP, int damage, vpii v) {
    HP -= damage;
    ...
    rep(i,1,N) {
        if(v[i].first<=0) {
            flag = 1;
            vpii ttv = tv;
            ttv[i].first = a[i].first-1;
            dfs(second+1, HP, v[i].second, ttv);
        }
    }
    if(flag==0) dfs(second+1, HP, 0, tv);
}
void Solve() {
    int HP;
    cin >> N >> HP;
