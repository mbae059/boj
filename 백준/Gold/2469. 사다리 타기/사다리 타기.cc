...
char matrix[1111][33] {};
string str;
bool dfs(int curIdx, int curRow, char c) {
    if(curRow==M+1 && str[curIdx]==c) return true;
    else if(curRow==M+1) return false;

    if(curRow==K) {
        if(curIdx==0) {
            if(matrix[curRow][1]!='?') return true;

            if(dfs(curIdx, curRow+1, c)) {
                matrix[curRow][curIdx+1] = '*';
                return true;
            }
            if(dfs(curIdx+1, curRow+1, c)) {
                matrix[curRow][curIdx+1] = '-';
                return true;
            }
            return false;
            ...
            if(matrix[curRow][N-1]!='?') return true;
            if(dfs(curIdx-1, curRow+1, c)) {
                matrix[curRow][curIdx] = '-';
                matrix[curRow][curIdx+1] = '*';
                return true;
            }
            if(dfs(curIdx, curRow+1, c)) {
        ...
        if(matrix[curRow][curIdx]=='?' && matrix[curRow][curIdx+1]=='?') {
            if(dfs(curIdx-1, curRow+1, c)) {
                matrix[curRow][curIdx] = '-';
                matrix[curRow][curIdx+1] = '*';
                return true;
            ...
            char c1 = matrix[curRow][curIdx];
            char c2 = matrix[curRow][curIdx+1];
    ...
    if(curIdx-1>=0 && matrix[curRow][curIdx]=='-') {
        return dfs(curIdx-1, curRow+1, c);
    }
    if(curIdx+1<N && matrix[curRow][curIdx+1]=='-') {
        return dfs(curIdx+1, curRow+1, c);
    }
    return dfs(curIdx, curRow+1, c);
}

void examine(int curIdx, int curRow, char c) {
    if(curRow==M+1 && str[curIdx]==c) return;
    else if(curRow==M+1) {
        rep(i,1,N-1) cout << 'x';
        exit(0);
    }
    ...
    if(curIdx-1>=0 && matrix[curRow][curIdx]=='-') {
        return examine(curIdx-1, curRow+1, c);
    }
    if(curIdx+1<N && matrix[curRow][curIdx+1]=='-') {
        return examine(curIdx+1, curRow+1, c);
    }
    return examine(curIdx, curRow+1, c);
}
void Solve() {
    cin >> N >> M;
    cin >> str;
