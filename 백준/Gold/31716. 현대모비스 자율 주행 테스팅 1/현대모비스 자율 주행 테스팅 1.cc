...
void Solve() {
    cin >> M >> K;
    N = 2;
    vector<vector<char>> matrix(3, vector<char>(M+2));
    rep(i,1,2) {
        rep(j,1,M) cin >> matrix[i][j];
    }
    //
    if(N==1) {
        if(matrix[1][1]=='.' or matrix[2][1]=='.') print(K);
        else print(-1);
        return;
    }
    if(K!=1) {
        if((matrix[1][1]=='#' and matrix[2][M]=='#') or (matrix[2][1]=='#' and matrix[1][M]=='#')) {
            print(-1);
            return;
        }
    }
    //
    auto bfs = [&] (pii p) -> vvi {
        vvi dp(2+1, vi(M+2, 1e18));
        if(matrix[p.first][p.second]=='#') return dp;
        qpii q;
    ...
    // print(dp2);
    int answer=1e18;
    if(matrix[1][1]=='.' and matrix[1][M]=='.' and dp1[1][M]!=1e18) chmin(answer, dp1[1][M] * K + K-1);
    if(matrix[1][1]=='.' and matrix[2][M]=='.' and dp1[2][M]!=1e18) chmin(answer, dp1[2][M] * K + 2*(K-1));
    if(matrix[2][1]=='.' and matrix[1][M]=='.' and dp2[1][M]!=1e18) chmin(answer, dp2[1][M] * K + 2*(K-1));
    if(matrix[2][1]=='.' and matrix[2][M]=='.' and dp2[2][M]!=1e18) chmin(answer, dp2[2][M] * K + K-1);
    if(answer>=1e18) print(-1);
    else print(answer);
}
int32_t main() {
