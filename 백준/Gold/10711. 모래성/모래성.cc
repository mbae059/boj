...
void Solve() {
    ...
    while(!q.empty()) { //bfs
        ...
        for(int i=0;i<8;i++) {
            ...
            if(sand[nextx][nexty]=='.') continue;
            dp[nextx][nexty]+=1;

            lazy.push({nextx, nexty}); //examine later because this point has been effected by current point
        }
        wave = cnt;
        if(q.empty()) {
            while(!lazy.empty()) {
                int x = lazy.front().first;
                int y = lazy.front().second;
                lazy.pop();
                if(sand[x][y]=='.') continue;
                if(visited[x][y]) continue;

                if(dp[x][y] >= sand[x][y]-'0') {
                    visited[x][y]=1;
                    q.push(Castle(x,y,wave+1));
                }
            }
        }
