...
void Solve() {
    ...
    auto getPoints = [&](int lamp) -> int {
        int points = 0;
        rep(i,1,N) {
            rep(j,1,M) {
                if(visited[i][j]!=0 && distance[i][j]<=lamp) {
                    points += matrix[i][j];
                }
            }
        }
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            if(distance[i][j]!=-1) mp[distance[i][j]] += matrix[i][j];
        }
    }
