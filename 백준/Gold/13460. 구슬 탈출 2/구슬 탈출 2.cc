...
struct State {
	int ry, rx, by, bx;
	int cnt;
	State(int ry, int rx, int by, int bx, int cnt) : ry(ry), rx(rx), by(by), bx(bx), cnt(cnt) {}
	void print() {
...
void Solve() {
	...
	queue<State> qs;
	qs.push(State(R.first, R.second, B.first, B.second, 0));
	while(!qs.empty()) {
		int ry = qs.front().ry;
		int rx = qs.front().rx;
		int by = qs.front().by;
		int bx = qs.front().bx;
		int cnt = qs.front().cnt;
		qs.pop();
		...
		rep(i,0,3) {
			[&]() {
				...
				while(matrix[nextry+dir[i][0]][nextrx+dir[i][1]]!='#' && !(nextry+dir[i][0]==nextby && nextrx+dir[i][1]==nextbx)) {
					nextry += dir[i][0];
					nextrx += dir[i][1];
					if(matrix[nextry][nextrx]=='O') {
						rflag = 1;
						nextry = -1;
						nextrx = -1;
						break;
				...
				while(matrix[nextby+dir[i][0]][nextbx+dir[i][1]]!='#' && !(nextby+dir[i][0]==nextry && nextbx+dir[i][1]==nextrx)) {
					nextby += dir[i][0];
					nextbx += dir[i][1];
					if(matrix[nextby][nextbx]=='O') {
						bflag = 1;
					}
				}

				while(inRange(nextry, nextrx) && matrix[nextry+dir[i][0]][nextrx+dir[i][1]]!='#' && !(nextry+dir[i][0]==nextby && nextrx+dir[i][1]==nextbx)) {
					nextry += dir[i][0];
					nextrx += dir[i][1];
					if(matrix[nextry][nextrx]=='O') {
						rflag = 1;
				...
				if(bflag==0 && !(ry==nextry && rx==nextrx && by==nextby && bx==nextbx) && cnt+1<10) qs.push(State(nextry, nextrx, nextby, nextbx, cnt+1));
			} ();
		}
	}
	cout << -1;
