...
void Solve() {
    cin >> A >> B;
    vvi dp(1'000'000+1, vi(3, 1e9));

    dp[A][0] = 0;
    dp[A][1] = 0;
    for(int i=A;i<=B;i++) {
        //chance 안 썼을 때
        if(i+1<=B) chmin(dp[i+1][0], dp[i][0] + 1);
        if(i*2<=B) chmin(dp[i*2][0], dp[i][0] + 1);
        
        if(i+1<=B) chmin(dp[i+1][1], dp[i][1] + 1);
        if(i*2<=B) chmin(dp[i*2][1], dp[i][1] + 1);
        
        if(i*10<=B) chmin(dp[i*10][1], dp[i][0] + 1);
    }
