...
void Solve() {
    ...
    auto getVolume = [&](double height) -> double {
        double volume = 0;
        for(auto [b,h,w,d] : v) {
            if(height<=b) continue;
            volume += min((double)h,height-b)*w*d;
        }
        return volume;
    };
    ...
    while(l<=r) {
        double mid = (l+r)/2;
        double volume = getVolume(mid);

        if(abs(mid-pre)<=0.000000001) break;
        pre = mid;
        if(volume>=V) r = mid;
        else l = mid;
    }
    cout << fixed << setprecision(2) << pre << endl;
    // vector<pair<double,double>> volumeV(1000);
    // pre-=0.5;
    // for(int i=0;i<1000;i++) {
    //     double volume = getVolume(pre);
    //     volumeV[i] = {volume,pre};
    //     pre+=0.001;
    // }
    // for(int i=1;i<1000;i++) {
    //     auto [v1,height1] = volumeV[i-1];
    //     auto [v2,height2] = volumeV[i];
    ...
    //     if(dif1<dif2) {
    //         cout << fixed << setprecision(2) << height1 << endl;
    //         break;
    //     }
    // }
}
