...
void Solve() {
    ...
    while(M--) {
        int a, b, c; cin >> a >> b >> c;
        if(c==0) v.pbk({a,b});
        else if(c==1) points[a]++;
        else points[b]++;
    }

    int answer=0;
    auto dfs = [&](auto&& self, int cur, int point) -> void {
        if(cur==v.size()) {
            if(point < points[K]) answer++;
            return;
        }
        ...
        points[a]++;
        if(points[a] > point && a!=K) self(self, cur+1, points[a]);
        else self(self, cur+1, point);
        points[a]--;

        points[b]++;
        if(points[b] > point && b!=K) self(self, cur+1, points[b]);
        else self(self, cur+1, point);
        points[b]--;
    };
