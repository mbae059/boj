...
const double epilson = 1e-7;
void Solve() {
    cin >> N;
    pair<double,double> xq[N+1] {};

    rep(i,1,N) cin >> xq[i].first;
    rep(i,1,N) cin >> xq[i].second;
    ...
    while(r-l>epilson) {
        ...
        double F=0;
        rep(i,1,N) {
            double r = mid - xq[i].first;
            auto val = xq[i].second / (r*r);
            if(mid-xq[i].first>0) F += val;
            else F -= val;
        }
