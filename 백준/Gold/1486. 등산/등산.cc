...
int DijkstraToHotel(int y, int x) {
	rep(i,1,N) {
		rep(j,1,M) dp[i][j] = INT_MAX;
	}
	...
	while(!pq.empty()) {
		...
		rep(i,0,3) {
			...
			int dif = abs(matrix[ny][nx]-matrix[y][x]);
			if(dif>T) continue;
			int nd;
			if(matrix[ny][nx]>matrix[y][x]) nd = d+(matrix[ny][nx]-matrix[y][x]) * (matrix[ny][nx]-matrix[y][x]);
			else nd = d+1;
			if(nd>D) continue;
			if(dp[ny][nx]>nd) {
				dp[ny][nx] = nd;
