...
void dfs(int idx, int cnt) { //implement with dfs(1, 0). N and K must be global variable
	if(cnt==K) {
		for(int i=1;i<7;i++) {
			if(chk(arr[combination[i-1]], arr[combination[i]], arr[combination[i+1]])) return;
		}
		if(chk(arr[combination[7]], arr[combination[0]], arr[combination[1]])) return;
		if(chk(arr[combination[6]], arr[combination[7]], arr[combination[0]])) return;
		answer++;
		return;
	}
	for(int i=idx;i<N;i++) { //idx, N을 잘볼것
		...
		dfs(0, cnt+1); //be careful with i+1, cnt+1
		visited[i]=0;
