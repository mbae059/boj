...
void Solve() {
    ...
    auto doExamine = [&getSum, &cmp](vpii v, pii p) {
        sort(all(v));
        int n = v.size();
        if(n<=2) return 0;
        int a = (n+2)/2;
        ...
        vpii answer(n);
        for(int i=0;i<a;i++) {
            answer[i].first = (v[i].first+1)/2-1;
            answer[i].second = v[i].second;
        }
        for(int i=a;i<n;i++) {
            answer[i] = v[i];
        }
        int sum1 = getSum(v);
        int sum2 = getSum(answer);

        if(sum2>sum1-sum2) {
            print("YES");
            if(p.first) {
                answer.pbk({p.first/2, p.second});
                v.pbk(p);
            }
            sort(all(answer), cmp);
            sort(all(v), cmp);
