...
#define MAX 100003
vpii edge[MAX];
pii diameterA, diameterB; //node, dis
bool visited[MAX] {};
pii bfs(int x) {
...
int getSecondDiameter(int x, int exclude) {
    memset(visited, 0, sizeof(visited));

    visited[x] = 1;
    visited[exclude]=1; //don't go to another diameter node
    queue<pii> q;
    q.push({x,0});
    ...
    while(!q.empty()) {
        ...
        secondDiamter = max(secondDiamter, dis);
        for(auto next : edge[cur]) {
            int nextNode = next.first;
            int nextDis = dis + next.second;
...
void Solve() {
    ...
    diameterA = bfs(1);
    diameterB = bfs(diameterA.first);

    int secondDiameterA = getSecondDiameter(diameterA.first, diameterB.first);
    int secondDiameterB = getSecondDiameter(diameterB.first, diameterA.first);

    cout << max(secondDiameterA, secondDiameterB);
}
int main() {
