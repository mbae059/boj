...
vpdd v;
struct cmp { //pii
    bool operator() (const pair<int, double>& i, const pair<int, double>& j) {
...
void Dijkstra() {
    p_q<pair<int, double>, vector<pair<int,double>>, cmp> pq; //cur, dis
    for(int i=1;i<N;i++) {
        auto dist = getDist(v[0], v[i]);
        double t = dist/5;
        d[i] = t;
        pq.push({i, t});
    }
    ...
    while (!pq.empty()) {
        ...
        for(int i=1;i<N;i++) {
            auto dist = getDist(v[cur], v[i]);
            double t = min(dist/5, abs(dist-50)/5+2) + dis;
            if(d[i]>t) {
                d[i] = t;
                pq.push({i, t});
            }
