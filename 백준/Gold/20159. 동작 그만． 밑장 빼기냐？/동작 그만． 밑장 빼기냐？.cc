...
void Solve() {
    ...
    int prefix[2][MAX] {}; //[0] is even idx, [1] is odd idx
    rep(i, 1, N/2) {
        if(i*2<N) prefix[0][i] = prefix[0][i-1] + v[i*2];
        prefix[1][i] = prefix[1][i-1] + v[i*2-1];
    }
    int fastone[MAX] {};
    rep(i,1,N) {
        if(i%2) {
            fastone[i] = v[N] + prefix[0][(N-1)/2] - prefix[0][(i-1)/2] + prefix[1][(i-1)/2];
            // cout << v[N] << " " << prefix[0][(N-1)/2] << " " << prefix[0][(i-1)/2] << " " << prefix[1][(i-1)/2] << endl;
        }
        else {
            fastone[i] = prefix[0][(N-1)/2] - prefix[0][i/2-1] + prefix[1][i/2];
        }
        // cout << fastone[i] << endl;
    }
    cout << *max_element(fastone+1, fastone+N+1);
}
int main() {
