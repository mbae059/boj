...
int dir4[5][2] = {{0,0}, {0,1},{0,-1}, {1,0}, {-1,0}};
...
struct Node {
	int y, x, dir;
	Node(int y, int x, int dir) : y(y), x(x), dir(dir) {}
	Node() {
		Node(0,0,0);
	}
};
...
void Solve() {
	...
	while(!q.empty()) {
		...
		pii nextDir = turn(dir);

		if(d[y][x][nextDir.first]==-1) {
			d[y][x][nextDir.first] = d[y][x][dir] + 1;
			q.push(Node(y,x,nextDir.first));
		}
		
		if(d[y][x][nextDir.second]==-1) {
			d[y][x][nextDir.second] = d[y][x][dir] + 1;
			q.push(Node(y,x,nextDir.second));
		}

		rep(k,1,3) {
			int nexty = y + dir4[dir][0] * k;
			int nextx = x + dir4[dir][1] * k;
			if(!inRange(nexty, nextx)) continue;
			if(matrix[nexty][nextx]) break;
			if(d[nexty][nextx][dir]!=-1) continue;
			d[nexty][nextx][dir] = d[y][x][dir] + 1;
			q.push(Node(nexty, nextx, dir));
		}
	}
	cout << d[dest.y][dest.x][dest.dir];
