...
void Solve() {
    ...
    rep(j,1,N) {
        ...
        if(dp[0][j-1]>0) chmax(dp[0][j], dp[0][j-1] + a[j]);
        
        rep(i,1,K) {
            if(dp[i][j-1]>0) chmax(dp[i][j], dp[i][j-1] + a[j]);
            if(dp[i-1][j-1]>0) chmax(dp[i][j], dp[i-1][j-1] * 2);
        }
    }
    ...
    if(answer>1e11) print("MEGA");
    else if(answer<=0) print(-1);
    else print(answer);
