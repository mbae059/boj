...
void Solve() {
    ...
    //일단 0~M까지 고객을 스캔 후 need안에 넣음
    for(int i=0;i<M;i++) {
        if(a[i]) need.push(i);
    }

    rep(i,0,1'000'000) {
        //i+M+1 이후에 고객이 있다면 지금부터 커피를 넣을 수 있기 때문에 need안에 넣음
        if(i+M<=1'000'000 && a[i+M]) {
            need.push(i+M+1);
        }
        //고객이 있다면 반드시 서빙. ready되어있는 커피가 없다면 나가리
        if(a[i]) {
            if(ready) ready--;
            else fail();
        }
        //need가 비어있지 않고 토기 또한 준비되어 있다면 ready된 커피를 만듦
        else if(!need.empty() && plate>0) {
            if(need.front()>i) {
                ready++;
                need.pop();
                plate--;
