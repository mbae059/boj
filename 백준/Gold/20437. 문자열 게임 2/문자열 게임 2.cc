...
void LIS(vi& v) { //vector v's size is N
    ...
    rep(i,0,N-1) { // N
        ...
        if(iter!=lis.end()) {
            *iter = cur; //if found, erase that value. if not, cur is the highest value of lis
...
void Solve() {
    ...
    for(int i=0;i<str.size();i++) {
        int alphaIdx = str.at(i) - 'a';

        alpha[alphaIdx].pbk(i);
    }

    int mn = INF;
    int mx = 0;
    for(int i=0;i<26;i++) {
        for(int j=0;j+N-1<alpha[i].size();j++) {
            
            int front = alpha[i][j];
            int last = alpha[i][j+N-1];

            mn = min(mn, last-front+1);
            mx = max(mx, last-front+1);
        }
    }

    if(mn==INF && mx==0) cout << -1 << endl;
