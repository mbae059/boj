...
struct Node {
    ...
    Node(int y, int x, int gPass, int gAdjacent) : y(y), x(x), gPass(gPass), gAdjacent(gAdjacent) {}
};
struct cmp { //pii
    bool operator() (const Node& i, const Node& j) {
        if (g[i.y][i.x].first == g[j.y][j.x].first) {
            return g[i.y][i.x].second > g[j.y][j.x].second;
        }
        return g[i.y][i.x].first > g[j.y][j.x].first;
    }
};

bool inRange(int y, int x) {
    return 1<=y && y<=N && x>=1 && x<=M;
}
void Solve() {
    cin >> N >> M;
    
    p_q<Node, vector<Node>, cmp> pq;
    pii E;
    ...
    while(!pq.empty()) {
        int y = pq.top().y;
        int x = pq.top().x;
        int gPass = pq.top().gPass;
        int gAdjacent = pq.top().gAdjacent;
        pq.pop();

        if(g[y][x].first < gPass) continue;
        if(g[y][x].first == gPass && g[y][x].second < gAdjacent) continue;
        ...
        rep(i,0,3) {
            ...
            int nextgPass = gPass;
            int nextgAdjacent = gAdjacent;
            if(matrix[nexty][nextx]=='F') {
            }
            else if(matrix[nexty][nextx]=='g') {
                nextgPass++;
            }
            else {
                rep(j,0,3) {
                    int nextAdjacenty = nexty+dir[j][0];
                    int nextAdjacentx = nextx+dir[j][1];

                    if(!inRange(nextAdjacenty, nextAdjacentx)) continue;
                    if(matrix[nextAdjacenty][nextAdjacentx]=='g') {
                        nextgAdjacent++;
                        break;
                    }
                }
            }
            if(g[nexty][nextx].first > nextgPass) {
                g[nexty][nextx] = {nextgPass, nextgAdjacent};
                pq.push(Node(nexty,nextx, nextgPass, nextgAdjacent));
            }
            else if(g[nexty][nextx].first==nextgPass & g[nexty][nextx].second > nextgAdjacent) {
                g[nexty][nextx] = {nextgPass, nextgAdjacent};
                pq.push(Node(nexty,nextx, nextgPass, nextgAdjacent));
            }
        }
