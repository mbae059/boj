...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        while(!tpq.empty() && !pq.empty() && tpq.top() == pq.top()) {
            tpq.pop();
            pq.pop();
        }
        ...
        if(cur>1e9) no();
        if(i==N) break;
        if(pq.top()%answer.front()!=0) no();
        cur = pq.top() / answer.front();
    }
