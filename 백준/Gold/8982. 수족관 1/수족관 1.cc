...
void markDepth(int idx) {
    
    int left = idx-2;
    int right = idx+2;
    int markSink = tub[v[idx].first];

    for(int i=v[idx].first;i<v[idx+1].first;i++) {
        sink[i] = max(sink[i], markSink);
    }

    while(left>=0) {
        int start = v[left].first;
        int end = v[left+1].first;

        int height = v[left].second;
    ...
    markSink = tub[v[idx].first];
    while(right<N-1) {
        int start = v[right].first;
        int end = v[right+1].first;
        
        int height = v[right].second;
...
void Solve() {
    ...
    int holeIdx = 0;
    sort(all(holes));
    for(int i=1;i<N;i+=2) { //v index
        pii curV = v[i];
        Hole curHole = holes[holeIdx];
        pii curLeftHole = make_pair(curHole.a, curHole.b);
        pii curRightHole = make_pair(curHole.c, curHole.d);
