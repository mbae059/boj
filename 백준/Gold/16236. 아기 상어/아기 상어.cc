...
struct Shark {
    ...
    int size;
    int energy;
    Shark(int x=0, int y=0, int size=0) : x(x), y(y), size(size) {
        energy = 0;
    }
...
bool isFish(int x, int y){
    return 1<=matrix[x][y] && matrix[x][y]<=6;
}
void Solve() {
    cin >> N;

    int fish_cnt = 0; //count fish
    ...
    while(fish_cnt--) {
        ...
        while(!q.empty()) {
            ...
            if(isFish(x,y) && shark.size > matrix[x][y]) {
                if(minDis != INF && minDis < dis) break;
                minDis = dis;
                v.pbk({x,y});
            }
            for(int i=0;i<4;i++) {
                ...
                if(matrix[nextx][nexty] > shark.size) continue;
                visited[nextx][nexty] = 1;
                q.push(Node(nextx, nexty, dis+1));
            }
        }
        if(v.empty()) {
            break;
        ...
        int eatFishX = v.front().first;
        int eatFishY = v.front().second;

        matrix[shark.x][shark.y] = 0;
        shark.Update(eatFishX, eatFishY);
        matrix[eatFishX][eatFishY] = 0;
        cnt+=minDis;
        shark.Increment();
    }
    cout << cnt;
