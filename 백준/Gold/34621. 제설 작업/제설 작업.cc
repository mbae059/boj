...
struct MM {
    ...
    MM(bool _r, int _idx, int _value) {
        r = _r;
        idx = _idx;
        value = _value;
    }
...
void Solve() {
    ...
    while(cnt--) {
        MM mm = MM();
        rep(k, 1, N+M) {
            if(prefix[k]-subtract[k]==0) continue;
            if(mm.value > prefix[k]- subtract[k]) {
                if(k<=N) mm = MM(1, k, prefix[k]-subtract[k]);
                else mm = MM(0, k-N, prefix[k]-subtract[k]);
            }
        }
        if(mm.r==1) {
            subtract[mm.idx] = prefix[mm.idx];
            rep(j,1,M) {
                subtract[j+N] += matrix[mm.idx][j];
                matrix[mm.idx][j] = 0;
            }
        }
        else {
            subtract[N+mm.idx] = prefix[N+mm.idx];
            rep(i,1,N) {
                subtract[i] += matrix[i][mm.idx];
                matrix[i][mm.idx] = 0;
            }
        }
        chmax(answer, mm.value);
    }
    print(answer);
}
int32_t main() {
