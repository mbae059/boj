...
int answer[51][51] {};
int spaceVariable = 0;
void inRange(int y, int x) {
    if(r1<=y && y<=r2 && c1<=x && x<=c2) {
        answer[y-r1][x-c1] = value;
        spaceVariable = max(spaceVariable, value);
    }
}
void Solve() {
    cin >> r1 >> c1 >> r2 >> c2;
    int mx = max({abs(r1), abs(c1), abs(r2), abs(c2)});
    ...
    int space = 0;
    while(spaceVariable!=0) {
        space++;
        spaceVariable/=10;
    }
    rep(i,0,r2-r1) {
        rep(j,0,c2-c1) {
            cout << setw(space) << answer[i][j] << " ";
        }
        cout << endl;
    }
}
