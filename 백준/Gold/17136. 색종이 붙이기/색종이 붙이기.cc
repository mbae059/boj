...
void dfs(int y, int x) {
    ...
    for(int k=dp[y][x];k>=1;k--) {
        if(resource[k]==0) continue;
        resource[k] -= 1;
        bool flag = 0;
        rep(i,y-k+1, y) {
            rep(j,x-k+1,x) {
                if(visited[i][j]) flag= 1;
            }
        }
        if(flag==0) {
            rep(i,y-k+1, y) {
...
void Solve() {
    ...
    rep(i,1,10) {
        rep(j,1,10) {
            if(matrix[i][j]) dp[i][j] = min(min({dp[i-1][j], dp[i][j-1], dp[i-1][j-1]}) + 1, 5);
        }
    }
    dfs(10,10);
