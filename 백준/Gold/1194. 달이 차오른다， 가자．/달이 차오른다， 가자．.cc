...
void Bfs(pii me) {
    ...
    q.push({me,{0, 0}});
    visited[me.first][me.second][0]=1;
    ...
    while(!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            ...
            char path = graph[ny][nx];

            if(path=='#') continue;

            if(path >='a' && path <='f') nkey |= (1<< (path-'a'));
            else if(path >='A' && path <= 'F') {
                if(!(nkey & (1<< (path-'a')))) continue;
            }
            else if(path=='1') {
                cout << step+1;
                return;
