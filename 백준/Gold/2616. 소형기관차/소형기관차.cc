...
int dfs(int idx, int cnt) {
    ...
    if(idx+K-1<=N) {
        dp[idx][cnt] = max(dfs(idx+1,cnt), dfs(idx+K, cnt+1) + passenger[idx+K-1]-passenger[idx-1]);
    }
    return dp[idx][cnt];
}
void Solve() {
    cin >> N;
    rep(i,1,N) cin >> passenger[i];
    rep(i,1,N) {
        passenger[i] = passenger[i] + passenger[i-1];
    }
