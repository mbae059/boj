...
void Solve() {
    ...
    happyYoung[0] = INT_MAX;
    rep(i,1,N) {
        if(happy[i]==0) {
            happyYoung[i] = happyYoung[i-1];
        }
        else {
            happyYoung[i] = min(happyYoung[i-1], happy[i]);
        }
    }
    for(int i=N;i>=1;i--) {
        if(happy[i]==0) {
            happyOld[i] = happyOld[i+1];
        }
        else {
            happyOld[i] = max(happyOld[i+1], happy[i]);
        }
    }

    vi v1, v2;
    rep(i,1,N-1) {
        if(happyYoung[i]>happyOld[i+1]) {
            v1.pbk(i);
        }
    }

    rep(i,1,N) {
        fatigueYoung[i] = max(fatigueYoung[i-1], fatigue[i]);
    }

    fatigueOld[N+1] = INT_MAX;
    for(int i=N;i>=1;i--) {
        if(fatigue[i]==0) {
            fatigueOld[i] = fatigueOld[i+1];
        }
        else {
            fatigueOld[i] = min(fatigueOld[i+1], fatigue[i]);
        }
    }

    rep(i,1,N-1) {
        if(fatigueYoung[i]<fatigueOld[i+1]) {
            v2.pbk(i);
        }
    }
    ...
    for(int i=v1.size()-1;i>=0;i--) {
        ...
        if(*lower_bound(all(v2), value)==value) {
            cout << value;
            return;
        }
    }
