...
void Solve() {
    ...
    vector<p_q<int, vi, greater<int>>> pq(6);
    rep(i,1,N) {
        int t, a;
        cin >> t >> a;
        pq[a].push(t);
    ...
    auto getIdx = [&](int current_people) -> int {
        int mn = 1e9;
        int ret = -1;
        for(int i=1;i<=K-current_people;i++) {
            if(!pq[i].empty() && pq[i].top()<=t && pq[i].top()<mn) {
                chmin(mn, pq[i].top());
                ret = i;
            }
        }
    ...
    while(!pq[1].empty() || !pq[2].empty() || !pq[3].empty() || !pq[4].empty() || !pq[5].empty()) {
        int current_people = 0;
        
        while(true) {
            int idx = getIdx(current_people);
            if(idx==-1) break;
            current_people += idx;
            answer += t - pq[idx].top();
            pq[idx].pop();
