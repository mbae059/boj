...
int getVal(int a, int b, int c, int d) {
    return a+b*5+c*10+d*25;
}

void chmax(int& a, int& b, int& c, int& d, tiiii t) {
    auto [a1,b1,c1,d1] = t;
...
void Solve() {
    ...
    rep(j,1,X) {    
        ...
        rep(i,1,4) {
            if(j-t[i]<0) continue;
            if(visited[j-t[i]]==0) continue;
            int pa = dp[1][j-t[i]];
            int pb = dp[2][j-t[i]];
            int pc = dp[3][j-t[i]];
            int pd = dp[4][j-t[i]];
            
            vi temp {0,pa,pb,pc,pd};
            temp[i]++;
            int ca = temp[1];
            int cb = temp[2];
            int cc = temp[3];
        ...
        if(a!=0 || b!=0 || c!=0 || d!=0) visited[j] = 1;
    }
    cout << dp[1][X] << " " << dp[2][X] << " " << dp[3][X] << " " << dp[4][X];
}
int32_t main() {
