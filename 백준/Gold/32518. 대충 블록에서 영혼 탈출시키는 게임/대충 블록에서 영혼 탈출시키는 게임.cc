...
void Solve() {
	...
	auto dfs = [&](auto&& self, int cur) -> int {
		if(cur<=2) return 0;
		if(cur<=4) return 1;
		if(mp.contains(cur)) return mp[cur];
		...
		if(cur%3==2) ret = 3 * self(self, cur/3) + 2;
		else if(cur%3==0) ret = 2 * self(self, cur/3-1) + self(self, cur/3) + 2;
		else ret = max(2 * self(self, cur/3-1) + self(self, cur/3+1), self(self, cur/3-1) + 2 * self(self, cur/3)) + 2;
		return ret;
	};
