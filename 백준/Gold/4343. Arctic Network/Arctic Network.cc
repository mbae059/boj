...
void Solve() {
    ...
    for(int i=0;i<v.size();i++) {
        auto [d,n1,n2] = v[i];
        if(dsu.isSameParent(n1,n2)) continue;
        int p1 = dsu.getParent(n1);
        int p2 = dsu.getParent(n2);

        dsu.merge(n1,n2);
        int p = dsu.getParent(n1);
        
        answer = max(answer, d);

        if(p1==p) connected.erase(p2);
        else connected.erase(p1);
