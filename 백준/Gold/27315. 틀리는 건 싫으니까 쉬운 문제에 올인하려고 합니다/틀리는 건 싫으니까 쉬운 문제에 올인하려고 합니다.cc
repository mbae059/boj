...
void Solve() {
    ...
    vpii v;
    rep(i,1,N) {
        cin >> D >> P >> T >> E; //idea, implementation, data, editoral
        if(T) P = 0;
        if(E) {
            D = (D+1)/2;
            P/=2;
    ...
    int HD, HP; cin >> HD >> HP; //hanbyeol idea, implementation

    sort(all(v));
    p_q<int, vi, greater<>> pq;
