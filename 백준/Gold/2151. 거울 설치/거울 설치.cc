...
void Dijkstra(int num) {
    p_q<pii, vpii, cmp> pq;
    pq.push({ num, 0 });
    fill(d + 1, d + 2555, INF); //INF could be larger, varying from problem to problem
    d[num] = 0;
    while (!pq.empty()) {
...
bool isNode(int y, int x) {
    return matrix[y][x]=='#' || matrix[y][x]=='!';
}
void mark_dist(int y, int x) {
    int src = mark[y][x];

    for(int i=1;x-i>=1;i++) { //left row
        if(isNode(y, x-i)) {
            int tar = mark[y][x-i];
            edge[src].pbk({tar,1});
        }
        if(matrix[y][x-i]=='*') break;
    }
    for(int i=1;x+i<=cnt;i++) { //right row
        if(isNode(y, x+i)) {
            int tar = mark[y][x+i];
            edge[src].pbk({tar,1});
        }
    ...
    for(int i=1;y-i>=1;i++) { //upper column
        if(isNode(y-i, x)) {
            int tar = mark[y-i][x];
            edge[src].pbk({tar,1});
        }
        if(matrix[y-i][x]=='*') break;
    }
