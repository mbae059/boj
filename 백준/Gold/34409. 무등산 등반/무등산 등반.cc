...
void Solve() {
    cin >> N >> M;
    p_q<tiii, vtiii, greater<tiii>> pq;
    cin >> X >> Y;
    pq.push({0, X, Y});
    cin >> A >> B >> C;
    ...
    while(!pq.empty()) {
        ...
        rep(i,0,3) {
            ...
            int dif = matrix[nx][ny]-matrix[x][y];
            if(abs(dif)>C) continue;
            
            int nt = 1e9;
            if(dif>0) nt = dp[x][y]+dif*A;
            else if(dif==0) nt = dp[x][y]+1;
            else nt = dp[x][y]-dif*B;
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            if(answer.first < matrix[i][j]) {
                answer.first = matrix[i][j];
                answer.second = dp[i][j];
            }
        }
    }
    if(answer.second==1e9) cout << -1;
