...
struct Node {
    ...
    Node(int node, int visitedNode, int dis) : node(node), visitedNode(visitedNode), dis(dis) {}
};
Node bfs(int x) {
    bool visited[MAX] {};
    queue<Node> q;
    ...
    while(!q.empty()) {
        ...
        int visitedNode = q.front().visitedNode;
        int dis = q.front().dis;

        if(ret.visitedNode < visitedNode) {
            ret = q.front();
        }
        if(ret.visitedNode == visitedNode && ret.dis > dis) {
            ret = q.front();
        }
        q.pop();
        for(auto next : edge[cur]) {
            int nextNode = next.first;
            int nextVisitedNode = visitedNode + 1;
            int nextDis = dis + next.second;

            if(visited[nextNode]) continue;
            visited[nextNode]=1;
            q.push(Node(nextNode, nextVisitedNode, nextDis));
        }
    }
    return ret;
}
void Solve() {
    ...
    Node end1 = bfs(1);
    Node end2 = bfs(end1.node);

    int totalDis = end2.dis;
    int answer = (totalDis % T == 0 ? totalDis / T : totalDis / T + 1);
    cout << answer;
}
int main() {
    #ifdef BOJ
