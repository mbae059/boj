...
set<vector<int>> mnS;
int mnCost = INF;
void dfs(int idx, int p, int f, int s, int v, int c, vector<int>& saveIdx) {
    // cout << p << " " << f << " " << s << " " << v << " " << c << endl;
    // print(saveIdx);
    if(p >= mnNutrients[1] && f >= mnNutrients[2] && s >= mnNutrients[3] && v >= mnNutrients[4]) {
        if(mnCost > c) {
            mnS.clear();
            mnS.insert(saveIdx);
            mnCost = min(mnCost, c);
