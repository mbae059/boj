...
int T, N, K, M, S, H, W; // S is for MCMF, network flow
...
bool cmp(vector<string>& left, vector<string>& right) {
    ...
    for(int i=0;i<sz;i++) {
        string leftString = left.at(i);
        string rightString = right.at(i);

        if(leftString==rightString) continue;

        if(isNum(leftString.front()) && isNum(rightString.front())) {
            string realLeftStringNum, realRightStringNum;

            rep(j, 0, leftString.size()-1) {
                if(leftString[j]!='0') {
                    realLeftStringNum = leftString.substr(j);
                    break;
                }
            }
            
            rep(j, 0, rightString.size()-1) {
                if(rightString[j]!='0') {
                    realRightStringNum = rightString.substr(j);
                    break;
                }
            }

            if(realLeftStringNum==realRightStringNum) {
                return leftString.size() < rightString.size();
            }
            
            if(realLeftStringNum.size()!=realRightStringNum.size()) return realLeftStringNum.size() < realRightStringNum.size();

            return realLeftStringNum < realRightStringNum;
        }
        if(isNum(leftString.front()) || isNum(rightString.front())) {
            return leftString.front() < rightString.front();
        }

        char leftC = leftString.front();
        char rightC = rightString.front();

        if(toupper(leftC)==toupper(rightC)) {
            return leftC < rightC;
        }
        return toupper(leftC) < toupper(rightC);
...
void Solve() {
    ...
    rep(i,0,N-1) {
        ...
        if(!v_input.empty()) v[i].pbk(v_input);
    }
    sort(v, v+N, cmp);
    print();
}
