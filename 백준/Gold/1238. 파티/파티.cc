...
vpii redge[MAX]; // first is idx, second is weight of edge
int d[MAX] {};
int rd[MAX] {};
...
void RDijkstra(int num) {
    p_q<pii, vpii, cmp> pq;
    pq.push({ num, 0 });
	
    fill(rd + 1, rd + 1 + N, INF); //INF could be larger, varying from problem to problem
    rd[num] = 0;
    while (!pq.empty()) {
        int cur = pq.top().first;
        int dis = pq.top().second;
        pq.pop();
        if(rd[cur] < dis) continue; //could be used if d[cur] has been updated more than once.
        for (auto next : redge[cur]) {
            if(rd[next.first] > rd[cur] + next.second) {
                rd[next.first] = rd[cur]+next.second;
                pq.push({next.first, rd[next.first]});
            }
        }
    }
}
