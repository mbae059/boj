...
bool inRange(int y, int x) {
    return -500 <= y && y <= 500 && -500 <= x && x <= 500;
}

Vec<3, int> dist(1111,1111,4,-1);
void Solve() {
    pii src, dest;
	...
	dist[src.first+500][src.second+500][0] = 0;
	while(!q.empty()) {
		auto [d, y, x] = q.front();
		q.pop();

		vpii v = {{y+1, x}, {2*y,2*x}, {-x,y}};
		for(auto [ny, nx] : v) {
			if(!inRange(ny,nx)) continue;
			int nd = d+1;
			if(dist[ny+500][nx+500][nd%4]==-1) {
				dist[ny+500][nx+500][nd%4] = nd;
				q.push({nd, ny, nx});
			}
		}
	}
	...
	while(K--) {
		...
		int val = dist[dest.first+500][dest.second+500][x%4];
		if(val!=-1 && val<=x) yes();
		else no();
	}
}
