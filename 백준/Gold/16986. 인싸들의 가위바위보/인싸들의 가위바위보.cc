...
bool visited[MAX] {};
//
void dfs(int k, int bidx, int cidx, int awin, int bwin, int cwin) {
    if(awin==K) {
        cout << 1;
        exit(0);
    }
    ...
    if(bidx==21 || cidx==21) return;
    // cout << k << " " << bidx << " " << cidx << " " << awin << " " << bwin << " " << cwin<<endl;
    if(k==2) {
        if(matrix[oppo[1][bidx]][oppo[2][cidx]]==2) {
            dfs(1, bidx+1, cidx+1, awin, bwin+1, cwin);
        }
        else {
            dfs(3, bidx+1, cidx+1, awin, bwin, cwin+1);
        }
        return;
    }
    ...
    rep(i,1,N) {
        ...
        if(matrix[i][oppo[1][bidx]]==2) {
            dfs(3, bidx+1, cidx, awin+1, bwin, cwin);
        }
        else {
            dfs(2, bidx+1, cidx, awin, bwin+1, cwin);
        }
        break;

        case 3:
        if(matrix[i][oppo[2][cidx]]==2) {
            dfs(1, bidx, cidx+1, awin+1, bwin, cwin);
        }
        else {
            dfs(2, bidx, cidx+1, awin, bwin, cwin+1);
        }
        break;
        }
        visited[i]=0;
