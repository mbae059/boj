...
void Solve() {
    ...
    cin >> place[1].first >> place[1].second >> place[2].first >> place[2].second;

    for(int i=3;i<=8;i+=2) {
        cin >> place[i].first >> place[i].second >> place[i+1].first >> place[i+1].second;
        dp[i][i+1] = 10;
        dp[i+1][i] = 10;
    }
    ...
    for(int i=1;i<=8;i++) {
        for(int j=1;j<=8;j++) {
            chmin(dp[i][j], getDist(place[i], place[j]));
        }
    }
