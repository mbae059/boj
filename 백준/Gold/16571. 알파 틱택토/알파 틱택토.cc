...
bool chk(int turn) {
	...
	if(matrix[1][1]==matrix[2][2] && matrix[2][2]==matrix[3][3] && matrix[1][1]==turn) return 1;
	if(matrix[3][1]==matrix[2][2] && matrix[2][2]==matrix[1][3] && matrix[2][2]==turn) return 1;
	...
	rep(i,1,3) {
		rep(j,1,3) {
			if(matrix[i][j]==0) {
				...
				int ret = dfs((turn == 1 ? 2 : 1));
				matrix[i][j] = 0;
				mx = max(mx, ret);
				if(ret==turn) return turn;
			}
		}
	}
