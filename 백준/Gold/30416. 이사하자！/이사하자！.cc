...
*/
int CCW(pii A, pii B, pii C) { //A, B, C is in order
    int ccw = (B.first - A.first) * (C.second - A.second) - (C.first - A.first) * (B.second - A.second); //Cross product
    if(ccw>0) return 1;
...
void Solve() {
    ...
    rep(i,1,N) {
        ...
        sort(all(tv));
        do {
            int flag1 = CCW(tv[0], tv[1], tv[2]);
            int flag2 = CCW(tv[1], tv[2], tv[3]);
            int flag3 = CCW(tv[2], tv[3], tv[0]);
            int flag4 = CCW(tv[3], tv[0], tv[1]);
            if(flag1==1 && flag2==1 && flag3==1 && flag4==1) break;
        } while(next_permutation(all(tv)));
        v.pbk(tv);
    }
    int answer=1;
    for(int i=0;i<N;i++) { //i is the exterior box
        int cnt=0;
        for(int j=0;j<N;j++) { //interior box
            auto tv1 = v[i];
            auto tv2 = v[j];
            [&] () {
                for(int k=0;k<4;k++) {
                    if(!(CCW(tv1[0], tv1[1], tv2[k])>=0 && CCW(tv1[1], tv1[2], tv2[k])>=0 && CCW(tv1[2], tv1[3], tv2[k])>=0 && CCW(tv1[3], tv1[0], tv2[k])>=0)) {
                        return;
                    }
                }
                cnt++;
