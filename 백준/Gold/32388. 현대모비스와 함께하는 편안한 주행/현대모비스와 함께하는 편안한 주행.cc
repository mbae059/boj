...
void Solve() {
    ...
    edge[0] = {0,0};
    edge[N+1] = {A,B};
    auto getDist = [&](int idx1, int idx2) -> double {
        int dx = edge[idx1].first - edge[idx2].first;
        int dy = edge[idx1].second - edge[idx2].second;
        int dist2 = dx * dx + dy * dy;
        if((idx1==0 && idx2==N+1) || (idx1==N+1 && idx2==0)) return sqrt(dist2);
        else if(idx1==0 || idx1==N+1 || idx2==0 || idx2==N+1) return dist2<=1 ? 0 : sqrt(dist2)-1;
        else return dist2<=4 ? 0 : sqrt(dist2)-2;
    };
    rep(i,1,N) {
        cin >> edge[i].first >> edge[i].second;
    }
    p_q<pair<double,int>, vector<pair<double,int>>, greater<>> pq;
    pq.push({0, 0});
    ...
    while(!pq.empty()) {
        ...
        for(int next=1;next<=N+1;next++) {
            double nextDistance = getDist(cur, next) + dist;
