...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            ...
            if(matrix[i][j]=='*') cnt[i][j] = 1;
            else if(matrix[i][j]=='#') cnt[i][j] = 2;
            else cnt[i][j] = 1e9;
        }
    }
    ...
    while(!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            for(int j=1;j<=d;j++) {
                ...
                cnt[ny][nx] -= 1;
                if(cnt[ny][nx]<=0 && (matrix[ny][nx]=='#' || matrix[ny][nx]=='*') && visited[ny][nx]==0) {
                    q.push({ny,nx});
                    visited[ny][nx] = 1;
                }
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            if(visited[i][j] && (matrix[i][j]=='#' || matrix[i][j]=='*')) c++;
            else if(visited[i][j]==0 && (matrix[i][j]=='#' || matrix[i][j]=='*')) nc++;
        }
    }
    cout << c << " " << nc << endl;
}
