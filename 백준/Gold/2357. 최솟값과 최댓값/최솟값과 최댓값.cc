...
pii segment(int node, int nodeLeft, int nodeRight) { // use when s, v is available and segment tree is about sum
    if (nodeLeft == nodeRight) {
        return s[node] = {v[nodeLeft], v[nodeLeft]};
    }
	int mid = (nodeLeft+nodeRight)/2;
	pii l = segment(node * 2, nodeLeft, mid);
	pii r = segment(node * 2 + 1, mid + 1, nodeRight);
    return s[node] = {min(l.first, r.first), max(l.second, r.second)};
}
pii query(int node, int l, int r, int nodeLeft, int nodeRight) { //l and r is the range.
    if (nodeRight < l || r < nodeLeft) {
		return {INT_MAX, -INT_MAX};
	}
    if (l <= nodeLeft && nodeRight <= r) return s[node];
	int mid = (nodeLeft+nodeRight)/2;
	pii lq = query(node * 2, l, r, nodeLeft, mid);
	pii rq = query(node * 2 + 1, l, r, mid + 1, nodeRight);
