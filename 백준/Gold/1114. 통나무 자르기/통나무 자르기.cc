...
bool examine(ll x) {
    ...
    ll tempFirstCut = 0, tempLongest = 0;
    for(ll i=0;i<cutTree.size();i++) {
        if(l+cutTree.at(i) > x) {
            cnt++;
            l=0;
        }
...
void Solve() {
    cin >> L >> K >> C;
    cut.resize(K);
    rep(i,0,K-1) cin >> cut[i];
    cut.pbk(L);
    
    sort(all(cut));
    cut.erase(unique(cut.begin(), cut.end()), cut.end());

    cutTree.pbk(cut.front());

    for(ll i=1;i<cut.size();i++) {
        cutTree.pbk(cut.at(i)-cut.at(i-1));
    }

    ll low = 1;
    ll high = L;

    ll adequateTreeSize = 0;
    while(low<=high) {
        ll middle = (low+high)/2;

        if(examine(middle)) {
            high = middle-1;
            adequateTreeSize = middle;
        }
        else low = middle+1;
    }

    ll longest=0, firstCut = 0;
    ll l = 0; 
    ll firstIdx = 0;
    int cnt = 0;
    for(ll i=cutTree.size()-1;i>=0;i--) {
        if(l+cutTree.at(i)>adequateTreeSize) {
            firstIdx = i;
            firstCut = cut.at(i);
    ...
    if(cnt<C) firstCut = cut.front();
    longest = max(longest, cut.at(firstIdx));
