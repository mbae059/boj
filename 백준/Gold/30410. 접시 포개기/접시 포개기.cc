...
void Solve() {
    ...
    for(int i=0;i<v.size();i++) {
        ...
        auto [a,b] = p;
        if(a==1 && b%2==0) {
            if(tv.empty() || tv.back().first==1) tv.pbk({2,b/2});
            else tv.back().second += b/2;
        }
        else if(a==1 && b%2==1) {
            tv.pbk(p);
        }
        else if(a==2) {
            if(tv.empty() || tv.back().first==1) tv.pbk({a,b});
            else tv.back().second += b;
        }
    }
    int answer=1;
    for(int i=0;i<tv.size();i++) {
        auto [a, b] = tv[i];
        if(a==1) {
            chmax(answer, getAnswer(b/2));
            continue;
        }

        int l = 0;
        int r = 0;
        if(i-1>=0) l = tv[i-1].second;
        if(i+1<tv.size()) r = tv[i+1].second;

        b += l/2 + r/2;
        chmax(answer, getAnswer(b));
    }
    cout << answer;
}
int32_t main() {
