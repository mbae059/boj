...
//remember that size of set lis is correct but not its element
void LIS(vi& v) { //vector v's size is N
    si lis;
    rep(i,0,N-1) { // N
        int cur = v[i];
        auto iter = lis.lower_bound(cur); //get value that is above cur so that it could be replaced with cur
        if(iter!=lis.end()) lis.erase(iter); //if found, erase that value. if not, cur is the highest value of lis
        lis.insert(cur); //insert cur
    }
    cout << lis.size();
}
/*
