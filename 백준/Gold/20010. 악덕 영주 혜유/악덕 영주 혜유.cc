...
vector<Edge> edge;
vector<pii> dedge[MAX]; //used for Dijkstra
void Kruskal() {
	...
	for (int i = 0; i < N; i++) { //setting nodes
		parent[i] = i;
	}
	int sum = 0;
	for (int i = 0; i < edge.size() ; i++) {
		...
		if (!isSameParent(nodeA, nodeB)) {
			...
			dedge[nodeA].pbk({ nodeB, dis });
			dedge[nodeB].pbk({ nodeA, dis });
...
int Dijkstra(int num) {
	p_q<pii, vpii, cmp> pq;
	pq.push({ num, 0 });
	fill(d + 0, d + N, INF); //INF could be larger, varying from problem to problem
	d[num] = 0;
	while (!pq.empty()) {
