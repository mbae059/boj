...
bool isConnected(const si& s) {
    ...
    while(!q.empty()) {
        ...
        cnt++;
        for(auto next : edge[cur]) {
            if(visited[next]==0 && s.find(next)!=s.end()) {
                visited[next] = 1;
                q.push(next);
            }
        }
...
void dfs(int idx, int cnt) { //implement with dfs(1, 0). N and K must be global variable
	...
	for(int i=idx;i<=N;i++) { //idx, N을 잘볼것
        ...
        if(isConnected(s1) && isConnected(s2)) {
            // print();
            int s1Sum = getPopulation(s1);
            int s2Sum = getPopulation(s2);
            // cout << s1Sum << " : " << s2Sum << endl << endl;
            answer = min(answer, abs(s1Sum-s2Sum));
        }
