...
struct CCTV {
    int kind;
    int y, x;
    CCTV(int kind, int y, int x) : kind(kind), y(y), x(x) {}

    void watch(int dirIdx) {
        switch (kind)
        {
        case 1:
            watchOneDir(dirIdx);
    ...
    void watchOneDir(int dirIdx) {
        int nexty = y, nextx = x;
        while(true) {
            nexty += dir[dirIdx][0];
            nextx += dir[dirIdx][1];
            if(!inRange(nexty, nextx) || matrix[nexty][nextx]==6) break;
            if(matrix[nexty][nextx]==0 || matrix[nexty][nextx]==9) {
                visited[nexty][nextx]+=1;
                matrix[nexty][nextx] = 9;
            }
        }
    }

    void noWatch(int dirIdx) {
        
        switch (kind)
        {
        case 1:
            noWatchOneDir(dirIdx);
            break;
        case 2:
            if(dirIdx%2==0) {
                noWatchOneDir(0);
    ...
    void noWatchOneDir(int dirIdx) {
        ...
        while(true) {
            ...
            if(!inRange(nexty, nextx) || matrix[nexty][nextx]==6) break;
            if(matrix[nexty][nextx]==9) {
                visited[nexty][nextx] -= 1;
                if(visited[nexty][nextx]==0) matrix[nexty][nextx] = 0;
            }
        }
    }
};
