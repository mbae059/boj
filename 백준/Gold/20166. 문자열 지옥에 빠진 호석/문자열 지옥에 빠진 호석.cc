...
map<string, int> mp;
vector<string> v;
int ring(int num, int limit) {
    if(num==0) return limit;
...
struct Node {
    ...
    Node(int y, int x, string str) : y(y), x(x), str(str) {}
};
void bfs(int a, int b) {
    queue<Node> q;
    string temp(1, matrix[a][b]);
    q.push(Node(a,b,temp));
    ...
    while(!q.empty()) {
        ...
        rep(i,0,7) {
            ...
            q.push(Node(nexty, nextx, str+matrix[nexty][nextx]));
        }
    }
}
void Solve() {
