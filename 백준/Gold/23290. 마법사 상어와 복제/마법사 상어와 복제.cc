...
array<array<Node, 5>,5> copy(const array<array<Node, 5>,5>& matrix) {
    array<array<Node, 5>,5> ret;
    rep(i,1,4) {
        rep(j,1,4) {
            ret[i][j].fish = matrix[i][j].fish;
        }
    }
    return ret;
}
...
void fishMove(array<array<Node, 5>,5>& matrix, pii shark, int s) {
    array<array<Node, 5>,5> after;
    rep(i,1,4) {
        rep(j,1,4) {
            for(auto& f : matrix[i][j].fish) {
                rep(k,1,8) {
                    int y = i + dy[f];
                    int x = j + dx[f];
                    if(!inRangeN(y,x) || (make_pair(y,x)==shark) || matrix[y][x].fishsmell) {
                        f = turnDirection(f);
                        if(k==8) after[i][j].fish.pbk(f);
                        continue;
                    }
                    after[y][x].fish.pbk(f);
                    break;
                }
            }
            after[i][j].fishsmell = matrix[i][j].fishsmell;
        }
    }
    matrix = after;
}

int dfs(int y, int x, int cur, const vi& temp, array<array<Node, 5>,5>& matrix) {
    y += d[temp[cur]][0];
    x += d[temp[cur]][1];

    if(cur==2) {
        return matrix[y][x].fish.size();
    }
    int ret = matrix[y][x].fish.size();
    vi v = matrix[y][x].fish;
    matrix[y][x].fish.clear();
    ret += dfs(y, x, cur+1, temp, matrix);
    matrix[y][x].fish = v;
    return ret;
}
void sharkMove(pii& shark, array<array<Node, 5>,5>& matrix, int s) {
    int mx = -1;
    vi saveMove;
    vi temp;
    rep(i,0,3) {
            ...
            rep(k,0,3) {
                ...
                [&] () {
                    ...
                    for(auto l : temp) {
                        y += d[l][0];
                        x += d[l][1];
                        if(!inRangeN(y,x)) return;
                    }
                    tie(y,x) = shark;
                    int fisheat = dfs(y, x, 0, temp, matrix);
                    if(mx < fisheat) {
                        mx = fisheat;
                        saveMove = temp;
                    }
    ...
    for(auto i : saveMove) {
        y += d[i][0];
        x += d[i][1];
        if(matrix[y][x].fish.size()) matrix[y][x].fishsmell = s;
        matrix[y][x].fish.clear();
    }
}
void fishsmell(array<array<Node, 5>,5>& matrix, int s) {
    rep(i,1,4) {
        rep(j,1,4) {
            if(matrix[i][j].fishsmell<=s-2) matrix[i][j].fishsmell=0;
        }
    }
}

void copyComplete(array<array<Node, 5>,5>& dest, const array<array<Node, 5>,5>& src) {
    rep(i,1,4) {
        rep(j,1,4) {
            for(auto f : src[i][j].fish) {
                dest[i][j].fish.pbk(f);
            }
        }
    }
}
void print(const array<array<Node, 5>,5>& matrix, pii shark) {
    rep(i,1,4) {
        rep(j,1,4) {
            cout << i << " " << j << endl;
            cout << matrix[i][j].fish.size() << " : ";
            print(matrix[i][j].fish);
            cout << "FISHSMELL " << matrix[i][j].fishsmell << endl << endl;
        }
    }
...
void Solve() {
    cin >> M >> S;
    N = 4;
    array<array<Node, 5>,5> matrix;
    rep(i,1,M) {
        int a, b, c; cin >> a >> b >> c;
        matrix[a][b].fish.pbk(c);
    }
    pii shark;
    cin >> shark.first >> shark.second;

    for(int i=1;i<=S;i++) {
        auto cpy = copy(matrix);
        fishMove(matrix, shark, i);
        // print(matrix, shark);
        sharkMove(shark, matrix, i);
        fishsmell(matrix, i);
        copyComplete(matrix, cpy);
    }
    int answer = 0;
    rep(i,1,4) {
        rep(j,1,4) {
            answer += matrix[i][j].fish.size();
        }
    }
    cout << answer;
}
