...
int cnt = 0;
pii dp[MAX] {};
pair<si, int> dfs(int node) { //kind, total
    si folderKind;
    int folderTotal = 0;

    for(auto next : edge[node]) {
        int nextNode = next.first;
        int isFolder = next.second;

        if(isFolder) {
            pair<si, int> subFolder = dfs(nextNode);
            
            for(auto i : subFolder.first) {
                folderKind.insert(i);
            }
            folderTotal += subFolder.second;

        }
        else {
            folderKind.insert(nextNode);
            folderTotal += 1;
        }
    }
    dp[node] = make_pair(folderKind.size(), folderTotal);
    return make_pair(folderKind, folderTotal);
}
void Solve() {
    cin >> N >> M;
    rep(i,1,N+M) {
