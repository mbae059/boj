    ...
    if(signedMatrix[idx][idx]=='-') {
        for(int i=-1;i>=-10;i--) {
            ...
            [&] () {
                rep(j, idx+1, N) {
                    sum += answer[j];
                    if(sum>0 && signedMatrix[idx][j]!='+') return;
                    if(sum<0 && signedMatrix[idx][j]!='-') return;
                    if(sum==0 && signedMatrix[idx][j]!='0') return;
                }
                dfs(idx-1);
            } ();
        }
    }
    else if(signedMatrix[idx][idx]=='+') {
        rep(i,1,10) {
            answer[idx] = i;
            int sum = i;
            [&] () {
