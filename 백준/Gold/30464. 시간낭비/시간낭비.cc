...
int dfs(int cur) {
	if(visited[cur]) return (bw[cur]==-1) ? -1e18 : bw[cur];
	visited[cur] = 1;

	int next = cur - a[cur];
	if(next<1) bw[cur] = dp[cur];
	else bw[cur] = max(1+dfs(next), dp[cur]);
	if(bw[cur]==-1) return -1e18;
	return bw[cur];
}
void Solve() {
    cin >> N;
	...
	for(int i=N-1;i>=1;i--) {
		int next = a[i] + i;
		if(next<=N && dp[next]!=-1) dp[i] = dp[next]+1;
	}
	...
	int cnt = 0;
	while(cur<N) {
		if(bw[cur]>=0) chmax(answer, cnt + bw[cur]);
		if(a[cur]==0) break;
		cur = cur + a[cur];
		cnt++;
	}
