...
void Solve() {
    ...
    double prefixRight[N+1] {};
    double prefixLeft[N+1] {};

    for(int i=2;i<=N;i++) {
        double fatigue = getDist(a[i-1][0], a[i-1][1], a[i][0], a[i][1]);
        if(a[i-1][1] < a[i][1]) prefixRight[i] = prefixRight[i-1] + 3 * fatigue;
        else if(a[i-1][1] == a[i][1]) prefixRight[i] = prefixRight[i-1] + 2 * fatigue;
        else prefixRight[i] = prefixRight[i-1] + fatigue;
    }
    
    for(int i=N-1;i>=1;i--) {
        double fatigue = getDist(a[i+1][0], a[i+1][1], a[i][0], a[i][1]);
        if(a[i][1] > a[i+1][1]) prefixLeft[i] = prefixLeft[i+1] + 3*fatigue;
        else if(a[i+1][1] == a[i][1]) prefixLeft[i] = prefixLeft[i+1] + 2 * fatigue;
        else prefixLeft[i] = prefixLeft[i+1] + fatigue;
    }

    while(Q--) {
        int u, v; cin >> u >> v;
        if(u<v) cout << fixed << setprecision(10) << prefixRight[v] - prefixRight[u] << endl;
        else cout << fixed << setprecision(10) << prefixLeft[v] - prefixLeft[u] << endl;
    }
}
int32_t main() {
