...
#define MAX 111
int N1, N2, M1, M2;
bool puzzle1[MAX][MAX] {};
bool puzzle2[MAX][MAX] {};
bool temp_arr[MAX][MAX] {};
void print() {
    rep(i,0,6) {
        rep(j,0,6) {
            cout << puzzle2[i][j] << " ";
...
void rotate() {
    ...
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++) {
            temp_arr[i][j] = puzzle2[N-j-1][i];
        }
    }
    memcpy(puzzle2, temp_arr, sizeof(puzzle2));
    
    if(N2 < M2) {
        rep(i,0,N-1) {
            rep(j,0,N-1) {
                puzzle2[i][j] = puzzle2[i][j+M2-N2];
            }
        }
    }
    swap(N2, M2);
                ...
                [&] () {
                    rep(k, 0, N2-1) {
                        rep(l, 0, M2-1) {
                            int row = i+k;
                            int column = j+l;
                            if(puzzle1[row][column] && puzzle2[k][l]) {
                                return;
                            }
                        }
                    }
                    int row = max(N1, i+N2);
                    int column = max(M1, j+M2);
