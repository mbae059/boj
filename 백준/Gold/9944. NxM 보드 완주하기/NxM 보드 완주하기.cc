...
pii mv(int y, int x, int dir) {
    while(true) {
        int ny = y+dy[dir];
        int nx = x+dx[dir];

        if(inRange(ny,nx) && visited[ny][nx]==0 && matrix[ny][nx]=='.') {
            visited[ny][nx] = 1;
            y = ny;
            x = nx;
        }
...
bool isEnd() {
    rep(i,1,N) {
        rep(j,1,M) {
            if(visited[i][j]==0 && matrix[i][j]=='.') return false;
        }
    }
    return true;
}
...
void dfs(int y, int x, int step) {
    ...
    rep(i,0,3) {
        int ny = y + dy[i];
        int nx = x + dx[i];
        if(!inRange(ny,nx) || matrix[ny][nx]=='*' || visited[ny][nx]) continue;
        tie(ny,nx) = mv(y,x, i);
        dfs(ny,nx,step+1);
        while(ny!=y || nx!=x) {
            visited[ny][nx] = 0;
...
void Solve() {
    ...
    while(cin >> N >> M) {
        ...
        cout << "Case " << ++idx <<": " << (answer==1e9 ? -1 : answer) << endl;
    }
}
int32_t main() {
