...
void Solve() {
	...
	vvi p(4, vi(N+1));
	vvi prefixP(4, vi(N+1));
	vvi t(4, vi(N+1));
	vvi prefixT(4, vi(N+1));

	rep(i,1,3) {
		rep(j,1,N) {
			cin >> p[i][j];
			prefixP[i][j] = prefixP[i][j-1] + p[i][j];
		}
	}

	rep(j,1,N) {
		chmax(prefixP[3][j], prefixP[3][j-1]);
	}

	rep(i,1,3) {
		rep(j,1,N) {
			cin >> t[i][j];
			prefixT[i][j] = prefixT[i][j-1] + t[i][j];
		}
	}
	...
	rep(j,0,N) {
		int idx = 0;
		for(int l=N;l>=0;l--) {
			while(idx<N && prefixT[1][j] + prefixT[2][l] + prefixT[3][idx+1] <= T) idx++;
			
			if(prefixT[1][j] + prefixT[2][l] + prefixT[3][idx]<=T) chmax(answer, prefixP[1][j] + prefixP[2][l] + prefixP[3][idx]);
		}
	}
	print(answer);
}
