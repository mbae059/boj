...
void rdfs(int cur) {
    for(auto next : edge[cur]) {
        ...
        tinDegree[next]++;
        if(depth[next]%2==0) answer[next] = max(answer[next], answer[cur]);
        else answer[next] = min(answer[next], answer[cur]);
        
        if(tinDegree[next]==inDegree[next]) rdfs(next);
    }
}
void Solve() {
    cin >> N >> R;
