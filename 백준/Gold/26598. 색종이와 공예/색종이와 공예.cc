...
void Solve() {
    ...
    auto doBfs = [&](int yyy, int xxx) -> tuple<int,int,int,int,int> {
        qpii q;
        q.push({yyy,xxx});
        visited[yyy][xxx] = 1;
        int cnt = 0;
        ...
        while(!q.empty()) {
            ...
            rep(i,0,3) {
                ...
                if(matrix[ny][nx]!=matrix[yyy][xxx]) continue;
                visited[ny][nx] = 1;
                q.push({ny,nx});
            }
        }
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            ...
            auto [cnt, y1, x1, y2, x2] = doBfs(i,j);

            if(cnt!=(abs(y2-y1)+1) * (abs(x2-x1)+1)) {
                print("BaboBabo");
                return;
            }
        }
