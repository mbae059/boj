    ...
    while(true) {
        ...
        [&]() {
            for(auto c : str) {
                ...
                int ny = y + dy;
                int nx = x + dx;
                assert(inRange(ny,nx));
                if(matrix[ny][nx]=='#') continue;
                if(matrix[ny][nx]=='b') {
                    int nny = ny +dy;
                    int nnx = nx +dx;
                    if(matrix[nny][nnx]=='b' || matrix[nny][nnx]=='#') continue;
                ...
                bool flag = 1;
                for(auto [a,b] : target) {
                    if(matrix[a][b]!='b') flag=0;
                }
                
                if(flag) {
                    for(auto [a,b] : target) {
                        if(matrix[a][b]=='w') matrix[a][b] = 'W';
                        else if(matrix[a][b]=='b') matrix[a][b] = 'B';
                    }
                    cout << "Game " << cnt++ << ": " << "complete" << endl;
                    rep(i,1,N) {
                        rep(j,1,M) {
                            cout << matrix[i][j];
                        }
