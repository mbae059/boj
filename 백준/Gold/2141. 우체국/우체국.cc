...
void Solve() {
    ...
    sort(all(v));
    prefix[0] = v.front().second;

    for(int i=1;i<N;i++) {
        prefix[i] = prefix[i-1] + v.at(i).second;
    }
    ...
    while(low<=high) {
        ...
        ll leftPeople = prefix[middle];
        ll rightPeople = prefix[N-1] - prefix[middle];

        if(leftPeople >= rightPeople) {
            high = middle-1;
            answer = min(answer,v.at(middle).first);
        }
        else {
            low = middle+1;
            // answer = min(answer, v.at(middle+1).first);
        }
    }
    cout << answer;
}
