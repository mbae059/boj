...
void Solve() {
    ...
    auto dfs = [&](auto&& self, int cur, int parent, int d) -> pii {
        pii ret = {cur,d};
        for(auto [next, dd] : edge[cur]) {
            if(next==parent) continue;
            auto p = self(self, next, cur, d+dd);
            if(ret.second < p.second) ret = p;
        }
        // cout << cur << " " << ret.first << " " << ret.second << endl;
        return ret;
    };
    auto p1 = dfs(dfs, 1, -1, 0);
    auto p2 = dfs(dfs, p1.first, -1, 0);

    if(K==1) cout << p2.second;
    else cout << sum2;
