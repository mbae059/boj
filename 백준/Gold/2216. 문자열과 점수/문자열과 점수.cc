...
int go(int idx1, int idx2) {
    ...
    if(ret!=-1) return ret;
    if(idx1==N) return ret = go(idx1, idx2+1) + B;
    if(idx2==M) return ret = go(idx1+1, idx2) + B;

    //miss idx1
    int ret1 = go(idx1, idx2+1) + B;
    int ret2 = go(idx1+1, idx2) + B;
    int ret3 = go(idx1+1, idx2+1) + (str1[idx1]==str2[idx2] ? A : C);
    return ret = max({ret1,ret2,ret3});
}
void Solve() {
    cin >> A >> B >> C;
    cin >> str1 >> str2;
