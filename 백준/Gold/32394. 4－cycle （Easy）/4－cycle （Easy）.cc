...
void Solve() {
    ...
    auto hasFourVertex = [](int a, int b, int c, int d) {
    ...
    for(int i=0;i<M;i++) {
        for(int j=i+1;j<M;j++) {
            ...
            if(!hasFourVertex(a,b,c,d)) continue;
            if(s.contains({a,c}) && s.contains({b,d})) answer++;
            if(s.contains({a,d}) && s.contains({b,c})) answer++;
        }
    }
