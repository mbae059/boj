...
using tiiiii = tuple<int, int, int, int, int> ; //get<0>(t);
...
void Solve() {
    ...
    rep(i,1,N) {
        rep(j,1,N) {
            if(inRangeN(i+1,j)) chmin(dp[i+1][j], dp[i][j]+ (matrix[i+1][j]>=matrix[i][j] ? matrix[i+1][j]-matrix[i][j]+1:0));
            if(inRangeN(i,j+1))chmin(dp[i][j+1], dp[i][j]+ (matrix[i][j+1]>=matrix[i][j] ? matrix[i][j+1]-matrix[i][j]+1:0));
        }
    }
    cout << dp[N][N];
}
