...
bool inRange(int y, int x) {
    return 1<=y && y+A-1<=N && 1<=x && x+B-1<=M;
}
bool isContact(pii a) {
    for(int i=a.first;i<a.first+A;i++) {
        for(int j=a.second;j<a.second+B;j++) {
...
void Solve() {
    ...
    while(!q.empty()) {
        ...
        rep(i,0,3) {
            ...
            if(d[nexty][nextx]!=-1) continue;
            if(isContact({nexty,nextx})) continue;
