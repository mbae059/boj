...
void print(const vvi& v, int s1, int s2, int e1, int e2) {
    rep(i,s1,s2) {
        rep(j,e1,e2) {
            cout << v[i][j] << " ";
        }
...
int getDSU(int y, int x) {
    return (y-1)*M+x;
}
void Solve() {
    cin >> N >> M;
    vvi matrix(N+1, vi(M+1));
    mii inDegree;
    mii answer;
    rep(i,1,N) {
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            rep(k,0,3) {
                ...
                if(matrix[i][j]==matrix[ny][nx]) dsu.merge(getDSU(i,j), getDSU(ny,nx));
            }
        }
    }
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            rep(k,0,3) {
                ...
                int nx = j + dx[k];
                if(!inRange(ny,nx)) continue;
                if(dsu.isSameParent(getDSU(i,j), getDSU(ny,nx))) continue;
                if(matrix[i][j]>matrix[ny][nx]) {
                    inDegree[dsu.getParent(getDSU(i,j))]++;
                }
            }
        }
    }
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            if(inDegree[dsu.getParent(getDSU(i,j))]==0) {
                q.push({i,j});
                visited[i][j] = 1;
            }
        }
    ...
    while(!q.empty()) {
        ...
        rep(i,0,3) {
            ...
            if(matrix[ny][nx]>matrix[y][x]) {
                inDegree[dsu.getParent(getDSU(ny,nx))]--;
                chmax(answer[dsu.getParent(getDSU(ny,nx))], answer[dsu.getParent(getDSU(y,x))] + 1);
            }
            else if(matrix[ny][nx]==matrix[y][x]) answer[dsu.getParent(getDSU(ny,nx))] = answer[dsu.getParent(getDSU(y,x))];
            if(inDegree[dsu.getParent(getDSU(ny,nx))]!=0) continue;
            q.push({ny,nx});
            visited[ny][nx] = 1;
        }
    }
    rep(i,1,N) {
        rep(j,1,M) cout << answer[dsu.getParent(getDSU(i,j))] << " ";
        cout << endl;
    }
}
int32_t main() {
