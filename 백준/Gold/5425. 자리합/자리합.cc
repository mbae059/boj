...
int go(int idx, int sum, bool tight, const vi& digits) {
    if(idx==-1) return sum;
    ...
    int ret = 0;
    for(int i=0;i<=k;i++) {
        bool ntight = digits[idx]==i ? tight : 0;
        ret += go(idx-1, sum+i, ntight, digits);
    }
    return dp[idx][sum][tight] = ret;
}
int getDigitDP(int num) {
    memset(dp,-1,sizeof(dp));
    auto digits = getDigit(num);
    return go(digits.size()-1, 0, 1, digits);
}
void Solve() {
    cin >> A >> B;
    // if(A==0) cout << getDigitDP(B) << endl;
    cout << getDigitDP(B) - getDigitDP(A-1) << endl;
}
int32_t main() {
