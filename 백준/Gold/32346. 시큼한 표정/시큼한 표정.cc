...
void Solve() {
    ...
    vi right(N+1);
    for(int i=1;i<=N;i++) {
        if(str[i]=='<') left[i] = left[i-1] + 1;
        if(str[i]=='>') right[i] = right[i-1] + 1;
    }
    
    vmint dp1(N+1, 1);
    vector<vmint> dp2(N+1, vmint(N+1));
    ...
    rep(i,0,N) {
        rep(j,i,N) {
            dp2[i][j] = dp2[j][i] = dp1[i+j] /(dp1[i] *dp1[j]);
        }
    }
    auto getCnt = [&](int l, int r) -> mint {
        int mid = l+r>>1;
        int lcnt = right[mid] - right[l-1];
        int rcnt = left[r] - left[mid];
        if(lcnt==rcnt && lcnt+rcnt==r-l+1) return dp2[l-1][N-r];
        return 0;
    };
