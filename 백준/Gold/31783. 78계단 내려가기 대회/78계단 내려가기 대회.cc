...
struct Segtree {
    ...
    T query(int node, int l, int r, int nodeLeft, int nodeRight) {
        if(l>r) return -1e18;
        if (nodeRight < l || r < nodeLeft) return -1e18; //could be 0, 1e9, -1e9
...
void Solve() {
    ...
    Segtree<int, bop> segtree(N);

    sort(all(v));
    
    rep(i,2,N) {
        int idx = N - (lower_bound(all(v), h[i]+b[i]) - v.begin());
        int mx1 = segtree.query(1,1,idx,1,N);
        int mx2 = segtree.query(1,1,i-1,1,N);
        segtree.update(1,1,N,i,max(mx1+a[i], mx2));
    }
    cout << segtree.query(1,1,N,1,N);
}
int32_t main() {
