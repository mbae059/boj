...
void Solve() {
    ...
    vvi volcano(N+1, vi(M+1));
    vvi inDegree(N+1, vi(M+1));
    rep(i,1,N) {
        rep(j,1,M) cin >> src[i][j];
    }
    
    rep(i,1,N) {
        rep(j,1,M) cin >> dest[i][j];
    }

    rep(i,1,N) {
        rep(j,1,M) {
            matrix[i][j] = dest[i][j] - src[i][j];
        }
    }
    ...
    qpii q;
    //for when the indegree of the node is 1
    rep(i,1,N) {
        rep(j,1,M) {
            if(volcano[i][j]) continue;
            rep(k,0,1) {
                int ny = i+dy[k];
                int nx = j+dx[k];
    ...
    rep(i,1,N) {
        rep(j,1,M) {
            if(volcano[i][j] || visited[i][j]) continue;
            ...
            while(!q.empty()) {
                auto [y,x] = q.front();
                q.pop();
                if((y+x)%2) sum1 += matrix[y][x];
                else sum2 += matrix[y][x];
                for(int k=0;k<4;k++) {
                    int ny = y+dy[k];
                    int nx = x+dx[k];
                    if(!inRange(ny,nx)) continue;
