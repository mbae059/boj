...
void Solve() {
    ...
    auto isSlump = [](auto&& self, string& str, int cur) -> bool {
        if(str.size()<=cur) return false;
        if(cur==0) return (str[0]=='D' || str[0]=='E') && self(self, str, 1);
        if(cur==1) return (str[1]=='F' && self(self, str, 2));

        bool b1 = str[cur]=='F' && self(self, str, cur+1);
        bool b2 = str[cur]=='G' && str.size()==cur+1;
        string tstr = str.substr(cur);
        bool b3 = self(self, tstr, 0);

        return b1||b2||b3;
    };

    auto isSlimp = [&isSlump](auto&& self, string& str, int cur) -> bool {
        if(str.size()<=cur) return false;
        if(cur==0) return (str[0]=='A' && self(self, str, 1));
        else if(str.size()==2) return str[1]=='H';
        string tstr1 = str.substr(2, str.size()-3);
        string tstr2 = str.substr(1, str.size()-2);

        return (str[1]=='B' && self(self, tstr1, 0) && str[str.size()-1]=='C') || (isSlump(isSlump, tstr2, 0) && str[str.size()-1]=='C');
    };
    ...
    for(int i=1;i<str.size();i++) {
        ...
        if(isSlimp(isSlimp, str1, 0) && isSlump(isSlump,str2,0)) {
            print("YES");
            return;
        }
    }
