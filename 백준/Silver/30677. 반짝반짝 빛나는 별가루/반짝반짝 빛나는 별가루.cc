...
int A, B, D, E, F, G, H, I, J, K, L, M, N, O, P, Q, S, T, U, V, W, X, Y, Z;
...
void Solve() {
    ...
    rep(i,1,N) {
        if(q[i]==0) {
            fatigue = max(fatigue - R, 0ll);
            continue;
        }
        
        int idx = q[i];

        double val = base[idx] * (100 + combo[i-1] * C) * (100 + skill[idx] * s[idx]);
        answer += val / 10000;
        skill[idx] += 1;
        fatigue += p[idx];
        if(fatigue>100) {
