...
int floatingNumberMul(string str, int a) {
    int ret = 0;
    if(str.find('.')==-1) return stoi(str);
    
    auto x = str.size();
    ret = stoi(str.substr(0,x-2)) * a + (str.back()-'0') * a / 10;
    return ret;
}
void Solve() {
    cin >> N;
    ...
    int answer=0;
    rep(i,1,N) {
        int x = floatingNumberMul(k[i], a[i]) - b[i];
        int y = - floatingNumberMul(k[i],b[i]) + a[i];
        answer = max(answer + x, answer + y);
    }
    cout << answer;
}
int32_t main() {
