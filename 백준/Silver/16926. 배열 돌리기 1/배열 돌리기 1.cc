...
void rotate(int r) {
    if(N-1-r-r+1 >= 3) swap(matrix[r][r], matrix[r+1][r]);

    for(int i=r;i<M-1-r;i++) {
        swap(matrix[r][i], matrix[r][i+1]);
    }
    for(int i=r;i<N-1-r;i++) {
        swap(matrix[i][M-1-r], matrix[i+1][M-1-r]);
    }

    for(int i=M-1-r;i>r;i--) {
        swap(matrix[N-1-r][i], matrix[N-1-r][i-1]);
    }
    for(int i=N-1-r;i>r+2;i--) {
        swap(matrix[i][r], matrix[i-1][r]);
    }
}
void Solve() {
    cin >> N >> M >> R;
