...
void Solve() {
    ...
    int monoPrediction = 0;
    rep(i,1,N) {
        int val = 0;
        rep(j,1,M) {
            if(src[j]==arr[i][j]) val++;
        }
        monoPrediction = max(monoPrediction, val);
    }

    for(int i=0;i<(1<<N);i++) {
        if(__popcount(i)<=1 || __popcount(i)%2==0) continue;
        ...
        int ensemble = 0;
        for(int j=1;j<=M;j++) {
            int val = decide[j][0] > decide[j][1] ? 0 : 1;
            if(val==src[j]) ensemble++;
        }
        if(ensemble > monoPrediction) {
            cout << 1;
            return;
        }
    }
