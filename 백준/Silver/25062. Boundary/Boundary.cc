...
void Solve() {
    ...
    int exterior = N*M - (N-2) * (M-2);

    vi v;
    v.pbk(1);

    auto doesFit = [&](int x) {
        if(a%x==0 && (b-2)%x==0) return true;
        if(a%x==1 && (b-1)%x==0) return true;
        
        if(b%x==0 && (a-2)%x==0) return true;
        if(b%x==1 && (a-1)%x==0) return true;
        
        if(a%x==1 && (b-2)%x==0 && b%x==0) return true;
        if(b%x==1 && (a-2)%x==0 && a%x==0) return true;

        return false;
    };

    for(int i=2;i*i<=exterior;i++) {
        if(doesFit(i)) v.pbk(i);
        if(doesFit(exterior/i)) v.pbk(exterior/i);
    }
