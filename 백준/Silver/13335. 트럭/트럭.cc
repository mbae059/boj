...
int KMP(string w_str, string p_str) {
   int idx = 0;
   int compare_cnt = 0;
   int matchLen = 0;
   size_t pattSize = p_str.size();
   size_t textSize = w_str.size();
   while (true) {
      matchLen = 0;
      if (isValid(idx + pattSize, textSize)) {
         int temp_idx = 0;
         string compare_str = w_str.substr(idx, pattSize);
         compare_cnt++;
         while (compare_str[temp_idx] == p_str[temp_idx]) {
            temp_idx++;
            matchLen++;
            if (temp_idx == pattSize) return compare_cnt;
            compare_cnt++;}
         if (idx >= textSize - 1) return compare_cnt;
         else idx += distance(p_str, matchLen);
      }
      else return -1;
   }
}
