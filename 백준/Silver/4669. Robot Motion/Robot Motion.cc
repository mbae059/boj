...
void Solve() {
    while(true) {
        ...
        while(!q.empty()) {
            ...
            int ny = y+mp[matrix[y][x]].first;
            int nx = x+mp[matrix[y][x]].second;
            if(!inRange(ny,nx)) {
                cout << visited[y][x] << " step(s) to exit" << endl;
                break;
            }
            if(visited[ny][nx]) {
                cout << visited[ny][nx]-1 << " step(s) before a loop of " << visited[y][x]-visited[ny][nx]+1 << " step(s)" << endl;
                break;
            }
            q.push({ny,nx});
