...
void Solve() {
    cin >> N >> M;
    vtiii compass(M);
    for(auto& [a,b,c] : compass) cin >> a >> b >> c;

    rep(i,1,N) {
        rep(j,1,N) {
            [&] () {
                for(auto [a,b,c] : compass) {
                    if(c==1 && i<a && j==b) continue;
                    if(c==2 && i<a && j>b) continue;
                    if(c==3 && i==a && j>b) continue;
                    if(c==4 && i>a && j>b) continue;
                    if(c==5 && i>a && j==b) continue;
                    if(c==6 && i>a && j<b) continue;
                    if(c==7 && i==a && j<b) continue;
                    if(c==8 && i<a && j<b) continue;
                    return;
                }
                cout << i << " " << j;
                exit(0);
