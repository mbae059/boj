...
void Solve() {
    ...
    vi factors = getFactors(T);

    int answer=0;
    while(N--) {
        int x; cin >> x;
        auto l = lower_bound(all(factors), x);
        auto r = lower_bound(all(factors), x);

        if(l==factors.begin()) continue;
        l = prev(l, 1);
        
        if(r==factors.end()) answer += (x - *l);
        else answer += min((x-*l), (*r-x));
    }
    print(answer);
}
int32_t main() {
