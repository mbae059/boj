...
void Solve() {
    ...
    for(char c='a';c<='z';c++) {
        int alphaIdx = c-'a';
        prefix[alphaIdx][0] = (str.front()==c ? 1 : 0);
        for(int i=1;i<=str.size()-1;i++) {
            prefix[alphaIdx][i] = prefix[alphaIdx][i-1] + (str.at(i)==c ? 1 : 0);
        }        
    }
    ...
    rep(i,1,N) {
        char c; cin >> c;
        int a, b; cin >> a >> b;
        if(a==0) cout << prefix[c-'a'][b] << endl;
        else cout << prefix[c-'a'][b]-prefix[c-'a'][a-1] << endl;
    }
