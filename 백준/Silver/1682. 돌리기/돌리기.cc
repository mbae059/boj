...
void Solve() {
    ...
    rep(i,0,3) cin >> target[0][i];
    rep(i,0,3) cin >> target[1][3-i];
    
    auto doA = [](const vvi& v) {
        vvi ret(2, vi(4));
        rep(i,0,3) ret[0][i] = v[1][i];
        rep(i,0,3) ret[1][i] = v[0][i];
        return ret;
    };

    auto doB = [](const vvi& v) {
        vvi ret(2, vi(4));
        rep(i,1,3) {
            ret[0][i] = v[0][i-1];
            ret[1][i] = v[1][i-1];
    ...
    auto doD = [](const vvi& v) {
        vvi ret = v;
        swap(ret[0][0], ret[1][3]);
        return ret;
    };
    ...
    while(!q.empty()) {
        ...
        for(auto nv : {nva, nvb, nvc, nvd}) {
            if(s.contains(nv)) continue;
            s.insert(nv);
            q.push({nv, cnt+1});
        }
    }
