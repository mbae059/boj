...
void Solve() {
    ...
    pq.push({0,1}); //distance, idx
    while(!pq.empty()) {
        auto [dist, cur] = pq.top();
        pq.pop();
        if(dist > d1[cur]) continue;
        for(auto [next, d] : edge[cur]) {
            int nextDistance = dist + d;
            if(d1[next]<nextDistance) continue;
            pq.push({nextDistance, next});
            d1[next] = nextDistance;
        }
    }

    pq.push({0,N}); //distance, idx
    while(!pq.empty()) {
        auto [dist, cur] = pq.top();
        pq.pop();
        ...
        for(auto [next, d] : edge[cur]) {
            int nextDistance = dist + d;
            if(d2[next]<nextDistance) continue;
            pq.push({nextDistance, next});
    ...
    for(int i=1;i<=N;i++) {
        if(t[i]==-1) continue;
        if(d1[i]==1e18) continue;
        if(d2[i]==1e18) continue;
        chmin(answer, d1[i] + t[i]*(K-1) + d2[i]);
    }
