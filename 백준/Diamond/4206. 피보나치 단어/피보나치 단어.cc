...
void init_fibo() {
    ...
    for(int i=2;i<k;i++) {
        fibo_string[i] = fibo_string[i-1] + fibo_string[i-2];
    }
}
...
int kmp(const string& base, const vi& pi, const string& pattern) {
    int ret= 0 ;
...
void Solve() {
    ...
    while(cin >> N >> pattern) {
        ...
        for(int i=0;i<k;i++) {
            dp[i] = kmp(fibo_string[i], pi, pattern);
        }

        const string str1 = fibo_string[k-1].substr(fibo_string[k-1].size() - (pattern.size() - 1)) + fibo_string[k-2].substr(0, pattern.size()-1);
        const string str2 = fibo_string[k-2].substr(fibo_string[k-2].size() - (pattern.size() - 1)) + fibo_string[k-2].substr(0, pattern.size()-1);


        int between[2] = {kmp(str1, pi, pattern), kmp(str2, pi, pattern)};

        int x = 0;
        for(int i=k;i<=100;i++, x = 1-x) {
            dp[i] = dp[i-1]+dp[i-2] + between[x];
        }
        cout << "Case " << idx << ": " << dp[N] << endl;
        idx++;
    }
}
int32_t main() {
